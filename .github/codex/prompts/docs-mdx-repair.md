# Kova Docs MDX Repair Agent

You are repairing generated Kova documentation after a fast MDX validation failure.
...
1. Read `.kova-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".kova-sync/mdx/${LOCALE}.json"`.
