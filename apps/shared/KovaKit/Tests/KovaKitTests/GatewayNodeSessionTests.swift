...
struct GatewayNodeSessionTests {
    ...
    func scannedSetupCodePrefersBootstrapAuthOverStoredDeviceToken() async throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["KOVA_STATE_DIR"]
        setenv("KOVA_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("KOVA_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("KOVA_STATE_DIR")
            }
    ...
    func normalizeCanvasHostUrlPreservesExplicitSecureCanvasPort() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "https://canvas.example.com:9443/__kova__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com")!)

        #expect(normalized == "https://canvas.example.com:9443/__kova__/cap/token")
    }

    @Test
    func normalizeCanvasHostUrlBackfillsGatewayHostForLoopbackCanvas() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "http://127.0.0.1:18789/__kova__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com:7443")!)

        #expect(normalized == "https://gateway.example.com:7443/__kova__/cap/token")
    }
