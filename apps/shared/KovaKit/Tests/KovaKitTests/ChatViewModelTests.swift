...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> KovaChatHistoryPayload
{
    KovaChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> KovaChatSessionEntry {
    KovaChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> KovaChatSessionEntry
{
    KovaChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> KovaChatModelChoice {
    KovaChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [KovaChatHistoryPayload],
    sessionsResponses: [KovaChatSessionsListResponse] = [],
    modelResponses: [[KovaChatModelChoice]] = [],
    ...
    -> (TestChatTransport, KovaChatViewModel)
{
...
private func sendUserMessage(_ vm: KovaChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, KovaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KovaChatHistoryPayload]
    private let sessionsResponses: [KovaChatSessionsListResponse]
    private let modelResponses: [[KovaChatModelChoice]]
    ...
    private let stream: AsyncStream<KovaChatTransportEvent>
    private let continuation: AsyncStream<KovaChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<KovaChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<KovaChatTransportEvent> {
        self.stream
    }
    ...
    func requestHistory(sessionKey: String) async throws -> KovaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KovaChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [KovaChatAttachmentPayload]) async throws -> KovaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return KovaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KovaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KovaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [KovaChatModelChoice] {
    ...
    func emit(_ evt: KovaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = KovaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = KovaChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: KovaChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = KovaChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KovaChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(KovaChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == KovaChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = KovaChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KovaChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = KovaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = KovaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = KovaChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { KovaChatViewModel(sessionKey: "main", transport: transport) }
