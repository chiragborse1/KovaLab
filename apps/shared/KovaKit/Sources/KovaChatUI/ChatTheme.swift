...
enum KovaPlatformImageFactory {
    static func image(_ image: KovaPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
