...
struct ChatMessageBubble: View {
    let message: KovaChatMessage
    let style: KovaChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? KovaChatTheme.userText : KovaChatTheme.assistantText
    ...
    private var inlineAttachments: [KovaChatMessageContent] {
    ...
    private var toolCalls: [KovaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [KovaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? KovaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return KovaChatTheme.onboardingAssistantBubble
        }
        return KovaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return KovaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: KovaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? KovaChatTheme.userText : KovaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: KovaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(KovaChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [KovaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: KovaChatTheme.assistantText)
            }
        }
    }
}
