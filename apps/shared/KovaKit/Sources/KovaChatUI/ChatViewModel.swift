...
private let chatUILogger = Logger(subsystem: "ai.kova", category: "KovaChatUI")

@MainActor
@Observable
public final class KovaChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [KovaChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [KovaChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [KovaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KovaChatPendingToolCall] = []
    public private(set) var sessions: [KovaChatSessionEntry] = []
    private let transport: any KovaChatTransport
    private var sessionDefaults: KovaChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: KovaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any KovaChatTransport,
    ...
    public var sessionChoices: [KovaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [KovaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KovaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KovaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KovaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: KovaChatMessage) -> KovaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> KovaChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return KovaChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: KovaChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: KovaChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: KovaChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [KovaChatMessage],
        incoming: [KovaChatMessage]) -> [KovaChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [KovaChatMessage]) -> [KovaChatMessage] {
        var result: [KovaChatMessage] = []
    ...
    private static func dedupeKey(for message: KovaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KovaChatMessageContent] = [
            KovaChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> KovaChatAttachmentPayload in
            KovaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KovaChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = KovaChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: KovaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KovaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KovaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KovaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KovaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
