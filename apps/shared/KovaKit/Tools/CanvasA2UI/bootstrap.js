...
import "@a2ui/lit/ui";
import { themeContext } from "@kovaai/a2ui-theme-context";
...
class KovaA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.kovaA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["kova:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.kovaCanvasA2UIAction ??
      globalThis.kovaCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.kovaCanvasA2UIAction) {
...
if (!customElements.get("kova-a2ui-host")) {
  customElements.define("kova-a2ui-host", KovaA2UIHost);
}
