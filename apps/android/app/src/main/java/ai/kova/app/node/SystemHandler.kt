...
private const val NOTIFICATION_CHANNEL_BASE_ID = "kova.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Kova Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Kova Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Kova Active")
      }
