...
import ai.kova.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      KovaCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      KovaCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      KovaCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      KovaCanvasA2UICommand.Push.rawValue, KovaCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      KovaCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      KovaCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      KovaCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      KovaLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      KovaDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      KovaDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      KovaDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      KovaDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      KovaNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      KovaNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      KovaSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.kova.app.protocol.KovaPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      KovaContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      KovaContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      KovaCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      KovaCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      KovaMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      KovaMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      KovaSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      KovaSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      KovaCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
