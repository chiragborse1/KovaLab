...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Kova Android gateway connection failure.
    ...
    - tell me whether `kova devices list` should show a pending pairing request
    - if more signal is needed, ask for `kova qr --json`, `kova devices list`, and `kova nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Kova gateway diagnostics", report))
