...
val androidStoreFile = providers.gradleProperty("KOVA_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("KOVA_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("KOVA_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("KOVA_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set KOVA_ANDROID_STORE_FILE, " +
            "KOVA_ANDROID_STORE_PASSWORD, KOVA_ANDROID_KEY_ALIAS, and " +
            "KOVA_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/KovaKit/Sources/KovaKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.kova.app"
        minSdk = 31
        targetSdk = 36
        versionCode = 2026050100
        versionName = "0.2.0"
        ndk {
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "kova-$versionName-$buildType.apk"
                    } else {
                        "kova-$versionName-$flavorName-$buildType.apk"
                    }
