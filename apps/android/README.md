...
- Play build: `apps/android/build/release-bundles/kova-<version>-play-release.aab`
- Third-party build: `apps/android/build/release-bundles/kova-<version>-third-party-release.aab`
...
- Canvas web content already supports live reload when loaded from Gateway `__kova__/canvas/` (see `docs/platforms/android.md`).
...
- `SEND_SMS`
- `READ_CALL_LOG`
- `READ_MEDIA_IMAGES`, `READ_MEDIA_VISUAL_USER_SELECTED`, and legacy `READ_EXTERNAL_STORAGE` for photo library access
...
2) Android app is connected to that gateway and `kova nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device (do not run gateway with `KOVA_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__kova__/`).
...
- `KOVA_ANDROID_GATEWAY_URL=ws://...` (default: from your local Kova config)
- `KOVA_ANDROID_GATEWAY_TOKEN=...`
- `KOVA_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - approve pending device pairing (`kova devices approve --latest`) and rerun.
