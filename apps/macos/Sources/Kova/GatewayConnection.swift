...
private let gatewayConnectionLogger = Logger(subsystem: "ai.kova", category: "gateway.connection")
...
/// (ControlChannel, debug actions, SwiftUI LocalChat, etc.).
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: KovaProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(KovaGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> KovaSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return KovaSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> KovaChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [KovaChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> KovaChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
