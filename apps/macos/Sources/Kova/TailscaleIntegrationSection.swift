...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.chiragborse1/KovaLab.json. Prefer KOVA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.chiragborse1/KovaLab.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.chiragborse1/KovaLab.json. Restart the gateway to apply."
        }
