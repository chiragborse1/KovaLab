...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = KovaConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            KovaConfigFile.canonicalHostForComparison(host) ==
                KovaConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = KovaConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: KovaConfigFile.loadDict(),
            draft: .init(
        ...
        KovaConfigFile.saveDict(synced.root)
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/kova"
        state.remoteCliPath = ""
        return state
    }
}
