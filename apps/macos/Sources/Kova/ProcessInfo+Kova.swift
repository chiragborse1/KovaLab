...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.kova.mac`) even if the shipped
    ...
    {
        if environment["KOVA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "kova.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "kova.nixMode") { return true }
