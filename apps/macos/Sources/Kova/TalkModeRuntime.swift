...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.kova", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.kova", category: "talk.tts")
    ...
    private func sendAndSpeak(_ transcript: String) async {
        ...
        let activeSessionKey = await MainActor.run { LocalChatManager.shared.activeSessionKey }
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [KovaChatMessage] = messages.compactMap { item in
