...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.kova", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = KovaConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: KovaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: KovaLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        var caps: [String] = [KovaCapability.canvas.rawValue, KovaCapability.screen.rawValue]
        if browserControlEnabled, connectionMode == .local {
            caps.append(KovaCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(KovaCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(KovaCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: KovaConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: KovaLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            KovaSystemCommand.which.rawValue,
            KovaSystemCommand.run.rawValue,
            KovaSystemCommand.execApprovalsGet.rawValue,
            KovaSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(KovaCapability.browser.rawValue) {
            commands.append(KovaBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(KovaCapability.camera.rawValue) {
        ...
        if capsSet.contains(KovaCapability.location.rawValue) {
