...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.kova", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KovaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KovaSessionPreviewEntry,
    ...
    private static func cache(payload: KovaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KovaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KovaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KovaKit.AnyCodable]) -> [KovaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KovaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KovaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KovaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KovaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KovaChatMessage) -> String? {
