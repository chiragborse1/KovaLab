...
struct MacGatewayChatTransport: KovaChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> KovaChatSessionsListResponse {
        ...
        let defaults = decoded.defaults.map {
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return KovaChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [KovaChatAttachmentPayload]) async throws -> KovaChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KovaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KovaGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: KovaProtocol.ModelChoice) -> KovaChatModelChoice {
        KovaChatModelChoice(
            modelID: model.id,
            name: model.name,
