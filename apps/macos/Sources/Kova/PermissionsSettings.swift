    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so Kova can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = KovaLocationMode.off.rawValue
    ...
    private var locationMode: KovaLocationMode {
        KovaLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: KovaLocationMode) async -> Bool {
