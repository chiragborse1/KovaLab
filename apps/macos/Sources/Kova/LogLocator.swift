...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["KOVA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/kova")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("kova-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("kova-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/kova/ (rolling or stdout), or nil if none exist.
