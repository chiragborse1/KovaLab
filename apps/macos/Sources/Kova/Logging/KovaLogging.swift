...
enum KovaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KovaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KovaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KovaLogging.bootstrapIfNeeded()
        let label = KovaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KovaOSLogHandler: AppLogLevelBackedHandler {
...
struct KovaFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KovaLogging.parseLabel(self.label)
