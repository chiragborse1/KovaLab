...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KovaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.kova", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KovaKit.AnyCodable]()) {
                $0[$1.key] = KovaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: KovaProtocol.AnyCodable?) -> [String: KovaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KovaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KovaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KovaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("kova.control.heartbeat")
    static let controlAgentEvent = Notification.Name("kova.control.agent")
}
