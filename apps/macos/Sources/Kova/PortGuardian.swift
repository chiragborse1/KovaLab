...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.kova", category: "portguard")
    #if DEBUG
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "kova", "tsx", "pnpm", "bun"]
    ...
    private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("kova-gateway")
                || cmd.contains("kova-gateway")
            {
                return true
            }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("kova"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
