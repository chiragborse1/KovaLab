...
enum CommandResolver {
    private static let projectRootDefaultsKey = "kova.gatewayProjectRootPath"
    private static let helperName = "kova"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let kovaEntry = root.appendingPathComponent("kova.mjs").path
        if FileManager().isReadableFile(atPath: kovaEntry) { return kovaEntry }
        let binEntry = root.appendingPathComponent("bin/kova.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/kova")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let kovaPaths = self.kovaManagedPaths(home: home)
        if !kovaPaths.isEmpty {
            extras.insert(contentsOf: kovaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + kovaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func kovaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".kova"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func kovaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectKovaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("kova.mjs").path,
            root.appendingPathComponent("bin/kova.js").path,
        ]
    ...
    static func hasAnyKovaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.kovaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let kovaPath = self.projectKovaExecutable(projectRoot: root) {
            return [kovaPath, subcommand] + extraArgs
        }
        if let kovaPath = self.kovaExecutable(searchPaths: searchPaths) {
            return [kovaPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "kova", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            kova entrypoint missing (looked for dist/index.js or kova.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real kova CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/kova"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/kova.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/kova.mjs"
            node "$PRJ/kova.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/kova.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/kova.js"
            node "$PRJ/bin/kova.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent kova"
          pnpm --silent kova \(quotedArgs);
        else
          echo "kova CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KovaConfigFile.loadDict()
