...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.kova", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["KOVA_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = KovaConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.kovaExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "kova CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["KOVA_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "kova@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "kova@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "kova@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "kova@\(target)"])
            }

        statusHandler("Installing kova@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed kova@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("kova ") {
            normalized = String(normalized.dropFirst("kova ".count))
        }
