...
struct KovaMacCLI {
    static func main() async {
        ...
        default:
            fputs("kova-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      kova-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      kova-mac discover [--timeout <ms>] [--json] [--include-local]
      kova-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      kova-mac connect
      kova-mac connect --url ws://127.0.0.1:18789 --json
      kova-mac discover --timeout 3000 --json
      kova-mac wizard --mode local
    """)
}
