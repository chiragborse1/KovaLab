...
@Suite(.serialized)
struct KovaConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("kova-config-\(UUID().uuidString)")
            .appendingPathComponent("kova.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["KOVA_CONFIG_PATH": override]) {
            #expect(KovaConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["KOVA_CONFIG_PATH": override]) {
            ...
            #expect(KovaConfigFile.remoteGatewayPort() == 19999)
            #expect(KovaConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(KovaConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(KovaConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(KovaConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(KovaConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["KOVA_CONFIG_PATH": override]) {
            ...
            KovaConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = KovaConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["KOVA_CONFIG_PATH": override]) {
            ...
            KovaConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = KovaConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["KOVA_CONFIG_PATH": override]) {
            ...
            KovaConfigFile.clearRemoteGatewayUrl()
            let root = KovaConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("kova-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "KOVA_CONFIG_PATH": nil,
            "KOVA_STATE_DIR": dir,
        ]) {
            #expect(KovaConfigFile.stateDirURL().path == dir)
            #expect(KovaConfigFile.url().path == "\(dir)/kova.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("kova-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("kova.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "KOVA_STATE_DIR": stateDir.path,
            "KOVA_CONFIG_PATH": configPath.path,
        ]) {
            KovaConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-kova-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = KovaConfigFile.loadDict()
