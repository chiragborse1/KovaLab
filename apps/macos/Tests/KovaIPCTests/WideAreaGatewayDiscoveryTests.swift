...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("KOVA_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("KOVA_WIDE_AREA_DOMAIN", "kova.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("KOVA_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("KOVA_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._kova-gw._tcp.kova.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.kova.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Kova)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/kova/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Kova)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/kova/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._kova-gw._tcp.kova.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
