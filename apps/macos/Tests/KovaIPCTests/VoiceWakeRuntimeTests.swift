...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["kova"]
        let text = "uh kova"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "kova")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["kova", "hey kova"]
        let text = "hey kova"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey kova")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey kova do thing"
        let config = WakeWordGateConfig(triggers: ["kova"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["kova"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "kova")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "kova tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["kova", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["kova", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "kova")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["kova"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "kova", start: 0.2, duration: 0.1, range: transcript.range(of: "kova")),
