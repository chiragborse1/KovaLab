...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: KovaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KovaProtocol.AnyCodable] = [
            "presence": .init([KovaProtocol.AnyCodable(entry)]),
        ]
        let payload = KovaProtocol.AnyCodable(payloadMap)
