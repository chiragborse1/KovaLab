...
final class RemindersService: RemindersServicing {
    func list(params: KovaRemindersListParams) async throws -> KovaRemindersListPayload {
        ...
        let payload: [KovaReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return KovaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: KovaRemindersAddParams) async throws -> KovaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = KovaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return KovaRemindersAddPayload(reminder: payload)
    }
