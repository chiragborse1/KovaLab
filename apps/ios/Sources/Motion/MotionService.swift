...
final class MotionService: MotionServicing {
    func activities(params: KovaMotionActivityParams) async throws -> KovaMotionActivityPayload {
        ...
        let mapped: [KovaMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return KovaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: KovaPedometerParams) async throws -> KovaPedometerPayload {
        ...
        let payload: KovaPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = KovaPedometerPayload(
