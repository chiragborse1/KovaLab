...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> KovaDeviceStatusPayload {
        ...
        return KovaDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> KovaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: KovaBatteryState = switch device.batteryState {
        ...
        return KovaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> KovaThermalStatusPayload {
        let state: KovaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return KovaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> KovaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return KovaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
