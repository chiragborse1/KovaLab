...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__KOVA_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<KovaTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.kova", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = KovaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> KovaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return KovaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> KovaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> KovaTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> KovaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = KovaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: KovaTalkPTTStopPayload) {
