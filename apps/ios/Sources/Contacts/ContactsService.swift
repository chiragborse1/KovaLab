...
final class ContactsService: ContactsServicing {
    ...
    func search(params: KovaContactsSearchParams) async throws -> KovaContactsSearchPayload {
        ...
        return KovaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: KovaContactsAddParams) async throws -> KovaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return KovaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return KovaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> KovaContactPayload {
        KovaContactPayload(
