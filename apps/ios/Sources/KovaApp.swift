...
@MainActor
enum KovaAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class KovaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? KovaAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> KovaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return KovaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Kova" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [KovaWatchAction]) -> [UNNotificationAction] {
...
struct KovaApp: App {
    ...
    @UIApplicationDelegateAdaptor(KovaAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        KovaAppModelRegistry.appModel = appModel
...
extension KovaApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Kova: installing uncaught exception handler")
