...
struct IOSGatewayChatTransport: KovaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KovaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(KovaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> KovaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(KovaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [KovaChatAttachmentPayload]) async throws -> KovaChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [KovaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(KovaChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(KovaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KovaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KovaChatEventPayload.self)
                        {
