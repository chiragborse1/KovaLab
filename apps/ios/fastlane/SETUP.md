...
ASC_KEYCHAIN_SERVICE=kova-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.kova.client
# or
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `KOVA_APNS_TEAM_ID`, `KOVA_APNS_KEY_ID`, and either `KOVA_APNS_PRIVATE_KEY_P8` or `KOVA_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/beta/Kova-<version>.ipa` exists
...
- Supported pinned iOS versions use SemVer like `0.2.0` or legacy CalVer like `2026.4.10`
...
- Fastlane sets `CFBundleShortVersionString` to the pinned iOS version, for example `0.2.0`
