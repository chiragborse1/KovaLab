...
- Beta release uses canonical `ai.kova.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
...
- `KOVA_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export KOVA_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Kova-<version>.ipa`
   - `apps/ios/build/beta/Kova-<version>.app.dSYM.zip`
...
- The gateway host also needs direct APNs auth configured separately with `KOVA_APNS_TEAM_ID`, `KOVA_APNS_KEY_ID`, and either `KOVA_APNS_PRIVATE_KEY_P8` or `KOVA_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.kova/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `KOVA_APNS_PRIVATE_KEY_PATH` at that file.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `kova.json`. `KOVA_APNS_RELAY_BASE_URL` remains a temporary env override only.
