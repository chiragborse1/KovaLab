...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.KOVA_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.KOVA_BUILD_PRIVATE_QA === "1";
...
function shouldStageBundledPluginRuntimeDependencies(packageJson: unknown): boolean {
  return (
    ...
    packageJson !== null &&
    (packageJson as { kova?: { bundle?: { stageRuntimeDependencies?: boolean } } }).kova?.bundle
...
function isPluginSdkSelfReference(id: string): boolean {
  return (
    id === "getkova/plugin-sdk" ||
    id.startsWith("getkova/plugin-sdk/") ||
    id === "@getkova/plugin-sdk" ||
    id.startsWith("@getkova/plugin-sdk/")
  );
}
...
function buildCoreDistEntries(): Record<string, string> {
  return {
    ...
    "tui/local-backend-child": "src/tui/local-backend-child.ts",
