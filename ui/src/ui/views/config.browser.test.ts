...
describe("config view", () => {
  ...
  it("renders MCP as preference controls before server object editing", () => {
    const onFormPatch = vi.fn();
    const onSectionChange = vi.fn();
    const { container } = renderConfigView({
      navRootLabel: "MCP",
      ...
      schema: {
        ...
        properties: {
          commands: {
            type: "object",
            properties: {
              mcp: { type: "boolean", title: "Enable MCP Command" },
            },
          },
          mcp: {
            type: "object",
            properties: {
              sessionIdleTtlMs: { type: "number", title: "MCP Session Idle TTL" },
              servers: { type: "object", properties: {} },
            },
          },
        },
      },
    ...
    expect(container.querySelector(".config-preference-grid")).not.toBeNull();
    expect(container.querySelector(".config-form--modern")).toBeNull();
    expect(normalizedText(container)).toContain("Enable MCP command");
    expect(normalizedText(container)).toContain("Connected servers");
    expect(normalizedText(container)).toContain("github");
    expect(normalizedText(container)).toContain("remote");

    const toggles = container.querySelectorAll<HTMLInputElement>(
      ".config-preference-card--toggle input[type='checkbox']",
    );
    ...
    const editServers = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.trim() === "Edit servers",
    );
    expect(editServers).toBeTruthy();
    editServers?.click();
    expect(onSectionChange).toHaveBeenCalledWith("mcp");
  });

  it("renders Profiles as preference controls for tool and browser defaults", () => {
    const onFormPatch = vi.fn();
    const { container } = renderConfigView({
      ...
      schema: {
        ...
        properties: {
          agents: {
            type: "object",
            properties: {
              defaults: { type: "object", properties: { model: { type: "string" } } },
            },
          },
    ...
    expect(container.querySelector(".config-preference-grid")).not.toBeNull();
    expect(container.querySelector(".config-form--modern")).toBeNull();
    expect(normalizedText(container)).toContain("Default tool profile");
    expect(normalizedText(container)).toContain("Filesystem workspace guard");
    expect(normalizedText(container)).toContain("Browser automation");
    expect(normalizedText(container)).toContain("Default model: gpt-5.5");

    const fullButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.trim() === "Full",
    );
    expect(fullButton).toBeTruthy();
    fullButton?.click();
    expect(onFormPatch).toHaveBeenCalledWith(["tools", "profile"], "full");
  });
