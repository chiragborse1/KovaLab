...
const CACHE_PREFIX = "kova-control-";
const EMBEDDED_CACHE_VERSION = "__KOVA_CONTROL_UI_BUILD_ID__";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__KOVA_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Kova", body: event.data.text() };
  }

  const title = data.title || "Kova";
  const options = {
    body: data.body || "",
