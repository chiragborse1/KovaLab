...
function resolveControlUiBuildId(): string {
  const explicit = process.env.KOVA_CONTROL_UI_BUILD_ID?.trim() || process.env.KOVA_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__KOVA_CONTROL_UI_BUILD_ID__"';
...
export default defineConfig(() => {
  ...
  return {
    base,
    define: {
      __KOVA_CONTROL_UI_BUILD_ID__: JSON.stringify(controlUiBuildId),
    },
