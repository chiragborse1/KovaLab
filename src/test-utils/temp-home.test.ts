...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("kova-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.KOVA_STATE_DIR).toBe(path.join(tempHome.home, ".kova"));
    ...
    expect(process.env.KOVA_STATE_DIR).toBe(previousStateDir);
