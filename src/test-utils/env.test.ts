...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "KOVA_ENV_TEST_A";
    const keyB = "KOVA_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "KOVA_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "KOVA_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "KOVA_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "KOVA_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "kova-home");
    ...
    process.env.KOVA_HOME = "/srv/kova-home";
    process.env.KOVA_STATE_DIR = "/srv/kova-state";
    process.env.KOVA_BUNDLED_PLUGINS_DIR = "/srv/kova-bundled";
    ...
    try {
      ...
      expect(env.KOVA_HOME).toBeUndefined();
      expect(env.KOVA_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.KOVA_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("KOVA_HOME", previousKovaHome);
      restoreEnvKey("KOVA_STATE_DIR", previousStateDir);
      restoreEnvKey("KOVA_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { KOVA_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processKovaHome: process.env.KOVA_HOME,
          processBundledDir: process.env.KOVA_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.KOVA_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processKovaHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.KOVA_HOME).toBe("/srv/kova-home");
    } finally {
      restoreEnvKey("KOVA_HOME", previousKovaHome);
    }
  });
