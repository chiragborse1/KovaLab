...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      KOVA_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when KOVA_THEME=light", async () => {
    const mod = await importThemeWithEnv({ KOVA_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when KOVA_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ KOVA_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats KOVA_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ KOVA_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("KOVA_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      KOVA_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ KOVA_THEME: "light" });
    const darkMod = await importThemeWithEnv({ KOVA_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    ...
    const textPairs = [
      ...
      [mod.lightPalette.assistantBorder, backgrounds.page],
      [mod.lightPalette.assistantTitle, backgrounds.page],
      [mod.lightPalette.userBorder, backgrounds.page],
      [mod.lightPalette.userTitle, backgrounds.page],
