...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["KOVA_STATE_DIR", "KOVA_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveKovaAgentDir() returns mainAgentDir
    process.env.KOVA_STATE_DIR = tmpDir;
    process.env.KOVA_AGENT_DIR = mainAgentDir;
