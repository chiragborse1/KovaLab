...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveKovaAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveKovaAgentDir()`
    // nullish branch. With KOVA_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-kova-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-path-ensure-"));
    process.env.KOVA_STATE_DIR = stateDir;
  });
