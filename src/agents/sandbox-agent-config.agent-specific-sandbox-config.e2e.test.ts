...
async function resolveContext(config: KovaConfig, sessionKey: string, workspaceDir: string) {
...
function createDefaultsSandboxConfig(scope: "agent" | "shared" | "session" = "agent"): KovaConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): KovaConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
