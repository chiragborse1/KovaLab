...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousKovaAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousKovaAgentDir = process.env.KOVA_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousKovaAgentDir === undefined) {
      delete process.env.KOVA_AGENT_DIR;
    } else {
      process.env.KOVA_AGENT_DIR = previousKovaAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "KOVA_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.KOVA_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.KOVA_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.KOVA_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: KovaConfig = {
  models: {
    providers: {
      "custom-proxy": {
