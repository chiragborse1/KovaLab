...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/kova/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./kova-plugin-tools.js", () => ({
  resolveKovaPluginToolsForOptions: () => [],
}));

vi.mock("./kova-tools.nodes-workspace-guard.js", () => ({
...
describe("createKovaTools TTS config wiring", () => {
  beforeEach(() => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createKovaTools } = await import("./kova-tools.js");
...
describe("createKovaTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });

  it("passes preserved channel delivery context into the cron tool", async () => {
    const { createKovaTools } = await import("./kova-tools.js");
