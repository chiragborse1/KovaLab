...
// One watcher per unique watched directory. Multiple workspaces that share the
// same skill root subscribe to the same watcher, so file descriptors scale with
// distinct directories instead of agent/workspace count.
...
function resolveWatchTargets(workspaceDir: string, config?: KovaConfig): WatchTarget[] {
...
function resolveWatchDebounceMs(config?: KovaConfig): number {
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: KovaConfig }) {
