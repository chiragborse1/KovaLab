...
describe("provider attribution", () => {
  it("resolves the canonical Kova product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Kova runtime wrapper.",
      product: "Kova",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { KOVA_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { KOVA_VERSION: "2026.3.22" })).toEqual({
      originator: "kova",
      version: "2026.3.22",
      "User-Agent": "kova/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai-codex", { KOVA_VERSION: "2026.3.22" })).toEqual(
      {
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ KOVA_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
