...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    kova: process.env.KOVA_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.kova === undefined) {
      delete process.env.KOVA_GATEWAY_TOKEN;
    } else {
      process.env.KOVA_GATEWAY_TOKEN = envSnapshot.kova;
    }
  });
  ...
  it("uses KOVA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.KOVA_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
