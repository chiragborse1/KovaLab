...
function withPdfModel(primary: string): KovaConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureKovaModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
