...
function createClaudeCliOverrideConfig(config: CliBackendConfig): KovaConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Kova's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__kova__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__kova__*");
  ...
  it("keeps Claude permission mode unset when Kova exec policy is not YOLO", () => {
    const resolved = resolveCliBackendConfig("claude-cli", {
