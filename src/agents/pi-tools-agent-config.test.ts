...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: KovaConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<KovaConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<KovaConfig["agents"]>["list"]>[number]["tools"];
  }): KovaConfig {
    return {
      tools: params.tools,
      agents: {
