...
const INTERRUPTED_NETWORK_ERROR_RE =
  /\beconnrefused\b|\beconnreset\b|\beconnaborted\b|\benetreset\b|\behostunreach\b|\behostdown\b|\benetunreach\b|\bepipe\b|\bsocket hang up\b|\bconnection refused\b|\bconnection reset\b|\bconnection aborted\b|\bconnection lost\b|\bnetwork is unreachable\b|\bhost is unreachable\b|\bfetch failed\b|\bconnection error\b|\bnetwork request failed\b|\bnetwork connection lost\b/i;
...
function isOAuthRefreshContentionMessage(raw: string): boolean {
  return (
    ...
    (/\bfile lock timeout\b/i.test(raw) &&
      /(?:\/|\\|^)(?:oauth-refresh|kova-oauth-refresh)[^/\n\\]*?(?:\.lock)?\b/i.test(raw))
  );
}
...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: KovaConfig; sessionKey?: string; provider?: string; model?: string },
): string | undefined {
