...
const GOOGLE_LIVE_SINGLE_KEY = "KOVA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "KOVA_LIVE_ANTHROPIC_KEY",
    listVar: "KOVA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "KOVA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `KOVA_LIVE_${base}_KEY`;
