...
function makeFallbacksOnlyCfg(): KovaConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): KovaConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "kova-auth-suite-mock");
...
// https://github.com/chiragborse1/KovaLab/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("fails fast on session write-lock timeouts instead of trying model fallbacks", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/kova/session.jsonl.lock",
    });
  ...
  it("continues fallback after embedded provider business-denial payloads", async () => {
    ...
    const result = await runWithModelFallback<EmbeddedPiRunResult>({
      cfg,
      provider: "zai",
      model: "glm-5.1",
      run,
    ...
    expect(run).toHaveBeenCalledTimes(2);
    expect(run.mock.calls[1]).toEqual(["openai", "gpt-5.5"]);
    expect(result.attempts[0]).toMatchObject({
      provider: "zai",
      model: "glm-5.1",
      reason: "auth",
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("kova-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
