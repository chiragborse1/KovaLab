...
export function listAgentEntries(cfg: KovaConfig): AgentEntry[] {
...
export function listAgentIds(cfg: KovaConfig): string[] {
...
export function resolveDefaultAgentId(cfg: KovaConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
