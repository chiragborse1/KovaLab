...
let createKovaCodingTools: typeof import("./pi-tools.js").createKovaCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      KOVA_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "kova-test-no-bundled-extensions"),
    },
    async () => {
      ({ createKovaCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
describe("createKovaCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "kova-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "kova-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "kova-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "kova-safe-bins-redirect-",
        safeBins: ["head"],
