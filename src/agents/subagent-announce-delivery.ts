...
function createQueueOutcomeFromBoolean(params: {
  queued: boolean;
  sessionId: string;
}): EmbeddedPiQueueMessageOutcome {
  if (params.queued) {
...
async function resolveQueueEmbeddedPiMessageOutcome(
  ...
  return await Promise.resolve(
    subagentAnnounceDeliveryDeps.queueEmbeddedPiMessageWithOutcome(sessionId, message, options),
  );
}
...
function resolveDirectAnnounceTransientRetryDelaysMs() {
  return process.env.KOVA_TEST_FAST === "1"
    ? ([8, 16, 32] as const)
    : ([5_000, 10_000, 20_000] as const);
}

export function resolveSubagentAnnounceTimeoutMs(cfg: KovaConfig): number {
...
function getGatewayAgentResult(response: unknown): Record<string, unknown> | undefined {
  const result =
    response && typeof response === "object" && "result" in response
      ? (response as { result?: unknown }).result
      : undefined;
...
function hasGatewayAgentMessagingToolDelivery(response: unknown): boolean {
  ...
  for (const key of ["messagingToolSentTargets", "messagingToolSentTexts", "sentTargets"]) {
...
function inferDeliveryTargetChatType(target: { channel?: string; to?: string }) {
...
function hasSubagentCompletionEvent(events: readonly AgentInternalEvent[] | undefined): boolean {
  return (
    events?.some((event) => event.type === "task_completion" && event.source === "subagent") ===
    true
  );
}
...
function hasFailedSubagentNoOutputCompletion(
  events: readonly AgentInternalEvent[] | undefined,
): boolean {
  return (
    events?.some(
...
async function deliverTextCompletionDirect(params: {
  ...
  try {
    await sendCompletionFallback({
      ...
      idempotencyKey: `${params.directIdempotencyKey}:text-direct`,
...
function isGatewayAgentRunPending(response: unknown): boolean {
  ...
  return (
    status === "queued" || status === "pending" || status === "running" || status === "in_progress"
  );
}
...
async function sendSubagentAnnounceDirectly(params: {
  ...
  try {
    ...
    const isSubagentCompletion =
      sourceToolId === "subagent_announce" &&
      ((params.internalEvents?.length ?? 0) === 0 ||
        hasSubagentCompletionEvent(params.internalEvents));
    ...
    const shouldDeliverAgentFinal =
      deliveryTarget.deliver && !subagentDirectMessageCompletionRequiresMessageTool;
    const completionSourceReplyDeliveryMode = subagentDirectMessageCompletionRequiresMessageTool
      ? "message_tool_only"
      : undefined;
    ...
    if (
      ...
      const requesterQueueSettings = resolveQueueSettings({
        cfg,
        channel: sessionOnlyOrigin?.channel ?? requesterSessionOrigin?.channel,
      });
      ...
      if (requesterActivity.isActive) {
        if (completionWakeRetriedWithoutTranscriptWait) {
          defaultRuntime.log(
            `[warn] Active requester session could not be woken for subagent completion; falling back to requester-agent handoff: ${wakeOutcome.reason}`,
          );
        } else {
          try {
...
export const __testing = {
  setDepsForTest(overrides?: Partial<SubagentAnnounceDeliveryDeps>) {
    const effectiveOverrides =
      overrides?.queueEmbeddedPiMessage && !overrides.queueEmbeddedPiMessageWithOutcome
        ? {
            ...overrides,
            queueEmbeddedPiMessageWithOutcome: (
              sessionId: string,
              ...
              void options;
              return createQueueOutcomeFromBoolean({
                queued: overrides.queueEmbeddedPiMessage?.(sessionId, message) ?? false,
                sessionId,
              });
            },
          }
