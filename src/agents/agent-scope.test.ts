...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: KovaConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: KovaConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: KovaConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: KovaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: KovaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: KovaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/kova");
  });

  it("uses KOVA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "kova-home");
    vi.stubEnv("KOVA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as KovaConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".kova", "workspace"));
  });

  it("uses KOVA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "kova-home");
    vi.stubEnv("KOVA_HOME", home);
    // Clear state dir so it falls back to KOVA_HOME
    vi.stubEnv("KOVA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as KovaConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".kova", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/kova-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/kova-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kova-agent-scope-"));
