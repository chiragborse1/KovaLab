...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("kova-bundled-");
  const workspaceDir = await createTempDir("kova-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: KovaConfig) {
