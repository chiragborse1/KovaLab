...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  it("sends transcriptPrompt visibly and queues runtime context as hidden custom context", async () => {
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("KOVA_INTERNAL_CONTEXT");
  ...
  it("keeps current turn context visible on runtime-only turns", async () => {
    let seenPrompt: string | undefined;
      ...
      attemptOverrides: {
        ...
        currentTurnContext: {
          replyChain: [
            {
              sender: "Alice",
              body: "Hello from the replied message",
              isQuote: true,
            },
          ],
        },
    ...
    expect(seenPrompt).toContain("Reply chain of current user message");
