...
// `provider:"kova"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_KOVA_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyKovaAssistant(message: AgentMessage): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = (message as { provider?: unknown }).provider;
  const model = (message as { model?: unknown }).model;
  return provider === "kova" && typeof model === "string" && TRANSCRIPT_ONLY_KOVA_MODELS.has(model);
}
...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyKovaAssistant(message)) {
