...
async function runEnvProviderCase(params: {
  ...
  try {
    await ensureKovaModelsJson({});

    const modelPath = path.join(resolveKovaAgentDir(), "models.json");
...
describe("models-config", () => {
  beforeAll(async () => {
    ...
    ({ ensureKovaModelsJson, resetModelsJsonReadyCacheForTest } =
  ...
  it("writes marker-backed defaults but skips env-gated providers when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      await withTempEnv([...MODELS_CONFIG_IMPLICIT_ENV_VARS, "KIMI_API_KEY"], async () => {
        ...
        const result = await ensureKovaModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      await ensureKovaModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
