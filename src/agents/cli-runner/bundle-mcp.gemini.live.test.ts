...
const LIVE = isLiveTestEnabled(["KOVA_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "kova-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("kova_live_probe", "Kova Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Kova-configured streamable-http server", async () => {
    const geminiCommand = process.env.KOVA_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("kovaLiveProbe");
