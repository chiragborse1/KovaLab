...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    envSnapshot = captureEnv(["KOVA_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("kova-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("kova-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("kova-cli-bundle-mcp-bundled-");
    process.env.KOVA_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
export function createEnabledBundleProbeConfig(): KovaConfig {
  return {
    plugins: {
      entries: {
