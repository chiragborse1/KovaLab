...
const CLI_BACKEND_PRESERVE_ENV = "KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.KOVA_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.KOVA_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.KOVA_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.KOVA_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.KOVA_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.KOVA_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.kovaHistoryPrompt ?? params.prompt);
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.KOVA_MCP_TOKEN || env.KOVA_MCP_SESSION_KEY || env.KOVA_MCP_SENDER_IS_OWNER) {
