...
const ensureKovaModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureKovaModelsJson: (...args: Parameters<typeof ensureKovaModelsJsonMock>) =>
        ensureKovaModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("kova-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureKovaModelsJsonMock.mockReset();
  ensureKovaModelsJsonMock.mockResolvedValue({ wrote: false });
