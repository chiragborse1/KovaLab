...
function resolveAttachmentLimits(config: KovaConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".kova", "attachments");
  const relDir = path.posix.join(".kova", "attachments", attachmentId);
