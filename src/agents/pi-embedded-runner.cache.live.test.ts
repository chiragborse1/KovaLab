...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.KOVA_CACHE_TRACE,
      file: process.env.KOVA_CACHE_TRACE_FILE,
      messages: process.env.KOVA_CACHE_TRACE_MESSAGES,
      prompt: process.env.KOVA_CACHE_TRACE_PROMPT,
      system: process.env.KOVA_CACHE_TRACE_SYSTEM,
    };
    process.env.KOVA_CACHE_TRACE = "1";
    process.env.KOVA_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.KOVA_CACHE_TRACE_MESSAGES = "0";
    process.env.KOVA_CACHE_TRACE_PROMPT = "0";
    process.env.KOVA_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("KOVA_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("KOVA_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("KOVA_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("KOVA_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("KOVA_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "KOVA_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "KOVA_LIVE_ANTHROPIC_CACHE_MODEL",
