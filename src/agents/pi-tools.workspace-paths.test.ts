...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("kova-ws-", async (workspaceDir) => {
      await withTempDir("kova-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createKovaCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "kova" }],
          });
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello kova");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("kova-ws-", async (workspaceDir) => {
      await withTempDir("kova-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("kova-ws-", async (workspaceDir) => {
      const cfg: KovaConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createKovaCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-kova.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("kova-sandbox-", async (sandboxDir) => {
      await withTempDir("kova-workspace-", async (workspaceDir) => {
        ...
        const tools = createKovaCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
