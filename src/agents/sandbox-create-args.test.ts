...
import { KOVA_CLI_ENV_VALUE } from "../infra/kova-exec-env.js";
...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "kova.sandbox=1",
        "--label",
        "kova.sessionKey=main",
        "--label",
        "kova.createdAtMs=1700000000000",
        "--label",
    ...
    expect(args).toEqual(
      expect.arrayContaining(["--env", "LANG=C.UTF-8", "--env", `KOVA_CLI=${KOVA_CLI_ENV_VALUE}`]),
    );
  ...
  it("preserves the Kova exec marker when strict env sanitization is enabled", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining(["--env", "NODE_ENV=test", "--env", `KOVA_CLI=${KOVA_CLI_ENV_VALUE}`]),
    );
  });
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "kova-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("kova-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "kova-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "kova-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
