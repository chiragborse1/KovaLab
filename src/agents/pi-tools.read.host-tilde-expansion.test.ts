...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("kova-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("kova-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("kova-tilde-test-mkdir-");
  ...
  it("ignores KOVA_HOME for write operations", async () => {
    const kovaHome = await createTempDir("kova-home-override-", os.tmpdir());
    const dir = await createTempDir("kova-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("KOVA_HOME", kovaHome);

    createHostWorkspaceWriteTool(kovaHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(kovaHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores KOVA_HOME for mkdir operations", async () => {
    const kovaHome = await createTempDir("kova-home-override-", os.tmpdir());
    const dir = await createTempDir("kova-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(kovaHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores KOVA_HOME for readFile operations", async () => {
    const kovaHome = await createTempDir("kova-home-override-", os.tmpdir());
    const dir = await createTempDir("kova-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(kovaHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores KOVA_HOME for access operations", async () => {
    const kovaHome = await createTempDir("kova-home-override-", os.tmpdir());
    const dir = await createTempDir("kova-tilde-test-edit-");
