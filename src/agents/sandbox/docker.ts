...
import { markKovaExecEnv } from "../../infra/kova-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", `kova.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `kova.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markKovaExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "kova.configHash");
}
