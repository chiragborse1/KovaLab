  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is a Kova config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Kova config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: KovaConfig, agentId: string): boolean {
