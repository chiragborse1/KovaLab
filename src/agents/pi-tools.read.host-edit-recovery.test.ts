...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when KOVA_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const kovaHome = path.join(tmpDir, "kova-home");
    ...
    process.env.KOVA_HOME = kovaHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(kovaHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
