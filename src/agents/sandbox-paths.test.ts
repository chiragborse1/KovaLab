...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-managed-media-"));
...
async function withOutsideHardlinkInKovaTmp<T>(
  params: {
    kovaTmpDir: string;
  ...
  const hardlinkPath = path.join(params.kovaTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.kovaTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.kovaTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.kovaTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const kovaTmpDir = resolvePreferredKovaTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Kova tmp root",
      media: path.join(kovaTmpDir, "image.png"),
      expected: path.join(kovaTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Kova tmp root",
      media: pathToFileURL(path.join(kovaTmpDir, "photo.png")).href,
      expected: path.join(kovaTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Kova tmp root",
      media: path.join(kovaTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(kovaTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(kovaTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside kova tmp root",
      media: path.join(os.tmpdir(), "outside-kova", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Kova tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(kovaTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(kovaTmpDir, { recursive: true });
      const symlinkPath = path.join(kovaTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Kova tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKovaTmp(
      {
        kovaTmpDir,
  ...
  it("rejects symlinked Kova tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKovaTmp(
