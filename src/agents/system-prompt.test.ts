...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Kova CLI Quick Reference");
    ...
    expect(prompt).toContain("kova gateway restart");
    expect(prompt).toContain("Do not chain `kova gateway stop`");
  ...
  it("does not include embed guidance for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.kova-dev/canvas",
      },
    });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Kova docs: /tmp/kova/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Kova docs: /tmp/kova/docs");
    expect(prompt).toContain("Local source: /tmp/kova");
    expect(prompt).toContain(
      "For Kova behavior, commands, config, or architecture: consult local docs first.",
    );
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Kova Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/kova. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`kova ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Kova subagents");
