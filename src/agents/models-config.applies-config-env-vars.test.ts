...
const TEST_ENV_VAR = "KOVA_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/kova-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): KovaConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: KovaConfig) {
