...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("kova.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by Kova",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveKovaCliExecutable();
  const argv =
    executable === "kova" ? ["kova"] : [params.nodeExecutable ?? process.execPath, executable];
...
function nativePreToolUseMayRunLoopDetection(registration: NativeHookRelayRegistration): boolean {
  ...
  const loopDetection = registration.config?.tools?.loopDetection;
...
function nativeHookRelayEventHasLocalWork(
  ...
  if (event === "pre_tool_use") {
    return hasGlobalHooks("before_tool_call") || nativePreToolUseMayRunLoopDetection(registration);
  }
  ...
  try {
    ...
    for (const name of readdirSync(staleDir)) {
      ...
      try {
        ...
        // Ignore unparseable or racing bridge records.
      }
    }
  } catch (error) {
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `kova-native-hook-relays-${uid}`);
}
...
async function runNativeHookRelayPreToolUse(params: {
  ...
  // Kova plugin returns adjusted params here, we intentionally ignore them.
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  return [
    ...
    params.registration.agentId ?? "",
    params.registration.sessionKey ?? params.registration.sessionId,
    params.request.cwd ?? "",
...
function hasNativeHookRelayPermissionAllowAlways(key: string): boolean {
  const record = permissionAllowAlwaysApprovals.get(key);
  if (!record) {
    return false;
  }
  if (Date.now() > record.expiresAtMs) {
...
function rememberNativeHookRelayPermissionAllowAlways(key: string): void {
  pruneNativeHookRelayPermissionAllowAlways();
  permissionAllowAlwaysApprovals.set(key, {
    expiresAtMs: Date.now() + PERMISSION_ALLOW_ALWAYS_TTL_MS,
  });
  if (permissionAllowAlwaysApprovals.size > MAX_PERMISSION_ALLOW_ALWAYS_ENTRIES) {
    const oldest = permissionAllowAlwaysApprovals.keys().next().value;
    if (oldest) {
      permissionAllowAlwaysApprovals.delete(oldest);
    }
  }
}

function pruneNativeHookRelayPermissionAllowAlways(now = Date.now()): void {
  for (const [key, record] of permissionAllowAlwaysApprovals) {
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `kova-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveKovaCliExecutable(): string {
  ...
  throw new Error("Cannot resolve Kova CLI executable path for native hook relay");
}
...
export const __testing = {
  ...
  getNativeHookRelayBridgeRecordForTests(relayId: string): NativeHookRelayBridgeRecord | undefined {
    const record = readNativeHookRelayBridgeRecordIfExists(relayId);
  ...
  invokeNativeHookRelayBridgeForTests: invokeNativeHookRelayBridge,
