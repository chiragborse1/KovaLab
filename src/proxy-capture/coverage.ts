...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[kova proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[kova proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`kova proxy coverage\` for details.`,
  );
}
