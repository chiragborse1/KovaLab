...
// resolveKovaPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/kova-root.js", () => ({
  resolveKovaPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveKovaPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "kova" });
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // KOVA_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an KOVA_BUNDLED_PLUGINS_DIR override pointing at
