...
export type ChannelSetupWizardEnvShortcut = {
  ...
  isAvailable: (params: { cfg: KovaConfig; accountId: string }) => boolean;
  apply: (params: { cfg: KovaConfig; accountId: string }) => KovaConfig | Promise<KovaConfig>;
};
...
export type ChannelSetupWizardCredential = {
  ...
  keepPrompt: string;
  inputPrompt: string;
  allowEnv?: (params: { cfg: KovaConfig; accountId: string }) => boolean;
  inspect: (params: { cfg: KovaConfig; accountId: string }) => ChannelSetupWizardCredentialState;
  shouldPrompt?: (params: {
    cfg: KovaConfig;
    accountId: string;
    credentialValues: ChannelSetupWizardCredentialValues;
...
export type ChannelSetupWizardGroupAccess = {
  ...
  currentPolicy: (params: { cfg: KovaConfig; accountId: string }) => ChannelAccessPolicy;
  currentEntries: (params: { cfg: KovaConfig; accountId: string }) => string[];
  updatePrompt: (params: { cfg: KovaConfig; accountId: string }) => boolean;
  setPolicy: (params: {
    cfg: KovaConfig;
    accountId: string;
...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: KovaConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
