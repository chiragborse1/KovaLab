...
import type { KovaPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["KOVA_PLUGIN_CATALOG_PATHS", "KOVA_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveKovaPackageRootSync({ cwd: process.cwd() }),
    resolveKovaPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<KovaPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
