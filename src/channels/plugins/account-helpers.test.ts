...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): KovaConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as KovaConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: KovaConfig, expected: string) {
