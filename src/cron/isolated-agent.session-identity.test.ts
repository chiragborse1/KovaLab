...
describe("runCronIsolatedAgentTurn session identity", () => {
  ...
  it("passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.agentDir).toBe(path.join(home, ".kova", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".kova", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
  ...
  it("passes sessionFile to isolated cron runs", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.sessionFile).toContain(path.join(home, ".kova", "agents", "main", "sessions"));
