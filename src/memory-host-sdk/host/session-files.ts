...
function isDreamingNarrativeBootstrapRecord(record: unknown): boolean {
  ...
  if (
    ...
    candidate.customType !== "kova:bootstrap-context:full" ||
    !candidate.data ||
...
/**
 * Strip Kova-injected inbound metadata envelopes from a raw text block.
 *
 ...
 * impossible.
 *
 * See: https://github.com/chiragborse1/KovaLab/issues/63921
 */
...
function sanitizeSessionText(text: string, role: "user" | "assistant"): string | null {
  ...
  // Assistant-side machinery acks: quiet Pulse tokens mean "all clear,
  // nothing to do" for a Pulse tick. Drop on the assistant side
