...
describe("resolveReplyToMode", () => {
  ...
  it("uses registered channel threading adapters for runtime reply-mode resolution", () => {
    ...
    expect(resolveReplyToMode({} as KovaConfig, "whatsapp", "work", "group")).toBe("first");
    expect(resolveReplyToMode({} as KovaConfig, "whatsapp", "default", "group")).toBe("all");
  });
});
