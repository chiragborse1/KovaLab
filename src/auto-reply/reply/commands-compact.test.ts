...
function buildCompactParams(commandBodyNormalized: string, cfg: KovaConfig): HandleCommandsParams {
  return {
    cfg,
    ctx: {
      Provider: "whatsapp",
...
describe("handleCompactCommand", () => {
  beforeEach(() => {
    ...
    resolveAgentDirMock.mockImplementation(
      (_cfg: unknown, agentId: string) => `/tmp/workspace/.kova/agents/${agentId}/agent`,
    );
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/kova-session-store.json" },
        } as KovaConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/kova-agent-compact",
        sessionEntry: {
          sessionId: "session-1",
          updatedAt: Date.now(),
          groupId: "group-1",
