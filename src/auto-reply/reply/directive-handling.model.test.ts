...
function baseConfig(): KovaConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
function resolveModelSelectionForCommand(params: {
  ...
  return resolveModelSelectionFromDirective({
    ...
    cfg: { commands: { text: true } } as unknown as KovaConfig,
    agentDir: TEST_AGENT_DIR,
...
describe("/model chat UX", () => {
  ...
  it("auto-applies closest match for typos", () => {
    ...
    const cfg = { commands: { text: true } } as unknown as KovaConfig;
...
describe("handleDirectiveOnly model persist behavior (fixes #1435)", () => {
  ...
  it("resets thinking level to the model default", async () => {
    const directives = parseInlineDirectives("/think default");
    const sessionEntry = createSessionEntry({ thinkingLevel: "low" });
    const sessionStore = { [sessionKey]: sessionEntry };
    const result = await handleDirectiveOnly(
      createHandleParams({
