...
const { createKovaCodingToolsMock } = vi.hoisted(() => ({
  createKovaCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createKovaCodingTools: createKovaCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createKovaCodingToolsMock.mockClear();
    createKovaCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(vi.mocked(createKovaCodingTools)).toHaveBeenCalledWith(
