...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("kova.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("kova.reply.full-runtime");

type ReplyConfigWithMarker = KovaConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.KOVA_ALLOW_SLOW_REPLY_TESTS === "1" || env.KOVA_STRICT_FAST_REPLY_CONFIG === "0";
}
...
export function markCompleteReplyConfig<T extends KovaConfig>(
  config: T,
...
export function withFastReplyConfig<T extends KovaConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends KovaConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

export function isCompleteReplyConfig(config: unknown): config is KovaConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set KOVA_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as KovaConfig;
}
