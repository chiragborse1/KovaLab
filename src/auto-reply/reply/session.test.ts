...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-session-suite-"));
});
...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const root = await makeCaseDir("kova-thread-session-");
  ...
  it("forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const root = await makeCaseDir("kova-thread-session-existing-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork and creates fresh session when parent tokens exceed threshold", async () => {
    const root = await makeCaseDir("kova-thread-session-overflow-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("skips fork when resolved parent token estimate exceeds threshold", async () => {
    const root = await makeCaseDir("kova-thread-session-overflow-estimated-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("respects session.parentForkMaxTokens override", async () => {
    const root = await makeCaseDir("kova-thread-session-overflow-override-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await makeCaseDir("kova-topic-session-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("records topic-specific session files from SessionKey when MessageThreadId is absent", async () => {
    const root = await makeCaseDir("kova-topic-session-key-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("uses RawBody for command extraction and reset triggers when Body contains wrapped context", async () => {
    const root = await makeCaseDir("kova-rawbody-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as KovaConfig;
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await makeCaseDir("kova-rawbody-reset-case-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("drains stale system events when /new rotates an existing session", async () => {
    const root = await makeCaseDir("kova-rawbody-reset-system-events-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("rotates local session state for /new on bound ACP sessions", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-reset-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("rotates local session state for ACP /new when no matching conversation binding exists", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-reset-no-conversation-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps custom reset triggers working on bound ACP sessions", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-custom-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("keeps normal /new behavior for unbound ACP-shaped session keys", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-unbound-reset-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active conversation binding points to a non-ACP session", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-nonacp-binding-");
    const storePath = path.join(root, "sessions.json");
    const sessionKey = "agent:codex:acp:binding:discord:default:feedface";
    const existingSessionId = "session-existing";
    const now = Date.now();
  ...
  it("does not suppress /new when active target session key is non-ACP even with configured ACP binding", async () => {
    const root = await makeCaseDir("kova-rawbody-acp-configured-fallback-target-");
    const storePath = path.join(root, "sessions.json");
    const channelId = "1478836151241412759";
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await makeCaseDir("kova-session-store-default-");
    const stateDir = path.join(root, ".kova");
    ...
    vi.stubEnv("KOVA_STATE_DIR", stateDir);
    try {
      await fs.mkdir(path.dirname(storePath), { recursive: true });
      await writeSessionStoreFast(storePath, {
        [sessionKey]: {
  ...
  it.each([
    ...
    const storePath = await createStorePath("kova-generic-current-binding-");
...
describe("initSessionState reset policy", () => {
  ...
  it("defaults to daily reset at 4am local time", async () => {
    ...
    const root = await makeCaseDir("kova-reset-daily-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    const root = await makeCaseDir("kova-reset-daily-edge-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    const root = await makeCaseDir("kova-reset-idle-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("drains stale system events when idle rollover creates a new session", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("kova-reset-idle-system-events-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps the existing stale session for /reset soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("kova-reset-soft-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps the existing stale session for /reset: soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("kova-reset-soft-colon-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps the existing stale session for multiline /reset soft tails", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("kova-reset-soft-multiline-stale-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("does not preserve a stale session for unauthorized /reset soft", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const root = await makeCaseDir("kova-reset-soft-stale-unauthorized-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("uses per-type overrides for thread sessions", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("kova-reset-thread-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("detects thread sessions without thread key suffix", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("kova-reset-thread-nosuffix-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("kova-reset-type-default-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("kova-reset-legacy-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState browser tab cleanup", () => {
  it("closes tracked browser tabs when idle session expires", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
    const storePath = await createStorePath("kova-tab-cleanup-idle-");
  ...
  it("closes tracked browser tabs on explicit /new reset", async () => {
    const storePath = await createStorePath("kova-tab-cleanup-reset-");
  ...
  it("does not close browser tabs for a fresh session without previous state", async () => {
    const storePath = await createStorePath("kova-tab-cleanup-fresh-");
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    const root = await makeCaseDir("kova-channel-idle-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState reset triggers in WhatsApp groups", () => {
  ...
  function makeCfg(params: { storePath: string; allowFrom: string[] }): KovaConfig {
    return {
  ...
  it("applies WhatsApp group reset authorization across sender variants", async () => {
    ...
    const storePath = await createStorePath("kova-group-reset");
    const cases = [
      {
  ...
  it("starts a fresh session when a scoped WhatsApp group entry only contains activation state", async () => {
    ...
    const storePath = await createStorePath("kova-group-activation-backfill-");
    await writeSessionStoreFast(storePath, {
      [sessionKey]: {
...
describe("initSessionState reset triggers in Slack channels", () => {
  ...
  it("supports mention-prefixed Slack reset commands and preserves args", async () => {
    ...
    const storePath = await createStorePath("kova-slack-channel-new-");
    await seedSessionStore({
      storePath,
      sessionKey,
      sessionId: existingSessionId,
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("preserves behavior overrides across /new and /reset", async () => {
    const storePath = await createStorePath("kova-reset-overrides-");
  ...
  it("preserves selected auth profile overrides across /new and /reset", async () => {
    const storePath = await createStorePath("kova-reset-model-auth-");
  ...
  it("clears auto-sourced model/provider/auth overrides on /new and /reset (#69301)", async () => {
    const storePath = await createStorePath("kova-reset-auto-overrides-");
  ...
  it("preserves spawned session ownership metadata across /new and /reset", async () => {
    const storePath = await createStorePath("kova-reset-spawned-metadata-");
  ...
  it("requires operator.admin when Provider is internal even if Surface carries external metadata", async () => {
    const storePath = await createStorePath("kova-internal-reset-provider-authoritative-");
  ...
  it("keeps the existing session for /reset soft", async () => {
    const storePath = await createStorePath("kova-soft-reset-session-");
  ...
  it("keeps the existing session for /reset newline soft", async () => {
    const storePath = await createStorePath("kova-reset-newline-soft-");
  ...
  it("archives the old session store entry on /new", async () => {
    const storePath = await createStorePath("kova-archive-old-");
  ...
  it("archives the old session transcript on daily/scheduled reset (stale session)", async () => {
    ...
    try {
      ...
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("kova-stale-archive-");
  ...
  it("keeps provider-owned CLI sessions on implicit daily reset boundaries", async () => {
    ...
    try {
      vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
      const storePath = await createStorePath("kova-cli-implicit-reset-");
  ...
  it("honors explicit reset policies for provider-owned CLI sessions", async () => {
    const storePath = await createStorePath("kova-cli-explicit-reset-");
  ...
  it("disposes the previous bundle MCP runtime on session rollover", async () => {
    const storePath = await createStorePath("kova-stale-runtime-dispose-");
  ...
  it("idle-based new session does NOT preserve overrides (no entry to read)", async () => {
    const storePath = await createStorePath("kova-idle-no-preserve-");
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("uses lastCallUsage for totalTokens when provided", async () => {
    const storePath = await createStorePath("kova-usage-");
  ...
  it("uses lastCallUsage cache counters when available", async () => {
    const storePath = await createStorePath("kova-usage-cache-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("treats CLI usage as a fresh context snapshot when requested", async () => {
    const storePath = await createStorePath("kova-usage-cli-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("snapshots estimatedCostUsd instead of accumulating (fixes #69347)", async () => {
    const storePath = await createStorePath("kova-usage-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
  ...
  it("persists zero estimatedCostUsd for free priced models", async () => {
    const storePath = await createStorePath("kova-usage-free-cost-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
...
describe("initSessionState internal channel routing preservation", () => {
  ...
  it("preserves persisted external route when webchat views a channel-peer session (fixes #47745)", async () => {
    // Regression: local chat access must not overwrite an established
    ...
    // External route must be preserved — local chat is monitoring only
  ...
  it("lets direct webchat turns own routing for sessions with no prior external route", async () => {
    ...
    expect(result.sessionEntry.lastTo).toBe("session:local");
    expect(result.sessionEntry.deliveryContext?.channel).toBe("webchat");
    expect(result.sessionEntry.deliveryContext?.to).toBe("session:local");
  });
