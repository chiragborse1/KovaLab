...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kova-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop kova!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@kovaai_bot", { botUsername: "kovaai_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@kovaai_bot", { botUsername: "kovaai_bot" })).toBe(true);
