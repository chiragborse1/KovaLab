    ...
    if (!directives.rawThinkLevel) {
      ...
      return {
        text: withOptions(
          ...
          `default, ${formatThinkingLevels(resolvedProvider, resolvedModel)}`,
        ),
      };
    }
    return {
      text: `Unrecognized thinking level "${directives.rawThinkLevel}". Valid levels: default, ${formatThinkingLevels(resolvedProvider, resolvedModel)}.`,
    };
  }
