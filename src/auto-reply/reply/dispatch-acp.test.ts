...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: KovaConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: KovaConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: KovaConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: KovaConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: KovaConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): KovaConfig {
...
describe("tryDispatchAcpReply", () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: KovaConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/kova-session-store.json",
              sessionKey: canonicalSessionKey,
  ...
  it("routes default ACP text as one final reply to Discord", async () => {
    setReadyAcpResolution();
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({ text: "Received your test message." });
  ...
  it("routes default ACP text as one final reply to Slack", async () => {
    setReadyAcpResolution();
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({ text: "Shared update." });
  ...
  it("delivers default Slack ACP text directly as a final reply", async () => {
    setReadyAcpResolution();
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({ text: "Slack says hi." });
