...
type HomeEnvSnapshot = {
  ...
  KOVA_STATE_DIR: string | undefined;
  KOVA_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    KOVA_AGENT_DIR: process.env.KOVA_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".kova", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.KOVA_STATE_DIR = path.join(home, ".kova");
    process.env.KOVA_AGENT_DIR = path.join(home, ".kova", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".kova", "agent");
