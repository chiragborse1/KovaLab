...
function listAgents(cfg: KovaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: KovaConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<KovaConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: KovaConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: KovaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: KovaConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<KovaConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  KovaConfig,
  {
    bindingsRef: KovaConfig["bindings"];
    agentsRef: KovaConfig["agents"];
    sessionRef: KovaConfig["session"];
...
function buildEvaluatedBindingsByChannel(cfg: KovaConfig): Map<string, EvaluatedBindingsByChannel> {
...
function resolveRouteCacheForConfig(cfg: KovaConfig): Map<string, ResolvedAgentRoute> {
