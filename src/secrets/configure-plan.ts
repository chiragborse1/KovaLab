...
export function buildConfigureCandidates(config: KovaConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: KovaConfig;
  authoredKovaConfig?: KovaConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredKovaConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const kovaCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...kovaCandidates, ...authCandidates].toSorted((a, b) =>
