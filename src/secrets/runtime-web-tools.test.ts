...
function setConfiguredFetchProviderKey(configTarget: KovaConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: KovaConfig; env?: NodeJS.ProcessEnv }) {
...
function createProviderSecretRefConfig(provider: ProviderUnderTest, envRefId: string): KovaConfig {
  return asConfig({
    tools: {
      web: {
        search: {
...
function readProviderKey(config: KovaConfig, provider: ProviderUnderTest): unknown {
