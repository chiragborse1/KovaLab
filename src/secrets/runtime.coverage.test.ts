...
type SecretRegistryEntry = {
  id: string;
  configFile: "kova.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.KOVA_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_KOVA_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForKovaTarget(
  config: KovaConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every kova.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "kova.json" && !PLUGIN_OWNED_KOVA_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("kova.json", batch);
      const config = {} as KovaConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `KOVA_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForKovaTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `KOVA_AUTH_SECRET_TARGET_${entry.id}`;
