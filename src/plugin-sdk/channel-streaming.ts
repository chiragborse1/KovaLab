...
function buildNamedProgressLine(
  ...
  const aggregateText = formatToolAggregate(
    normalizedName,
  ...
  const compactCommandPrefix =
    (display.name === "exec" || display.name === "bash" || display.name === "shell") &&
    aggregateText.startsWith(`${display.emoji} `)
      ? aggregateText.slice(display.emoji.length + 1).trim()
      : undefined;
  const detail = aggregateText.startsWith(`${prefix}: `)
    ? aggregateText.slice(prefix.length + 2).trim()
    : compactCommandPrefix;
  const compactCommandLine =
    detail && (display.name === "exec" || display.name === "bash" || display.name === "shell");
  const text = compactCommandLine ? `${display.emoji} ${detail}` : aggregateText;
  return {
    kind,
    text,
    label: display.label,
