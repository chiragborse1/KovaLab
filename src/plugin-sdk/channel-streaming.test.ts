...
describe("channel-streaming", () => {
  ...
  it("keeps compacted raw progress lines from leaking unmatched markdown backticks", () => {
    const line = buildChannelProgressDraftLine(
      {
        ...
        args: {
          command:
            "node scripts/check-something-with-a-very-long-path /tmp/kova/some/really/deep/path/that/keeps/going/and/going/index.ts --flag value",
        },
      },
      { detailMode: "raw" },
    );
    ...
    expect(text).toBe(
      "Shelling\n\n🛠️ run node script…va/some/really/deep/path/that/keeps/going/and/going/index.ts …",
    );
  ...
  it("formats progress draft lines with shared tool display labels", () => {
    ...
    ).toBe("🛠️ sed -n '1,80p' extensions/discord/src/draft-stream.ts");
    expect(
      formatChannelProgressDraftLine({
        event: "tool",
        name: "web_search",
