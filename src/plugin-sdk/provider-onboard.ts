...
export type { KovaConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: KovaConfig, ...args: TArgs) => KovaConfig;
  applyConfig: (cfg: KovaConfig, ...args: TArgs) => KovaConfig;
};
...
>(params: {
  ...
  applyPreset: (cfg: KovaConfig, preset: TParams) => KovaConfig;
  primaryModelRef: string;
...
export function applyAgentDefaultModelPrimary(cfg: KovaConfig, primary: string): KovaConfig {
...
export function applyOpencodeZenModelDefault(cfg: KovaConfig): {
  next: KovaConfig;
  changed: boolean;
} {
