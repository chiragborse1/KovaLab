...
function normalizeAgentModelMapForAuthResult(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  let mutated = false;
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (
      ...
      next[normalizedKey] = {
        ...existingRecord,
        ...entryRecord,
        ...(existingRecord.params || entryRecord.params
          ? {
              params: {
                ...(typeof existingRecord.params === "object" &&
                existingRecord.params &&
                !Array.isArray(existingRecord.params)
                  ? existingRecord.params
                  : {}),
                ...(typeof entryRecord.params === "object" &&
                entryRecord.params &&
                !Array.isArray(entryRecord.params)
                  ? entryRecord.params
                  : {}),
              },
            }
...
function normalizeProviderConfigModelIdsForAuthResult(
  ...
  const nextModels = models.map((model) => {
    const id = modelKey(provider, normalizeGooglePreviewModelIdForConfig(provider, model.id));
    if (id === model.id) {
      return model;
    }
    mutated = true;
...
function normalizeProviderAuthConfigPatchModelRefs(
  ...
  if (defaults) {
    ...
    if (defaults.models) {
      const models = normalizeAgentModelMapForAuthResult(defaults.models);
      if (models !== defaults.models) {
        nextDefaults = { ...nextDefaults, models: models as typeof defaults.models };
      }
    }
