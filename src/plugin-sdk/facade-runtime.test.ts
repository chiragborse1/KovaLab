...
describe("plugin-sdk facade runtime", () => {
  it("honors bundled plugin dir overrides outside the package root", () => {
    const overrideA = createBundledPluginDir("kova-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("kova-facade-runtime-b-", "override-b");

    process.env.KOVA_BUNDLED_PLUGINS_DIR = overrideA;
    ...
    process.env.KOVA_BUNDLED_PLUGINS_DIR = overrideB;
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("kova-facade-identity-", "identity-check");
    process.env.KOVA_BUNDLED_PLUGINS_DIR = dir;
    const location = {
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("kova-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("kova-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("kova-facade-throw-");
    process.env.KOVA_BUNDLED_PLUGINS_DIR = dir;
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("kova-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("kova-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "kova.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("kova-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "kova.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("kova-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "kova.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
