...
export function collectDeepProbeFindings(params: {
  ...
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set KOVA_GATEWAY_TOKEN/KOVA_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("kova security audit --deep")}".`,
    });
  }
  return findings;
}
