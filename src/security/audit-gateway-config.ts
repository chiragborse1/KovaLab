...
type CollectDangerousConfigFlags = (cfg: KovaConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.KOVA_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.KOVA_GATEWAY_PASSWORD);
  ...
  if (bind === "loopback" && usesProxyTrust && trustedProxies.length === 0) {
    findings.push({
