...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: KovaConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "kova.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-security-install-"));
  });
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: KovaConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { KOVA_HOME: isolatedHome });
    ...
    await fs.mkdir(path.join(sharedExtensionsStateDir, "extensions", ".kova-install-backups"), {
      recursive: true,
      mode: 0o700,
    });
  });
    ...
    const cases = [
      {
        ...
        assert: (findings: Awaited<ReturnType<typeof runSharedExtensionsAudit>>) => {
          ...
          expect(finding?.detail).not.toContain(".kova-install-backups");
        },
      },
      {
