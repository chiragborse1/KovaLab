...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: KovaConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: KovaConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: KovaConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): KovaConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: KovaConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
