...
function resolveFetchConfig(config: KovaConfig): Record<string, unknown> | undefined {
...
function resolveSearchConfig(config: KovaConfig): Record<string, unknown> | undefined {
...
function addConfiguredSearchCredentialTargetIds(params: {
  ...
  const scopedConfig = asOptionalRecord(searchConfig[params.provider.id]);
  if (scopedConfig && configuredCredential === scopedConfig.apiKey) {
...
function addConfiguredFetchCredentialTargetIds(params: {
  ...
  const scopedConfig = asOptionalRecord(fetchConfig[params.provider.id]);
  if (scopedConfig && configuredCredential === scopedConfig.apiKey) {
    addConfigPathTargets({
      ...params,
...
function getCapabilityWebSearchAutoDetectTargets(config: KovaConfig): CommandSecretTargetScope {
...
function getCapabilityWebFetchAutoDetectTargets(config: KovaConfig): CommandSecretTargetScope {
  return getCapabilityWebProviderAutoDetectTargets({
    config,
...
function isScopedChannelSecretTargetEntry(params: {
  ...
  return (
    ...
    params.entry.configFile === "kova.json" &&
...
type CapabilityWebCommandSecretTargetParams = {
  ...
  resolveSelectedProviderId: (config: KovaConfig, providerId?: string | null) => string | undefined;
  autoDetectTargets: (config: KovaConfig) => CommandSecretTargetScope;
  selectedProviderTargetIds: (
    config: KovaConfig,
    providerId?: string | null,
  ) => SelectedProviderTargetIds;
