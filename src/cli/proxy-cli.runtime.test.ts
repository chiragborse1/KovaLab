...
describe("proxy cli runtime", () => {
  const envKeys = [
    "KOVA_DEBUG_PROXY_DB_PATH",
    "KOVA_DEBUG_PROXY_BLOB_DIR",
    "KOVA_DEBUG_PROXY_CERT_DIR",
    "KOVA_DEBUG_PROXY_SESSION_ID",
    "KOVA_DEBUG_PROXY_ENABLED",
  ] as const;
  const savedEnv = Object.fromEntries(envKeys.map((key) => [key, process.env[key]]));
  let tempDir = "";

  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "kova-proxy-cli-runtime-"));
    ...
    process.env.KOVA_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.KOVA_DEBUG_PROXY_ENABLED;
    delete process.env.KOVA_DEBUG_PROXY_SESSION_ID;
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.KOVA_DEBUG_PROXY_DB_PATH!,
      process.env.KOVA_DEBUG_PROXY_BLOB_DIR!,
    );
