...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: KovaConfig) => void }) => {
      ...
      return {
        path: "/tmp/kova.json",
        previousHash: "hash-1",
        snapshot: { path: "/tmp/kova.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: KovaConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: KovaConfig }>
    >(async () => ({
      path: "/tmp/kova.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: KovaConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/kova.json");
    ...
    expect(output).not.toContain("/tmp/kova.json\nforged");
