...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.KOVA_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.KOVA_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.KOVA_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.KOVA_PROFILE);
}
...
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  ...
  status=78
  printf '[%s] system-scoped kova gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
set "KOVA_RESTART_SCRIPT_DIR=%~dp0."
...
del "%~f0" >nul 2>&1
rmdir "%KOVA_RESTART_SCRIPT_DIR%" >nul 2>&1
exit /b %status%
# POWERSHELL
# Wait briefly to ensure file locks are released after update.
...
function Invoke-KovaStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".kova\\gateway.cmd"
...
if ($status -ne 0) {
  $status = Invoke-KovaStartupLauncher
}
if ($status -eq 0) {
...
`;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-restart-"));
