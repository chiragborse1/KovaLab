...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("kova-daemon-install-int-");
    configPath = path.join(tempHome, "kova.json");
    process.env.HOME = tempHome;
    process.env.KOVA_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Kova", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.KOVA_GATEWAY_TOKEN).toBeUndefined();
  });
});
