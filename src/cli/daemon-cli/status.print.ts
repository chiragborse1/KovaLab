...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("kova gateway install --force")}\` from the same --profile / KOVA_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.KOVA_PROFILE);
    defaultRuntime.error(
      errorText(
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.KOVA_PROFILE);
      defaultRuntime.error(
  ...
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.neuralstudio.in/troubleshooting`);
}
