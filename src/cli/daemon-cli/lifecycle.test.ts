...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KOVA_CONTAINER_HINT", "KOVA_PROFILE"]);
    delete process.env.KOVA_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["kova", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("fails restart when gateway remains unhealthy after the full timeout", async () => {
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toMatchObject({
      ...
      hints: [formatCliCommand("kova gateway status --deep"), formatCliCommand("kova doctor")],
    });
