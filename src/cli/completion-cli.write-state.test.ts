...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["kova.bash", "kova.fish", "kova.ps1", "kova.zsh"]),
    );
