...
export function buildToolPluginPackageManifest(params: {
  ...
  const kova =
    params.packageManifest.kova &&
    typeof params.packageManifest.kova === "object" &&
    !Array.isArray(params.packageManifest.kova)
      ? { ...(params.packageManifest.kova as JsonObject) }
      : {};
  return {
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("kova.plugin.json generated metadata is stale. Run kova plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `kova.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("kova.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`kova.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `kova.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(", ")}`,
    );
  }
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    ...
    files: ["dist", PLUGIN_MANIFEST_FILENAME, "README.md"],
    peerDependencies: {
  ...
  const descriptionLiteral = jsStringLiteral(`Add ${name} tools to Kova.`);
...
import { getToolPluginMetadata } from "getkova/plugin-sdk/tool-plugin";
...
`;
  ...
  writeJsonFile(path.join(rootDir, PLUGIN_MANIFEST_FILENAME), {
    id,
    name,
    description: `Add ${name} tools to Kova.`,
