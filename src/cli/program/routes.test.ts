...
describe("program routes", () => {
  ...
  it("passes parsed channel read-only route flags through", async () => {
    ...
    await expect(
      listRoute?.run(["node", "kova", "channels", "list", "--json", "--no-usage"]),
    ).resolves.toBe(true);
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(["gateway", "status"], ["node", "kova", "gateway", "status", "--url"]);
    await expectRunFalse(["gateway", "status"], ["node", "kova", "gateway", "status", "--token"]);
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "kova", "gateway", "status", "--password"],
    );
    await expectRunFalse(["gateway", "status"], ["node", "kova", "gateway", "status", "--timeout"]);
  });

  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "kova", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "kova", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "kova", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(route?.run(["node", "kova", "gateway", "status", "--no-probe"])).resolves.toBe(
      true,
    );
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "kova", "status", "--timeout"]);
  });

  it("routes status --json through the lean JSON command", async () => {
    const route = expectRoute(["status"]);
    await expect(
      route?.run(["node", "kova", "status", "--json", "--deep", "--usage", "--timeout", "5000"]),
    ).resolves.toBe(true);
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "kova", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "kova", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "kova", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "kova", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "kova", "config", "unset"]);
  });
  ...
  it("passes config unset path correctly when root option values precede command", async () => {
    ...
    await expect(
      route?.run(["node", "kova", "--profile", "work", "config", "unset", "update.channel"]),
    ).resolves.toBe(true);
  ...
  it("passes config unset path when root value options appear after subcommand", async () => {
    const route = expectRoute(["config", "unset"]);
    await expect(
      route?.run(["node", "kova", "config", "unset", "--profile", "work", "update.channel"]),
    ).resolves.toBe(true);
    expect(runConfigUnsetMock).toHaveBeenCalledWith({ path: "update.channel" });
  });
  ...
  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(["models", "list"], ["node", "kova", "models", "list", "--provider"]);
  });

  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "kova", "models", "status", "--probe-profile"],
    );
  });
  ...
  it("routes tasks list JSON through the lean task JSON command", async () => {
    ...
    expect(rootRoute?.canRun?.(["node", "kova", "tasks"])).toBe(false);
    await expect(
      rootRoute?.run(["node", "kova", "tasks", "--json", "--runtime", "cli", "--status=running"]),
    ).resolves.toBe(true);
    ...
    await expect(
      listRoute?.run(["node", "kova", "tasks", "list", "--json", "--runtime=cron"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks audit JSON through the lean task JSON command", async () => {
    ...
    expect(route?.loadPlugins).toBeUndefined();
    expect(route?.canRun?.(["node", "kova", "tasks", "audit"])).toBe(false);
    await expect(
      route?.run([
        "node",
        "kova",
  ...
  it("returns false for task JSON routes when option values are missing or unknown", async () => {
    await expectRunFalse(["tasks"], ["node", "kova", "tasks", "--json", "--runtime"]);
    await expectRunFalse(["tasks", "list"], ["node", "kova", "tasks", "list"]);
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "kova", "tasks", "audit", "--json", "--limit"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "kova", "tasks", "audit", "--json", "--unknown"],
    );
    expect(
      findRoutedCommand(["tasks", "cli"], ["node", "kova", "tasks", "--runtime", "cli"]),
    ).toBeNull();
  });
});
