...
describe("registerOnboardCommand", () => {
  ...
  it("runs onboarding even when no explicit flags are provided", async () => {
    await runCli(["onboard"]);

    expect(setupWizardCommandMock).toHaveBeenCalledWith(expect.any(Object), runtime);
  });
  ...
  it("forwards --gateway-token-ref-env", async () => {
    await runCli(["onboard", "--gateway-token-ref-env", "KOVA_GATEWAY_TOKEN"]);
    expect(setupWizardCommandMock).toHaveBeenCalledWith(
      expect.objectContaining({
        gatewayTokenRefEnv: "KOVA_GATEWAY_TOKEN",
      }),
      runtime,
    );
  });
