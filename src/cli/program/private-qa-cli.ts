...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.KOVA_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveKovaPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Kova source checkout.");
  }
