...
vi.mock("./register.settings.js", () => ({
  registerSettingsCommand: (program: Command) => {
...
describe("command-registry", () => {
  ...
  it("narrows to the primary command when command help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "kova", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "kova", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "kova", "status"], async () => {
      await program.parseAsync(["node", "kova", "status"]);
    });
