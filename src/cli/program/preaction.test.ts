...
beforeEach(() => {
  ...
  originalHideBanner = process.env.KOVA_HIDE_BANNER;
  ...
  delete process.env.KOVA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.KOVA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kova", "status", "--debug"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "agents", "list"],
    });
  ...
  it("loads plugins for local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "kova", "agent", "--local", "--message", "hi"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "channels", "add"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "kova", "configure"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@kovaai/matrix"],
      processArgv: ["node", "kova", "plugins", "install", "@kovaai/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "plugins", "install", MATRIX_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@kovaai/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "kova",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    ...
    vi.clearAllMocks();
    process.env.KOVA_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kova", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kova", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "kova", "agent", "--message", "hi", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kova", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "kova", "agents", "list", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kova", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "kova", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kova", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kova", "backup", "create", "--json"],
    });
