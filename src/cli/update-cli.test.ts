...
import type { KovaConfig, ConfigFileSnapshot } from "../config/types.kova.js";
...
vi.mock("../infra/kova-root.js", () => ({
  resolveKovaPackageRoot: vi.fn(),
  resolveKovaPackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/kova-config",
  };
});
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/kova-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as KovaConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/kova-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveKovaPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("kova-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("kova-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveKovaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("kova");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["kova", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/kova-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "kova-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("kova-completion-timeout");
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          KOVA_COMPLETION_SKIP_PLUGIN_COMMANDS: "1",
        }),
        timeout: 30_000,
      }),
    );
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        stdio: "inherit",
        env: expect.objectContaining({
          KOVA_UPDATE_POST_CORE: "1",
          KOVA_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("kova-downgraded-root");
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        KOVA_UPDATE_POST_CORE: "1",
        KOVA_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("passes pre-core plugin install records into the resumed post-core process", async () => {
      ...
      (_command: string, _args: string[], options?: { env?: NodeJS.ProcessEnv }) => {
        ...
        queueMicrotask(async () => {
          const installRecordsPath =
            options?.env?.KOVA_UPDATE_POST_CORE_INSTALL_RECORDS_PATH?.toString() ?? "";
    ...
    const spawnEnv = spawn.mock.calls.find(
      (call) =>
        ...
        call[2] !== null &&
        "env" in call[2] &&
        (call[2].env as NodeJS.ProcessEnv | undefined)?.KOVA_UPDATE_POST_CORE === "1",
    )?.[2]?.env as NodeJS.ProcessEnv | undefined;
    expect(spawnEnv?.KOVA_UPDATE_POST_CORE_INSTALL_RECORDS_PATH).toBeTruthy();
  });
    ...
    await withEnvAsync(
      {
        KOVA_UPDATE_POST_CORE: "1",
        KOVA_UPDATE_POST_CORE_CHANNEL: "stable",
        KOVA_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: installRecordsPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@kovaai/demo@1.0.0",
        resolvedSpec: "@kovaai/demo@1.0.0",
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @kovaai/demo@1.0.0"
  ...
  it("preserves fresh-process plugin failure details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.KOVA_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "kova", "update", "status", "--json"]);
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as KovaConfig,
        });
      },
  ...
  it("warns but still runs package updates when disk space looks low", async () => {
    ...
    expectPackageInstallSpec("getkova@latest");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    expect(
      vi
  ...
  it("refuses package updates from inside the gateway service process", async () => {
    mockPackageInstallStatus(createCaseDir("kova-update"));
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:chiragborse1/KovaLab#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("kova-update"));
        await updateCommand({ yes: true, tag: "github:chiragborse1/KovaLab#main" });
      },
      expectedSpec: "github:chiragborse1/KovaLab#main",
    },
    {
      name: "KOVA_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("kova-update"));
        await withEnvAsync(
          { KOVA_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/kova-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/kova-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "kova");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "kova", version: "2026.3.23" }),
      "utf-8",
    );
    ...
    expect(logs.join("\n")).toContain("expected installed version 2026.3.23-2");
  });

  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-update-package-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "kova");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "kova", version: "2026.4.21" }),
      "utf-8",
    );
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-update-current-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "kova");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "kova", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-update-optional-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "kova");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "kova", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("kova-update");
    const localAppData = createCaseDir("kova-localappdata");
    ...
    const portableGitUsr = path.join(localAppData, "Kova", "deps", "portable-git", "usr", "bin");
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("kova-update");
    const gitRoot = path.join(tempDir, "..", "kova");
    ...
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as KovaConfig,
      sourceConfig: { update: { channel: "stable" } } as KovaConfig,
      runtimeConfig: { update: { channel: "stable" } } as KovaConfig,
      config: { update: { channel: "stable" } } as KovaConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "git",
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `kova update`.",
    );
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("kova-updated-root");
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            KOVA_CONFIG_PATH: "./config/kova.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            KOVA_STATE_DIR: path.resolve("./state"),
            KOVA_CONFIG_PATH: path.resolve("./config/kova.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            KOVA_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ KOVA_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.KOVA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("kova-update-wizard");
    await withEnvAsync({ KOVA_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/kova as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ KOVA_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "kova"));
    });
