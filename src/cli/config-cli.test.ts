...
/**
 * Test for issue #6070:
 * `kova config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: KovaConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (cfg: KovaConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: { resolved: KovaConfig; config: KovaConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/kova.json",
    exists: true,
...
function setSnapshot(resolved: KovaConfig, config: KovaConfig) {
...
function withRuntimeDefaults(resolved: KovaConfig): KovaConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-kova.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: KovaConfig = {
    ...
    it("rejects plugin install record config updates", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          'plugins.installs["kova-web-search"].spec',
          '"@ollama/kova-web-search@0.2.2"',
          "--strict-json",
          "--dry-run",
        ]),
      ...
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("kova plugins install <spec>"),
      );
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("kova plugins update <plugin-id>"),
      );
    });
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__KOVA_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-kova.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: KovaConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("kova config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "kova config set channels.discord.token --ref-provider default --ref-source",
      );
      expect(helpText).toContain("--ref-id DISCORD_BOT_TOKEN");
      expect(helpText).toContain("kova config set --batch-file ./config-set.batch.json --dry-run");
    });
  });
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `kova-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `kova-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `kova-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `kova-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
  ...
  describe("path hardening", () => {
    ...
    it("rejects direct config set writes to auto-managed metadata", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
      ...
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("meta.lastTouchedVersion is auto-managed"),
      );
    });

    it("rejects parent metadata payloads that contain auto-managed keys", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          "meta",
          '{"lastTouchedAt":"2026-05-13T00:00:00.000Z"}',
          "--strict-json",
          "--dry-run",
        ]),
      ).rejects.toThrow("__exit__:1");
      ...
      expect(mockReadConfigFileSnapshot).not.toHaveBeenCalled();
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expect(mockError).toHaveBeenCalledWith(expect.stringContaining("meta is auto-managed"));
    });

    it("rejects config unset attempts for auto-managed metadata", async () => {
      ...
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("meta.lastTouchedAt is auto-managed"),
      );
    });
  });
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/kova.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.chiragborse1/KovaLab.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.chiragborse1/KovaLab.json");
    });
  });
});
