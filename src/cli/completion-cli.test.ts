...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_kova_gateway()");
    expect(script).toContain("(status) _kova_gateway_status ;;");
    expect(script).toContain("(restart) _kova_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "kova.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[kova]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_kova_register_completion]}" = "_kova_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _kova_register_completion
            [[ -z "\${precmd_functions[(r)_kova_register_completion]}" ]] || exit 12
            [[ "\${_comps[kova]-}" = "_kova_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'kova gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c kova -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c kova -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c kova -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
