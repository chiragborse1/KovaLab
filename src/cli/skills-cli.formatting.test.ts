...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-bundled-skills-test-"));
    process.env.KOVA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
