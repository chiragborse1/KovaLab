      ...
      resolvePluginUpdateSelection({
        installs: {
          "voice-call": createNpmInstall({
            ...
            resolvedName: "@kovaai/voice-call",
          }),
        },
        rawId: "@kovaai/voice-call@beta",
      }),
    ).toEqual({
      pluginIds: ["voice-call"],
      specOverrides: {
        "voice-call": "@kovaai/voice-call@beta",
      },
    });
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "kova-codex-app-server@0.2.0-beta.4",
      }),
    ).toEqual({
      pluginIds: ["kova-codex-app-server"],
      specOverrides: {
