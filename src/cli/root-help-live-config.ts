...
export function hasPluginHelpAffectingConfig(config: KovaConfig | null | undefined): boolean {
...
export function hasPluginHelpAffectingEnv(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.KOVA_BUNDLED_PLUGINS_DIR?.trim() || env.KOVA_DISABLE_BUNDLED_PLUGINS?.trim());
}
