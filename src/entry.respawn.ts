...
export const KOVA_NODE_OPTIONS_READY = "KOVA_NODE_OPTIONS_READY";
export const KOVA_NODE_EXTRA_CA_CERTS_READY = "KOVA_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.KOVA_NO_RESPAWN) ||
    isTruthyEnvValue(env.KOVA_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[KOVA_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[KOVA_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[KOVA_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[KOVA_NODE_OPTIONS_READY] = "1";
