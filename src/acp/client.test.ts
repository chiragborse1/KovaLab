...
const createTempDir = () => tempDirs.make("kova-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets KOVA_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "kova",
    });

    expect(env.KOVA_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("kova");
  });

  it("overrides pre-existing KOVA_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      KOVA_SHELL: "wrong",
    });
    expect(env.KOVA_SHELL).toBe("acp-client");
  ...
  it("preserves KOVA_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        KOVA_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["KOVA_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Kova bridge", () => {
    ...
    expect(env.KOVA_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.KOVA_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("KOVA_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "kova", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "kova", "dist", "entry.js");
    const shimPath = path.join(dir, "kova.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\kova\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/kova-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/kova-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/kova-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
