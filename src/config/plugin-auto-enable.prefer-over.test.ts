...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        KOVA_STATE_DIR: path.join(rootDir, "state"),
        KOVA_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
