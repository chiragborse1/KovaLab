...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Kova binary (${currentVersion}) is older than the config last written by Kova ${touchedVersion}.`,
    hints: [
      "Run the newer kova binary on PATH, or reinstall the intended gateway service from the newer install.",
