...
import { buildWebSearchProviderConfig, withTempHome, writeKovaConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    if (result.success) {
      expect(result.data.$schema).toBe("https://www.neuralstudio.in/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = KovaSchema.safeParse({ $schema: 123 });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://www.neuralstudio.in/config.json");
    }
  });
});
...
describe("plugins.entries.*.hooks", () => {
  ...
  it("accepts typed hook timeout policy", () => {
    const result = KovaSchema.safeParse({
      plugins: {
        entries: {
          "memory-core": {
  ...
  it("rejects invalid typed hook timeout policy", () => {
    for (const hooks of [
      { timeoutMs: 0 },
      { timeoutMs: 600_001 },
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@kovaai"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = KovaSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeKovaConfig(home, {
        gateway: { bind: "${KOVA_BIND}" },
      });

      const prev = process.env.KOVA_BIND;
      process.env.KOVA_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.KOVA_BIND;
        } else {
          process.env.KOVA_BIND = prev;
        }
      }
    });
  });
