...
import { resetConfigRuntimeState, type KovaConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        KOVA_DISABLE_PLUGIN_DISCOVERY_CACHE: undefined,
        KOVA_DISABLE_PLUGIN_MANIFEST_CACHE: undefined,
        KOVA_PLUGIN_CATALOG_PATHS: undefined,
        KOVA_MPM_CATALOG_PATHS: undefined,
        KOVA_PLUGIN_DISCOVERY_CACHE_MS: undefined,
        KOVA_PLUGIN_MANIFEST_CACHE_MS: undefined,
        KOVA_LOAD_SHELL_ENV: undefined,
        KOVA_DEFER_SHELL_ENV_FALLBACK: undefined,
        KOVA_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeKovaConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.KOVA_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected KOVA_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeKovaConfig(home, config);
...
export function buildWebSearchProviderConfig(params: {
  provider: NonNullable<
    NonNullable<NonNullable<NonNullable<KovaConfig["tools"]>["web"]>["search"]>["provider"]
  >;
  enabled?: boolean;
