...
function collectModelRefs(cfg: KovaConfig): string[] {
  const refs: string[] = [];
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: KovaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: KovaConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: KovaConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: KovaConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: KovaConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: KovaConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: KovaConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: KovaConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: KovaConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: KovaConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: KovaConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: KovaConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: KovaConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: KovaConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithEntries(cfg: KovaConfig): boolean {
  return Array.isArray(cfg.plugins?.allow) && cfg.plugins.allow.length > 0 && hasPluginEntries(cfg);
}

function hasConfiguredProviderModelOrHarness(cfg: KovaConfig, env: NodeJS.ProcessEnv): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: KovaConfig, env: NodeJS.ProcessEnv): boolean {
...
export function configMayNeedPluginAutoEnable(cfg: KovaConfig, env: NodeJS.ProcessEnv): boolean {
  if (hasPluginAllowlistWithEntries(cfg)) {
    return true;
  }
  if (hasConfiguredPluginConfigEntry(cfg)) {
...
function isPluginExplicitlyDisabled(cfg: KovaConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: KovaConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: KovaConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: KovaConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: KovaConfig, pluginId: string): KovaConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
