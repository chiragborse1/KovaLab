...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/kova/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/kova/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/kova/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/kova/nickserv-password.txt");
