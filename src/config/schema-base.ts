...
function stripChannelSchema(schema: ConfigSchema): ConfigSchema {
  ...
  // generated config schema so it does not show up as a configurable section.
...
function computeBaseConfigSchemaStablePayload(): BaseConfigSchemaStablePayload {
  ...
  const schema = KovaSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "KovaConfig";
  const schemaRoot = asJsonSchemaObject(schema);
  if (schemaRoot) {
    applyFieldDocumentation(schemaRoot, buildFieldDocumentation());
  }
  const baseHints = mapSensitivePaths(KovaSchema, "", buildBaseHints());
  const sensitiveUrlPaths = collectMatchingSchemaPaths(KovaSchema, "", isSensitiveUrlConfigPath);
  const stablePayload = {
