...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "kova.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      HOME: suiteHome,
      KOVA_HOME: undefined,
      KOVA_STATE_DIR: path.join(suiteHome, ".kova"),
      KOVA_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-config-plugin-validation-"));
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", async () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run kova doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
  ...
  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const installedPluginIndexPath = path.join(suiteHome, ".kova", "plugins", "installs.json");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run kova doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          KOVA_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
