...
const ETC_KOVA_DIR = path.join(ROOT_DIR, "etc", "kova");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "kova.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcKovaPath(...parts: string[]) {
  return path.join(ETC_KOVA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcKovaPath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "kova-includes-symlink-" }, async (tempRoot) => {
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "kova-includes-hardlink-" }, async (tempRoot) => {
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./extra.json5" }, path.join(configDir, "kova.json")),
        ).toThrow(/security checks|hardlink/i);
      });
    });

    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "kova-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "kova.json")),
