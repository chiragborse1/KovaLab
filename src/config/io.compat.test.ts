...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "kova-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  ...
  it("uses KOVA_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { KOVA_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
  ...
  it("honors explicit KOVA_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".kova", 20002, "custom.json");
      const io = createIoForHome(home, { KOVA_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("moves WhatsApp shared access defaults into accounts.default during runtime compat", () => {
    ...
    const next = migrated.next as KovaConfig | null;
