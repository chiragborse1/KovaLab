...
function collectConfigEnvVarsByTarget(cfg?: KovaConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: KovaConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: KovaConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: KovaConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: KovaConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.KOVA_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
