...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ KOVA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-kova",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-kova#general");
  });
  ...
  it("derives session transcripts dir from KOVA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { KOVA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when KOVA_STATE_DIR differs from stored paths", () => {
