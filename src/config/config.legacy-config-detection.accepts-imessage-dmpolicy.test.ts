...
function expectKovaSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = KovaSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectKovaSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = KovaSchema.safeParse(config);
