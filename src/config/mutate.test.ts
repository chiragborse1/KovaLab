...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/kova.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kova-config-mutate-" });
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".kova", "config", "plugins.json5");
