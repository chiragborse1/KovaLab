  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.kova/logs/kova.log" },
        hooks: {
          path: "~/.kova/hooks.json5",
        ...
        channels: {
          ...
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.kova/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".kova", "logs", "kova.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".kova", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".kova", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".kova", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".kova", "agents", "main"));
