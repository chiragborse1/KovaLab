...
// Keep integration tests deterministic: never read a real kova.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kova-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.KOVA_SESSION_CACHE_TTL_MS;
    process.env.KOVA_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.KOVA_SESSION_CACHE_TTL_MS;
    } else {
      process.env.KOVA_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
