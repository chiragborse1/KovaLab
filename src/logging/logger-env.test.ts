...
const logPathTracker = createSuiteLogPathTracker("kova-test-env-log-level-");

describe("KOVA_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.KOVA_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.KOVA_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.KOVA_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("KOVA_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid KOVA_LOG_LEVEL="nope"');
  });
});
