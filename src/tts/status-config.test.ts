...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kova-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  process.env.KOVA_STATE_DIR = path.join(home, ".kova");
  try {
    await run(home);
  } finally {
    restoreEnv("HOME", previousHome);
    restoreEnv("USERPROFILE", previousUserProfile);
    restoreEnv("KOVA_HOME", previousKovaHome);
    restoreEnv("KOVA_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".kova", "settings", "tts.json");
  ...
  it("derives the default prefs path from KOVA_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".kova-dev");
      ...
      vi.stubEnv("KOVA_CONFIG_PATH", path.join(stateDir, "kova.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
