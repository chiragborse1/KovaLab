...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("KOVA_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.KOVA_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray("KOVA_LIVE_CLI_BACKEND_ARGS", process.env.KOVA_LIVE_CLI_BACKEND_ARGS) ??
    params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(`KOVA_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`);
  }
  const resumeArgs =
    parseJsonStringArray(
      "KOVA_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.KOVA_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.KOVA_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.KOVA_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.KOVA_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.KOVA_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("KOVA_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("KOVA_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("KOVA_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("KOVA_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("KOVA_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("KOVA_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("KOVA_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("KOVA_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("KOVA_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("KOVA_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("KOVA_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
