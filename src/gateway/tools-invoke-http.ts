...
function resolveMemoryToolDisableReasons(cfg: KovaConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-kova-message-channel") ?? "");
  const accountId = normalizeOptionalString(getHeader(req, "x-kova-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-kova-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-kova-thread-id"));
