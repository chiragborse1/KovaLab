...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("kova.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("kova.gatewayTestHelpers.hoisted");
...
export function setTestConfigRoot(root: string): void {
  ...
  process.env.KOVA_CONFIG_PATH = path.join(root, "kova.json");
}
