...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("kova doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with KOVA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with KOVA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.KOVA_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://kova-gateway.ai:18789",
      onConnectError,
    });
