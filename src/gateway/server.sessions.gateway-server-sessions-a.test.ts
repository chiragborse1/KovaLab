...
beforeAll(async () => {
  ...
  sharedSessionStoreDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-"));
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.create stores local session model and parent linkage, and creates a transcript", async () => {
    const { dir, storePath } = await createSessionStoreDir();
    ...
    expect(created.payload?.key).toMatch(/^agent:ops:local:/);
    expect(created.payload?.entry?.label).toBe("Local Chat");
  ...
  test("sessions.create accepts an explicit key for persistent local sessions", async () => {
    await createSessionStoreDir();

    const key = "agent:ops-agent:local:direct:subagent-orchestrator";
    const created = await directSessionReq<{
      key?: string;
      sessionId?: string;
      entry?: {
    ...
    expect(created.payload?.entry?.label).toBe("Local Orchestrator");
    expect(created.payload?.sessionId).toMatch(
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
  });
  ...
  test("sessions.list surfaces transcript usage and model fallbacks from the transcript", async () => {
    ...
    const child = listed.payload?.sessions.find(
      (session) => session.key === "agent:main:local:child",
    );
    expect(parent?.childSessions).toEqual(["agent:main:local:child"]);
  ...
  test("sessions.patch preserves nested model ids under provider overrides", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gw-sessions-nested-"));
    const storePath = path.join(dir, "sessions.json");
    await fs.writeFile(
      storePath,
      JSON.stringify({
    ...
    await withEnvAsync({ KOVA_CONFIG_PATH: undefined }, async () => {
      const { clearConfigCache, clearRuntimeConfigSnapshot } = await getGatewayConfigModule();
      clearConfigCache();
      clearRuntimeConfigSnapshot();
      const cfg = {
      ...
      const configPath = path.join(dir, "kova.json");
      await fs.writeFile(configPath, JSON.stringify(cfg, null, 2), "utf-8");

      await withEnvAsync({ KOVA_CONFIG_PATH: configPath }, async () => {
  ...
  test("webchat clients cannot patch, delete, compact, or restore sessions", async () => {
    ...
    await connectOk(ws, {
      client: {
        id: GATEWAY_CLIENT_IDS.LOCAL_CHAT,
        version: "1.0.0",
        platform: "test",
  ...
  test("session:patch hook fires with correct context", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-patch-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("session:patch hook does not fire for webchat clients", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-webchat-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("session:patch hook only fires after successful patch", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-success-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("operator-client client can delete sessions even in webchat mode", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-sessions-operator-client-delete-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
