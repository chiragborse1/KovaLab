...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.KOVA_LIVE_ACP_BIND);
...
function shouldRequireBoundAssistantTranscript(liveAgent: LiveAcpAgent): boolean {
  return (
    liveAgent === "droid" ||
    liveAgent === "opencode" ||
    isTruthyEnvValue(process.env.KOVA_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT)
  );
}

function shouldRequireCronMcpProbe(): boolean {
  return isTruthyEnvValue(process.env.KOVA_LIVE_ACP_BIND_REQUIRE_CRON);
}
...
function resolveLiveParentModel(): string {
  return normalizeOpenAiModelRef(
    process.env.KOVA_LIVE_ACP_BIND_PARENT_MODEL?.trim() ||
      process.env.KOVA_LIVE_ACP_BIND_CODEX_MODEL?.trim() ||
...
async function prepareCodexHomeForLiveBindTest(): Promise<void> {
  ...
  const model = process.env.KOVA_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.KOVA_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.KOVA_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.KOVA_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "kova.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.KOVA_GATEWAY_PORT = previous.port;
        }
