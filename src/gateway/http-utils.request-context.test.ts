...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-kova-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-kova-message-channel": " Custom-Channel " }),
      model: "kova",
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-kova-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-kova-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(createReq({ "x-kova-scopes": "operator.write" }), {
        authMethod: "trusted-proxy",
        trustDeclaredOperatorScopes: true,
      }),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(createReq({ "x-kova-scopes": "operator.admin" }), {
        authMethod: "trusted-proxy",
        trustDeclaredOperatorScopes: true,
      }),
    ).toBe(true);
