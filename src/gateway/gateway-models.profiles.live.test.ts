...
import { ensureKovaModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, KovaConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.KOVA_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.KOVA_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.KOVA_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.KOVA_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.KOVA_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.KOVA_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["KOVA_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.KOVA_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.KOVA_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.KOVA_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.KOVA_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.KOVA_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.KOVA_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.KOVA_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.KOVA_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("KOVA_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("KOVA_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("KOVA_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.KOVA_LIVE_GATEWAY_MODELS;
    delete process.env.KOVA_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.KOVA_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.KOVA_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.KOVA_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.KOVA_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.KOVA_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: KovaConfig;
  agentDir: string;
}): Promise<KovaConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<KovaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.KOVA_CONFIG_PATH,
    token: process.env.KOVA_GATEWAY_TOKEN,
    skipChannels: process.env.KOVA_SKIP_CHANNELS,
    skipGmail: process.env.KOVA_SKIP_GMAIL_WATCHER,
    skipCron: process.env.KOVA_SKIP_CRON,
    skipCanvas: process.env.KOVA_SKIP_CANVAS_HOST,
    disableBonjour: process.env.KOVA_DISABLE_BONJOUR,
    logLevel: process.env.KOVA_LOG_LEVEL,
    agentDir: process.env.KOVA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.KOVA_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.KOVA_DISABLE_BONJOUR = "1";
    process.env.KOVA_LOG_LEVEL = "silent";
  }
  ...
  const hostAgentDir = resolveKovaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-live-state-"));
  ...
  process.env.KOVA_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".kova"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".kova", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.kova-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveKovaAgentDir();
  const sanitizedCfg: KovaConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-live-"));
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Kova live tool probe (local, safe): " +
    ...
    process.env.KOVA_CONFIG_PATH = previous.configPath;
    process.env.KOVA_GATEWAY_TOKEN = previous.token;
    process.env.KOVA_SKIP_CHANNELS = previous.skipChannels;
    process.env.KOVA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.KOVA_SKIP_CRON = previous.skipCron;
    process.env.KOVA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.KOVA_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.KOVA_LOG_LEVEL = previous.logLevel;
    process.env.KOVA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.KOVA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via KOVA_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.kova-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
