...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Kova MCP tool `kova-tools/cron` (server `kova-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__kova-tools__cron` or `mcp__kova_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the Kova cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Kova MCP tool `kova-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__kova-tools__cron` or `mcp__kova_tools__cron`. " +
      ...
      "I will verify externally with the Kova cron CLI."
    );
  }
  return (
    "Your previous Kova cron MCP tool call was cancelled before the job was created. " +
    "Retry the Kova MCP tool `kova-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__kova-tools__cron` or `mcp__kova_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runKovaCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  delete childEnv.VITEST;
  delete childEnv.VITEST_MODE;
  delete childEnv.VITEST_POOL_ID;
  delete childEnv.VITEST_WORKER_ID;
  const { stdout, stderr } = await execFileAsync(process.execPath, ["kova.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runKovaCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
