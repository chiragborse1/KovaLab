...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.KOVA_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const kovaDir = path.join(params.homeDir, ".kova");
  await fs.mkdir(kovaDir, { recursive: true });
  await fs.writeFile(
    path.join(kovaDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.KOVA_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-live-codex-bind-"));
      const tempHome = path.join(tempRoot, "home");
      const stateDir = path.join(tempRoot, "state");
      const workspace = path.join(tempRoot, "workspace");
      const configPath = path.join(tempRoot, "kova.json");
      ...
      const bindModel = process.env.KOVA_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("KOVA_CONFIG_PATH", previous.configPath);
        restoreEnvVar("KOVA_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("KOVA_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("KOVA_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("KOVA_SKIP_CRON", previous.skipCron);
        restoreEnvVar("KOVA_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("KOVA_STATE_DIR", previous.stateDir);
      }
    },
