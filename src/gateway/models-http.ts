...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([KOVA_MODEL_ID, KOVA_DEFAULT_MODEL_ID]);
  ids.add(`kova/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`kova/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== KOVA_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
