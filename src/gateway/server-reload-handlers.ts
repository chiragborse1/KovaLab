...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: KovaConfig) => ChannelHealthMonitor | null;
};
...
> & {
  ...
  initialConfig: KovaConfig;
  initialCompareConfig?: KovaConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: KovaConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: KovaConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (KOVA_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (KOVA_SKIP_CHANNELS=1 or KOVA_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: KovaConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
