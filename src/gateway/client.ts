...
import { resolveAllowInsecurePrivateWs } from "./connection-details.js";
...
export class GatewayClient {
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          "Run `kova doctor --fix` for guidance.",
      );
  ...
  private shouldPauseReconnectAfterAuthFailure(detailCode: string | null): boolean {
    ...
    if (
      ...
      detailCode === ConnectErrorDetailCodes.OPERATOR_CLIENT_DEVICE_IDENTITY_REQUIRED ||
  ...
  async request<T = Record<string, unknown>>(
    ...
    const p = new Promise<T>((resolve, reject) => {
      ...
      if (opts?.signal) {
        ...
        abortCleanup = () => opts.signal?.removeEventListener("abort", abortHandler);
      }
      this.pending.set(id, {
