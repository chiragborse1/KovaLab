...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gw-"));
    try {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gw-"));
  ...
  test("chat.history persists assistant image data URLs as managed image blocks", async () => {
    await withMainSessionStore(async (dir) => {
      ...
      process.env.KOVA_STATE_DIR = dir;
      const pngB64 =
