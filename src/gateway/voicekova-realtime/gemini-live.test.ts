...
describe("VoiceKovaGeminiLiveAdapter watchdog", () => {
  afterEach(() => {
    vi.useRealTimers();
  });

  it("stays paused while async Kova tool work is still running", () => {
    vi.useFakeTimers();
    const adapter = new VoiceKovaGeminiLiveAdapter();
...
describe("VoiceKovaGeminiLiveAdapter tool cancellation", () => {
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it("cancels async Kova tool work when Gemini closes after the working ack", () => {
    const adapter = new VoiceKovaGeminiLiveAdapter();
    const events: unknown[] = [];
    const internals = adapter as unknown as {
      asyncToolCallIds: Set<string>;
  ...
  it("defers goAway rotation until async Kova tool work finishes", () => {
    const adapter = new VoiceKovaGeminiLiveAdapter();
