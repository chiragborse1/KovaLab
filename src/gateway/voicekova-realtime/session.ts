...
import { VoiceKovaGeminiLiveAdapter } from "./gemini-live.js";
import {
  createVoiceKovaRealtimeToolRuntime,
  type VoiceKovaRealtimeToolRuntime,
} from "./tool-runtime.js";
import type {
  VoiceKovaClientEvent,
  VoiceKovaRealtimeAdapter,
  VoiceKovaServerEvent,
  VoiceKovaSessionConfigEvent,
  VoiceKovaToolCallEvent,
} from "./types.js";

const log = createSubsystemLogger("gateway").child("voicekova-realtime");

type VoiceKovaRealtimeSessionOptions = {
  ws: WebSocket;
  req: IncomingMessage;
  ...
  adapterFactory?: () => VoiceKovaRealtimeAdapter;
};

export class VoiceKovaRealtimeSession {
  private readonly id = randomUUID();
  private readonly startedAt = Date.now();
  private readonly ws: WebSocket;
  private readonly req: IncomingMessage;
  private readonly auth: ResolvedGatewayAuth;
  private readonly gatewayConfig: KovaConfig;
  ...
  private readonly adapterFactory: () => VoiceKovaRealtimeAdapter;
  private adapter: VoiceKovaRealtimeAdapter | null = null;
  private toolRuntime: VoiceKovaRealtimeToolRuntime | null = null;
  private config: VoiceKovaSessionConfigEvent | null = null;
  private handshakeTimer: ReturnType<typeof setTimeout> | null = null;
  private closed = false;
  private configStarted = false;

  constructor(opts: VoiceKovaRealtimeSessionOptions) {
    this.ws = opts.ws;
    this.req = opts.req;
    this.auth = opts.auth;
    ...
    this.adapterFactory = opts.adapterFactory ?? (() => new VoiceKovaGeminiLiveAdapter());
  }
  ...
  private async startSession(config: VoiceKovaSessionConfigEvent): Promise<void> {
    ...
    try {
      if (!process.env.GEMINI_API_KEY?.trim()) {
        throw new Error("GEMINI_API_KEY is required for VoiceKova real-time brain mode");
      }
      this.toolRuntime =
  ...
  private handleAdapterEvent(event: VoiceKovaServerEvent): void {
  ...
  private handleToolCall(event: VoiceKovaToolCallEvent): void {
    if (
  ...
  private resolveToolSessionKey(): string {
    ...
    if (configured) {
      return `agent:main:voicekova:${configured}`;
    }
    return `agent:main:voicekova:${this.id}`;
  }

  private send(event: VoiceKovaServerEvent): void {
...
function parseClientEvent(raw: RawData): VoiceKovaClientEvent | null {
  try {
    const parsed = JSON.parse(rawDataToString(raw)) as unknown;
    if (!parsed || typeof parsed !== "object" || !("type" in parsed)) {
      return null;
    }
    return parsed as VoiceKovaClientEvent;
  } catch {
    return null;
  }
}
