import type { VoiceKovaSessionConfigEvent } from "./types.js";
...
**Tool call timing:**
- Kova tools run asynchronously after an initial "working" result.
...
- When the real Kova tool result is injected, share it naturally if it is still relevant.
...
You are running inside Kova as the real-time brain. Use Kova tools directly for anything beyond basic conversation:
...
When in doubt, use the relevant Kova tool. Do not claim you lack access until a Kova tool confirms the task cannot be done.

## Mandatory Memory Rule

You do not have reliable memory of past sessions inside this live conversation. If the user asks what happened earlier, recently, last time, today, yesterday, or in any prior conversation, use Kova memory or session-history tools before answering.
`.trim();

export function buildInstructions(config: VoiceKovaSessionConfigEvent): string {
  const parts: string[] = [];
...
function buildDeviceContext(config: VoiceKovaSessionConfigEvent): string | null {
