...
import { VoiceKovaRealtimeToolRuntime } from "./tool-runtime.js";
import { buildToolResultContext } from "./tools.js";
import type { VoiceKovaToolCallEvent } from "./types.js";

const previousToolTimeoutMs = process.env.KOVA_VOICEKOVA_REALTIME_TOOL_TIMEOUT_MS;
const previousMaxConcurrentTools = process.env.KOVA_VOICEKOVA_REALTIME_MAX_CONCURRENT_TOOLS;

afterEach(() => {
  restoreEnv("KOVA_VOICEKOVA_REALTIME_TOOL_TIMEOUT_MS", previousToolTimeoutMs);
  restoreEnv("KOVA_VOICEKOVA_REALTIME_MAX_CONCURRENT_TOOLS", previousMaxConcurrentTools);
});

describe("VoiceKovaRealtimeToolRuntime", () => {
  it("does not expose ask_brain as a Gemini tool declaration", () => {
    const runtime = new VoiceKovaRealtimeToolRuntime([
      makeTool("ask_brain"),
      makeTool("nodes"),
      makeTool("web_search"),
    ]);
  ...
  it("does not turn non-cooperative cancellations into timeout injections", async () => {
    process.env.KOVA_VOICEKOVA_REALTIME_TOOL_TIMEOUT_MS = "10";
    const runtime = new VoiceKovaRealtimeToolRuntime([
  ...
  it("frees the concurrency slot after a non-cooperative tool times out", async () => {
    process.env.KOVA_VOICEKOVA_REALTIME_TOOL_TIMEOUT_MS = "10";
    process.env.KOVA_VOICEKOVA_REALTIME_MAX_CONCURRENT_TOOLS = "1";
    const runtime = new VoiceKovaRealtimeToolRuntime([
      makeTool("stuck", async () => await new Promise<never>(() => {})),
    ...
    expect(callbacks.progress.map((entry) => entry.summary)).toContain(
      "stuck failed: Kova tool timed out after 10ms",
    );
...
describe("VoiceKova realtime tool context", () => {
