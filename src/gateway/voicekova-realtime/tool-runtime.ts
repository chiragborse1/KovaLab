...
import { createKovaCodingTools } from "../../agents/pi-tools.js";
...
import type { VoiceKovaRealtimeToolDeclaration, VoiceKovaToolCallEvent } from "./types.js";
...
type ToolRuntimeDeps = {
  createTools?: typeof createKovaCodingTools;
};

export type VoiceKovaRealtimeToolRuntimeOptions = {
  config: KovaConfig;
  sessionId: string;
  sessionKey: string;
...
export class VoiceKovaRealtimeToolRuntime {
  readonly declarations: VoiceKovaRealtimeToolDeclaration[];
  ...
  handleToolCall(event: VoiceKovaToolCallEvent, callbacks: RuntimeCallbacks): boolean {
    ...
    if (this.inFlight.size >= this.maxConcurrentTools) {
      callbacks.sendToolResult(
        ...
        JSON.stringify({
          status: "busy",
          tool: event.name,
          error: "Too many Kova tools are already running.",
        }),
      );
      return true;
    }
  ...
  abortTool(callId: string): void {
    ...
    inFlight.controller.abort(new Error("Kova tool cancelled"));
  }
    ...
    try {
      ...
      const message =
        inFlight.abortReason === "timeout"
          ? `Kova tool timed out after ${this.timeoutMs}ms`
  ...
  private async executeToolWithTimeout(params: {
    ...
    const timeout = new Promise<never>((_, reject) => {
      inFlight.timeout = setTimeout(() => {
        if (inFlight.abortReason === "cancelled") {
          reject(new Error("Kova tool cancelled"));
          return;
        }
        inFlight.abortReason = "timeout";
        inFlight.controller.abort(new Error(`Kova tool timed out after ${this.timeoutMs}ms`));
        reject(new Error(`Kova tool timed out after ${this.timeoutMs}ms`));
      }, this.timeoutMs);
    });
...
export function createVoiceKovaRealtimeToolRuntime(
  options: VoiceKovaRealtimeToolRuntimeOptions,
): VoiceKovaRealtimeToolRuntime {
  const { sessionAgentId } = resolveSessionAgentIds({
    sessionKey: options.sessionKey,
    config: options.config,
  });
  const workspaceDir = resolveAgentWorkspaceDir(options.config, sessionAgentId);
  const createTools = options.deps?.createTools ?? createKovaCodingTools;
  return new VoiceKovaRealtimeToolRuntime(
    createTools({
      config: options.config,
      sessionKey: options.sessionKey,
      sessionId: options.sessionId,
      runId: `voicekova-realtime-${options.sessionId}`,
      trigger: "user",
      workspaceDir,
...
function resolveToolTimeoutMs(): number {
  const value = Number.parseInt(process.env.KOVA_VOICEKOVA_REALTIME_TOOL_TIMEOUT_MS ?? "", 10);
  return Number.isFinite(value) && value > 0 ? value : DEFAULT_TOOL_TIMEOUT_MS;
}

function resolveMaxConcurrentTools(): number {
  const value = Number.parseInt(process.env.KOVA_VOICEKOVA_REALTIME_MAX_CONCURRENT_TOOLS ?? "", 10);
