...
import { resolveRealtimeSenderIsOwner, VoiceKovaRealtimeSession } from "./session.js";
import type {
...
function makeAdapter(): VoiceKovaRealtimeAdapter {
  return {
    connect: vi.fn(),
    sendAudio: vi.fn(),
    commitAudio: vi.fn(),
...
describe("VoiceKovaRealtimeSession lifecycle", () => {
  it("sends session summary before closing after terminal adapter errors", () => {
    ...
    const internals = session as unknown as {
      adapter: VoiceKovaRealtimeAdapter;
      config: VoiceKovaSessionConfigEvent;
      handleAdapterEvent(event: VoiceKovaServerEvent): void;
    };
