...
const previousTestHandshakeTimeout = process.env.KOVA_TEST_HANDSHAKE_TIMEOUT_MS;
...
afterEach(() => {
  ...
  process.env.KOVA_TEST_HANDSHAKE_TIMEOUT_MS = previousTestHandshakeTimeout;
});

describe("VoiceKova realtime gateway upgrade", () => {
  it("accepts the realtime path without the generic gateway websocket handler", async () => {
    delete process.env.GEMINI_API_KEY;
    await withRealtimeGateway(async ({ port }) => {
      const ws = new WebSocket(`ws://127.0.0.1:${port}${VOICEKOVA_REALTIME_PATH}`);
      ...
      try {
        ...
        await expect(nextMessage).resolves.toMatchObject({
          type: "error",
          message: "GEMINI_API_KEY is required for VoiceKova real-time brain mode",
        });
      } finally {
  ...
  it("closes idle realtime sockets that never send session.config", async () => {
    process.env.KOVA_TEST_HANDSHAKE_TIMEOUT_MS = "50";
    await withRealtimeGateway(async ({ port }) => {
      const ws = new WebSocket(`ws://127.0.0.1:${port}${VOICEKOVA_REALTIME_PATH}`);
