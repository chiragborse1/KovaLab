...
export async function startGatewayServer(
  ...
  const minimalTestGateway = isVitestRuntimeEnv() && process.env.KOVA_TEST_MINIMAL_GATEWAY === "1";
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `kova config set gateway.auth.mode token` and `kova config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: KovaConfig) =>
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: KovaConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
