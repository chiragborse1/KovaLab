...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.KOVA_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as KovaConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "kova-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.KOVA_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
