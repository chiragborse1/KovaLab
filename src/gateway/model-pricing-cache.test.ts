...
describe("model-pricing-cache", () => {
    ...
    const config = {
      agents: {
        defaults: {
          ...
          pulse: { model: "moonshot/kimi-k2.6" },
          models: {
        ...
        list: [
          {
            ...
            pulse: { model: "cerebras/llama-3.3-70b" },
          },
        ],
      },
      channels: {
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as KovaConfig).map((ref) => modelKey(ref.provider, ref.model));
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/kova.plugin.json",
    ...overrides,
  };
}
