...
const { createKovaTools } = await import("../agents/kova-tools.js");
...
type SessionSendTool = ReturnType<typeof createKovaTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createKovaTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["KOVA_GATEWAY_PORT", "KOVA_GATEWAY_TOKEN"]);
  ...
  process.env.KOVA_GATEWAY_PORT = String(gatewayPort);
  process.env.KOVA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("KOVA_CONFIG_PATH missing in gateway test environment");
      }
