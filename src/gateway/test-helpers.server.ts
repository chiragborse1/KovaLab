...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.KOVA_CONFIG_PATH) {
    configPaths.add(process.env.KOVA_CONFIG_PATH);
  }
  if (process.env.KOVA_STATE_DIR) {
    configPaths.add(path.join(process.env.KOVA_STATE_DIR, "kova.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.KOVA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "kova-test-no-bundled-extensions")
    : "kova-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".kova-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".kova-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
...
const CONNECT_CHALLENGE_NONCE_KEY = "__kovaTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__kovaTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["KOVA_GATEWAY_TOKEN"]);
  const prev = process.env.KOVA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.KOVA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.KOVA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
