...
let lastCreateKovaToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/kova-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateKovaToolsContext?.agentTo,
          agentThreadId: lastCreateKovaToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createKovaTools: (ctx: Record<string, unknown>) => {
      lastCreateKovaToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateKovaToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-kova-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-kova-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateKovaToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateKovaToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateKovaToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateKovaToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateKovaToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateKovaToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-kova-message-to": "channel:24514",
        "x-kova-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
