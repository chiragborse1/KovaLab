...
describe("gateway auth", () => {
  ...
  it("resolves token/password from KOVA gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          KOVA_GATEWAY_TOKEN: "env-token",
          KOVA_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides KOVA_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        KOVA_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
