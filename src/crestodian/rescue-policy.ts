...
function resolveAgentEntry(cfg: KovaConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: KovaConfig, agentId?: string) {
  return resolveAgentEntry(cfg, agentId)?.tools?.exec;
}

function resolveScopedSandboxMode(cfg: KovaConfig, agentId?: string): "off" | "non-main" | "all" {
  return (
    resolveAgentEntry(cfg, agentId)?.sandbox?.mode ?? cfg.agents?.defaults?.sandbox?.mode ?? "off"
  );
}

function isYoloHostPosture(cfg: KovaConfig, agentId?: string): boolean {
...
export function resolveCrestodianRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Crestodian rescue is blocked because Kova sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  ...
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Crestodian rescue only accepts commands from an Kova owner.",
    };
  }
