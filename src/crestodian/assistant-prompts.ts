...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, Kova's ring-zero setup helper.",
  "Turn the user's request into exactly one safe Kova Crestodian command.",
  ...
  "Use the provided Kova docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Kova docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Kova source: ${
