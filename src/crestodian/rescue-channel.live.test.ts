...
const runLive =
  truthy(process.env.KOVA_LIVE_TEST) && truthy(process.env.KOVA_LIVE_CRESTODIAN_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.KOVA_LIVE_CRESTODIAN_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
...
async function runRescue(params: { commandBody: string; cfg: KovaConfig; ctx?: CommandContext }) {
...
describeLive("Crestodian live rescue channel smoke", () => {
  ...
  it("handles /crestodian status and a persistent approval roundtrip", async () => {
    ...
    vi.stubEnv("KOVA_CONFIG_PATH", configPath);
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    const config = JSON.parse(await fs.readFile(configPath, "utf8")) as KovaConfig;
