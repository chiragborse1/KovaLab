...
function createIntentEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-restart-intent-"));
  tempDirs.push(dir);
  return {
    ...process.env,
    KOVA_STATE_DIR: dir,
...
function intentPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.KOVA_STATE_DIR ?? "", "gateway-restart-intent.json");
}
...
describe("gateway restart intent", () => {
  ...
  it("does not follow an existing intent-path symlink when writing", () => {
    const env = createIntentEnv();
    const targetPath = path.join(env.KOVA_STATE_DIR ?? "", "attacker-target.txt");
