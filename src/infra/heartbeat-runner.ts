...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: KovaConfig, heartbeat?: HeartbeatConfig) {
  ...
  if (
    ...
    return agentDefaultTimeoutSeconds;
  }
  // The Pulse dispatcher awaits turns serially. Keep unset Pulse timeouts tied
  // to cadence instead of inheriting the long general agent timeout.
...
function isHeartbeatTypingEnabled(params: { cfg: KovaConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: KovaConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: KovaConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
