 ...
 * returning control to the caller (typically `triggerKovaRestart` →
...
/**
 * Parse kova gateway PIDs from lsof -Fpc stdout, excluding the current
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (
        currentPid != null &&
        currentCmd &&
        normalizeLowercaseStringOrEmpty(currentCmd).includes("kova")
      ) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an kova
...
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only kova gateway processes.
