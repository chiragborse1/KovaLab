...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/kova-state", "devices")).toEqual({
      dir: path.join("/tmp/kova-state", "devices"),
      pendingPath: path.join("/tmp/kova-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/kova-state", "devices", "paired.json"),
    });
  });
