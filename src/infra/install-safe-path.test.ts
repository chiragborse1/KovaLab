...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@kovaai/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@kovaai/matrix", id: "matrix", expected: true },
    { packageName: "@kovaai/matrix", id: "@kovaai/matrix", expected: true },
    { packageName: "@kovaai/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@kovaai/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@kovaai__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "kova-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "kova-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "kova-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "kova-install-safe-" }, async (parentDir) => {
