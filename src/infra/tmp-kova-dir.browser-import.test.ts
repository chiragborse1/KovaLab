...
describe("tmp-kova-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__kovaTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_KOVA_TMP_DIR, resolvePreferredKovaTmpDir } from "./src/infra/tmp-kova-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_KOVA_TMP_DIR,
            resolverType: typeof resolvePreferredKovaTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-kova-dir-browser-entry.ts",
      },
      write: false,
    });
