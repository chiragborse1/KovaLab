...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__kova_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "kova-root");
...
function setPackageRoot(root: string, name = "kova") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./kova-root.js").resolveKovaPackageRootSync,
  asyncResolver: typeof import("./kova-root.js").resolveKovaPackageRoot,
  opts: Parameters<typeof import("./kova-root.js").resolveKovaPackageRootSync>[0],
...
vi.mock("./kova-root.fs.runtime.js", () => ({
  kovaRootFsSync: mockFsModule(),
  kovaRootFs: mockFsPromisesModule(),
}));

describe("resolveKovaPackageRoot", () => {
  let resolveKovaPackageRoot: typeof import("./kova-root.js").resolveKovaPackageRoot;
  let resolveKovaPackageRootSync: typeof import("./kova-root.js").resolveKovaPackageRootSync;

  beforeAll(async () => {
    ({ resolveKovaPackageRoot, resolveKovaPackageRootSync } = await import("./kova-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "kova");
        const pkgRoot = path.join(project, "node_modules", "kova");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "kova");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "kova.mjs")));
    ...
    {
      name: "falls through from a non-kova moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-kova");
    ...
    {
      name: "returns null for non-kova package roots",
      setup: () => {
        const pkgRoot = fx("not-kova");
        setPackageRoot(pkgRoot, "not-kova");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "kova");
        state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "kova.mjs")));
        const pkgRoot = path.join(project, "node_modules", "kova");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
      },
