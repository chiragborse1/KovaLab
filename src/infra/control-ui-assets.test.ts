...
vi.mock("./kova-root.js", () => ({
  resolveKovaPackageRoot: vi.fn(async () => null),
  resolveKovaPackageRootSync: vi.fn(() => null),
}));
...
let kovaRoot: typeof import("./kova-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    kovaRoot = await import("./kova-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/kova");
    const wrapperArgv1 = abs("fixtures/bin/kova");
  ...
  it("uses resolveKovaPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/kova");
    (kovaRoot.resolveKovaPackageRoot as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      pkgRoot,
    );

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/kova"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "kova" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "kova.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-kova");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/kova-bundle");
    (
      kovaRoot.resolveKovaPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Kova.app/Contents/MacOS/Kova");
    const bundledUiDir = abs("fixtures/Kova.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/kova-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
