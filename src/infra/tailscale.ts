...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.KOVA_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Kova. You can keep running the web gateway without it: \`${formatCliCommand("kova gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
...
function funnelStatusBackendsForPort(status: Record<string, unknown>): Set<string> {
  ...
  for (const [host, handlers] of Object.entries(web)) {
    if (!enabledHosts.has(host) || !handlers || typeof handlers !== "object") {
      continue;
    }
