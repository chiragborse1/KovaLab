...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Kova tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideKova = path.join(os.tmpdir(), "outside-kova", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
