...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.KOVA_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-current-bindings-"));
    process.env.KOVA_STATE_DIR = testStateDir;
