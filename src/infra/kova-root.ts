...
import { kovaRootFs, kovaRootFsSync } from "./kova-root.fs.runtime.js";
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await kovaRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(kovaRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = kovaRootFsSync.realpathSync(normalized);
...
export async function resolveKovaPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveKovaPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
