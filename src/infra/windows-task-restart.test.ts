...
const resolvePreferredKovaTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".kova", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-kova-dir.js", () => ({
  resolvePreferredKovaTmpDir: () => resolvePreferredKovaTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredKovaTmpDirMock.mockReset();
    resolvePreferredKovaTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ KOVA_PROFILE: "work" });
    ...
    expect(script).toContain(
      'kova restart attempt source=windows-task-handoff target="Kova Gateway (work)"',
    );
    expect(script).toContain('schtasks /Run /TN "Kova Gateway (work)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers KOVA_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Kova Gateway (custom)" >>');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredKovaTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ KOVA_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-state-"));
