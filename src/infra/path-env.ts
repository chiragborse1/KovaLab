...
function candidateBinDirs(opts: EnsureKovaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Kova process is on.
  try {
  ...
  // Bundled macOS app: `kova` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.KOVA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "kova"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `kova`
...
/**
 * Best-effort PATH bootstrap so skills that require the `kova` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKovaCliOnPath(opts: EnsureKovaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KOVA_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.KOVA_PATH_BOOTSTRAPPED = "1";
