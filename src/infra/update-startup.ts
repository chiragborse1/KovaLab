...
import { resolveKovaPackageRoot } from "./kova-root.js";
...
function resolveAutoUpdatePolicy(cfg: KovaConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: KovaConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.KOVA_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by KOVA_NO_AUTO_UPDATE", {
