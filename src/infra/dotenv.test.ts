...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "KOVA_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "KOVA_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "KOVA_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "KOVA_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "kova-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.kova/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the gateway.env compatibility fallback after ~/.kova/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".kova");
        process.env.KOVA_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "kova", "gateway.env"),
  ...
  it("does not warn about dotenv conflicts when the key is already set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        process.env.FOO = "from-shell";
        await writeEnvFile(path.join(stateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(path.join(base, ".config", "kova", "gateway.env"), "FOO=from-gateway\n");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "KOVA_STATE_DIR=./evil-state",
            "KOVA_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.KOVA_STATE_DIR).toBe(stateDir);
        expect(process.env.KOVA_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "KOVA_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "KOVA_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "KOVA_LIVE_GEMINI_KEY=sk-gemini-live",
            "KOVA_LIVE_OPENAI_KEY=sk-openai-live",
            "KOVA_GATEWAY_TOKEN=attacker-token",
            "KOVA_GATEWAY_PASSWORD=attacker-password",
            "KOVA_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks KOVA_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.KOVA_STATE_DIR).toBeUndefined();
        expect(process.env.KOVA_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (KOVA_AGENT_DIR, KOVA_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, KOVA_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "KOVA_AGENT_DIR=./evil-agent",
            `KOVA_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "KOVA_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.KOVA_AGENT_DIR;
        delete process.env.KOVA_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.KOVA_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.KOVA_AGENT_DIR).toBeUndefined();
        expect(process.env.KOVA_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.KOVA_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks KOVA_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "KOVA_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.KOVA_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.KOVA_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "KOVA_PINNED_PYTHON=./attacker-python",
            "KOVA_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.KOVA_PINNED_PYTHON;
        delete process.env.KOVA_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.KOVA_PINNED_PYTHON).toBeUndefined();
        expect(process.env.KOVA_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "KOVA_PINNED_PYTHON=/trusted/python",
            "KOVA_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.KOVA_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.KOVA_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "KOVA_LIVE_GEMINI_KEY=sk-gemini-live",
            "KOVA_LIVE_OPENAI_KEY=sk-openai-live",
            "KOVA_GATEWAY_TOKEN=trusted-token",
            "KOVA_GATEWAY_PASSWORD=trusted-password",
            "KOVA_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.KOVA_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.KOVA_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.KOVA_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.KOVA_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.KOVA_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.KOVA_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.KOVA_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "KOVA_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("loads the gateway.env compatibility fallback during CLI startup", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        process.env.KOVA_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(path.join(base, ".config", "kova", "gateway.env"), "BAR=from-gateway\n");
  ...
  it("does not load gateway.env when KOVA_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.KOVA_STATE_DIR = customStateDir;
        await writeEnvFile(path.join(base, ".config", "kova", "gateway.env"), "FOO=from-gateway\n");
        ...
        expect(process.env.KOVA_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });

  it("keeps the legacy state-dir fallback for CLI dotenv loading only in explicit compatibility mode", async () => {
    await withIsolatedEnvAndCwd(async () => {
