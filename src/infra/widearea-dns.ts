...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.KOVA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*(?:kova|kova)-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "kova";
  ...
  records.push(`_kova-gw._tcp IN PTR ${instanceLabel}._kova-gw._tcp`);
  records.push(`${instanceLabel}._kova-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._kova-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; kova-content-hash: ${contentHash}\n${contentBody}`;
}
