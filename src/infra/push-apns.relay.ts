...
const GATEWAY_DEVICE_ID_HEADER = "x-kova-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-kova-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-kova-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.KOVA_APNS_RELAY_BASE_URL);
  const configBaseUrl = normalizeNonEmptyString(configuredRelay?.baseUrl);
  const baseUrl = envBaseUrl ?? configBaseUrl;
  const baseUrlSource = envBaseUrl ? "KOVA_APNS_RELAY_BASE_URL" : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or KOVA_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.KOVA_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error("http relay URLs require KOVA_APNS_RELAY_ALLOW_HTTP=true (development only)");
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(env.KOVA_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs),
      },
    };
  } catch (err) {
