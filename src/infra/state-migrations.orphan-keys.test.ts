...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".kova");
...
function sharedMainOpsConfig(sharedStorePath: string): KovaConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: KovaConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { KOVA_STATE_DIR: stateDir };
