...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kova-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".kova");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".kova", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      KOVA_STATE_DIR: path.join(home, ".kova"),
      ...env,
    };
