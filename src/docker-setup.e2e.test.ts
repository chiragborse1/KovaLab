...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "kova-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    KOVA_GATEWAY_TOKEN: "test-token",
    KOVA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    KOVA_WORKSPACE_DIR: join(sandbox.rootDir, "kova"),
    KOVA_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("kova-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    KOVA_CONFIG_DIR: configDir,
    KOVA_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KOVA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      KOVA_EXTRA_MOUNTS: undefined,
      KOVA_HOME_VOLUME: "kova-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("KOVA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("KOVA_EXTRA_MOUNTS=");
    expect(envFile).toContain("KOVA_HOME_VOLUME=kova-home"); // pragma: allowlist secret
    expect(envFile).toContain("KOVA_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `KOVA_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("kova-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/kova`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("kova-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg KOVA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node kova-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
      'run --rm --no-deps --entrypoint node kova-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]',
    );
    expect(log).not.toContain("run --rm kova-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KOVA_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("KOVA_DISABLE_BONJOUR=0");
  });

  it("avoids shared-network kova-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(prestartLines.some((line) => /\bcompose\b.*\brun\b.*\bkova-cli\b/.test(line))).toBe(
      false,
    );
  });
  ...
  it("writes KOVA_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KOVA_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("KOVA_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh kova-gateway -c");
  });
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KOVA_CONFIG_DIR: configDir,
      KOVA_WORKSPACE_DIR: workspaceDir,
      KOVA_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/kova -xdev");
  });

  it("reuses existing config token when KOVA_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("KOVA_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when KOVA_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "KOVA_GATEWAY_TOKEN=dotenv-token-123\nKOVA_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("KOVA_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "KOVA_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("KOVA_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("KOVA_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats KOVA_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("KOVA_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg KOVA_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg KOVA_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      KOVA_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker kova-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(log).toContain(
        "run --rm --no-deps kova-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate kova-gateway"));
  ...
  it("rejects injected multiline KOVA_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KOVA_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("KOVA_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid KOVA_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KOVA_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid KOVA_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      KOVA_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("KOVA_HOME_VOLUME must match");
  });

  it("rejects KOVA_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("KOVA_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/KOVA_DISABLE_BONJOUR: \$\{KOVA_DISABLE_BONJOUR:-\}/g)).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:kova-gateway"');
    expect(compose).toContain("depends_on:\n      - kova-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/KOVA_GATEWAY_TOKEN: \$\{KOVA_GATEWAY_TOKEN:-\}/g)).toHaveLength(2);
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        "${KOVA_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.kova-auth-profile-secrets}:/home/node/.config/kova",
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{KOVA_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
