...
`;
    ...
    expect(parsed.kova.emoji).toBe("💾");
    expect(parsed.kova.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.kova.emoji).toBe("📝");
    expect(parsed.kova.events).toEqual(["command"]);
    expect(parsed.kova.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.kova.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"kova": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"kova": {"events": ["test"]}}');
  });
...
describe("resolveKovaMetadata", () => {
  it("extracts kova metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveKovaMetadata(frontmatter);
  ...
  it("returns undefined when kova key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        kova: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Kova" },
            { id: "npm", kind: "npm", package: "@kovaai/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@kovaai/hook");
  });
...
`;
    ...
    const kova = resolveKovaMetadata(frontmatter);
    expect(kova).toBeDefined();
    expect(kova?.emoji).toBe("💾");
    expect(kova?.events).toEqual(["command:new", "command:reset"]);
    expect(kova?.requires?.config).toEqual(["workspace.dir"]);
    expect(kova?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const kova = resolveKovaMetadata(frontmatter);
    expect(kova?.emoji).toBe("disk");
    expect(kova?.events).toEqual(["command:new"]);
  });
});
