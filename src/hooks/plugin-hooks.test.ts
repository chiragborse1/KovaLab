...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "kova-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.KOVA_BUNDLED_HOOKS_DIR;
    process.env.KOVA_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.KOVA_BUNDLED_HOOKS_DIR;
    } else {
      process.env.KOVA_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".kova", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): KovaConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("kova-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Kova hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".kova", "extensions", "claude-bundle");
