...
export function hasConfiguredInternalHooks(config: KovaConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: KovaConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
