...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/kova-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = KovaConfig>(
  overrides: T = {} as T,
): KovaConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): KovaConfig {
