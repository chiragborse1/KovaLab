...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:kova security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:kova security audit --deep)");
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:kova devices approve req-123)",
      "muted(Fallback: cmd:kova devices approve --latest)",
      "muted(Inspect: cmd:kova devices list)",
    ]);
  });
