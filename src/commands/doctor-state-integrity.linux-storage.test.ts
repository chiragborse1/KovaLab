...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.kova", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.kova", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/kova-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/kova/.kova",
    });

    expect(result).toEqual({
      path: "/mnt/slow/kova/.kova",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".kova"), {
      platform: "darwin",
      mountInfo,
    });
