...
function listOrphanAgentDirs(cfg: KovaConfig, stateDir: string): OrphanAgentDir[] {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".kova"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for KOVA_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // KOVA_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: KovaConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.KOVA_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: KovaConfig, agentId: string): boolean {
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.kova).",
        `  Set locally: KOVA_STATE_DIR=~/.kova ${formatCliCommand("kova doctor")}`,
      ].join("\n"),
    );
  }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.kova out of git; it contains credentials and session history.",
