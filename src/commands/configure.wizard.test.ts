...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: KovaConfig }) =>
...
vi.mock("./onboard-helpers.js", () => ({
  ...
  applyWizardMetadata: (cfg: KovaConfig) => cfg,
  ensureWorkspaceAndSessions: vi.fn(),
  guardCancel: <T>(value: T) => value,
  printWizardHeader: mocks.printWizardHeader,
  probeGatewayReachable: mocks.probeGatewayReachable,
  resolveGatewayHttpLinks: mocks.resolveGatewayHttpLinks,
...
function setupBaseWizardState(config: KovaConfig = {}) {
  ...
  mocks.resolveGatewayHttpLinks.mockReturnValue({
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: KovaConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: KovaConfig) =>
