...
export function normalizeLegacyTalkConfig(cfg: KovaConfig, changes: string[]): KovaConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as KovaConfig["talk"]) ?? {};
