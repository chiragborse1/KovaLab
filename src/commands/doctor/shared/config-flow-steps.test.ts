...
describe("doctor config flow steps", () => {
  beforeEach(() => {
    ...
    migrateLegacyConfigMock.mockImplementation((config: KovaConfig) => ({
      config,
      changes: [],
    }));
  ...
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    ...
    expect(result.state.fixHints).toContain(
      'Run "kova doctor --fix" to migrate legacy config keys.',
    );
  ...
  it("removes unknown keys and adds preview hint", () => {
    ...
    const result = applyUnknownConfigKeyStep({
      state: {
        cfg: {},
        candidate: { bogus: true } as unknown as KovaConfig,
        pendingChanges: false,
        fixHints: [],
      },
      shouldRepair: false,
    ...
    expect(result.state.fixHints).toContain('Run "kova doctor --fix" to remove these keys.');
  });
});
