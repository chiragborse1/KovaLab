...
export function normalizeLegacyCommandsConfig(cfg: KovaConfig, changes: string[]): KovaConfig {
  ...
  return {
    ...cfg,
    commands: commands as KovaConfig["commands"],
  };
}

export function normalizeLegacyBrowserConfig(cfg: KovaConfig, changes: string[]): KovaConfig {
  ...
  return {
    ...cfg,
    browser: browser as KovaConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as KovaConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<KovaConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<KovaConfig["models"]>>;
type ModelDefinitionEntry = NonNullable<ModelProviderEntry["models"]>[number];
type AgentRuntimePolicyPatch = NonNullable<
  NonNullable<NonNullable<KovaConfig["agents"]>["defaults"]>["agentRuntime"]
>;
...
export function normalizeLegacyRuntimeModelRefs(cfg: KovaConfig, changes: string[]): KovaConfig {
  ...
  return changed
    ? {
        ...cfg,
        agents: nextAgents as KovaConfig["agents"],
      }
    : cfg;
}
...
export function normalizeLegacyOpenAICodexModelsAddMetadata(
  ...
  return {
    ...
    models: {
      ...rawModels,
      providers: nextProviders as NonNullable<KovaConfig["models"]>["providers"],
    },
  };
}
...
export function normalizeLegacyNanoBananaSkill(cfg: KovaConfig, changes: string[]): KovaConfig {
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<KovaConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as KovaConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<KovaConfig["tools"]>["media"],
    },
  };
}
