...
vi.mock("../channels/plugins/bundled.js", async () => {
  ...
  function resolveTelegramAccountId(cfg: KovaConfig): string {
  ...
  function detectTelegramAllowFromMigration(params: { cfg: KovaConfig; env: NodeJS.ProcessEnv }) {
    const root = params.env.KOVA_STATE_DIR;
    if (!root) {
      return [];
    }
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: KovaConfig; env: NodeJS.ProcessEnv }) =>
...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "kova-doctor-"));
  tempRoots.push(root);
  return root;
}
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: KovaConfig }) {
  const oauthDir = ensureCredentialsDir(params.root);
  writeLegacyTelegramAllowFromStore(oauthDir);
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { KOVA_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".kova"),
    legacyDir: path.join(root, ".kova"),
  };
}
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      cfg,
      env: { KOVA_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: KovaConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
  ...
  it("leaves the current state dir alone when no legacy state dir names are supported", async () => {
    const root = await makeTempRoot();
    const { targetDir } = getStateDirMigrationPaths(root);
    fs.mkdirSync(path.join(targetDir, "sessions"), { recursive: true });
    fs.writeFileSync(path.join(targetDir, "sessions.json"), "{}", "utf-8");

    expectUnmigratedWithoutWarnings(await runFreshStateDirMigration(root));
  });
});
