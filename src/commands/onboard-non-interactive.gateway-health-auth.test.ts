...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("does not fall back to stale KOVA_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.KOVA_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.KOVA_GATEWAY_TOKEN = "stale-env-token";
    process.env.KOVA_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
