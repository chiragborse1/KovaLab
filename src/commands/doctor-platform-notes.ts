...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".kova", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: KovaConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["KOVA_GATEWAY_TOKEN", await getenv("KOVA_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["KOVA_GATEWAY_PASSWORD", await getenv("KOVA_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "KOVA_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn =
    normalizeOptionalString(env.KOVA_NO_RESPAWN) ??
    normalizeOptionalString(env.KOVA_NO_RESPAWN) ??
    "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- KOVA_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
