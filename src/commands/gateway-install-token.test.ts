...
const secretRefKeyMock = vi.hoisted(() => vi.fn(() => "env:default:KOVA_GATEWAY_TOKEN"));
...
describe("resolveGatewayInstallToken", () => {
  ...
  it("validates SecretRef token but does not persist resolved plaintext", async () => {
    const tokenRef = { source: "env", provider: "default", id: "KOVA_GATEWAY_TOKEN" };
    resolveSecretInputRefMock.mockReturnValue({ ref: tokenRef });
    resolveSecretRefValuesMock.mockResolvedValue(
      new Map([["env:default:KOVA_GATEWAY_TOKEN", "resolved-token"]]),
    );
    ...
    const result = await resolveGatewayInstallToken({
      ...
      env: { KOVA_GATEWAY_TOKEN: "resolved-token" } as NodeJS.ProcessEnv,
    });
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("kova config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("kova config set gateway.auth.mode password");
  ...
  it("passes the install env through to gateway auth resolution", async () => {
    const env = {
      KOVA_GATEWAY_PASSWORD: "dotenv-password", // pragma: allowlist secret
