...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-doctor-cron-"));
...
function createCronConfig(storePath: string): KovaConfig {
  return {
    cron: {
      store: storePath,
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("rewrites stale managed dreaming jobs to the isolated agentTurn shape", async () => {
    ...
    expect(job).toMatchObject({
      ...
      payload: {
        kind: "agentTurn",
        message: "__kova_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
      delivery: { mode: "none" },
    });
