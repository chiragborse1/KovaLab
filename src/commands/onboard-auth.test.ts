...
vi.mock("../agents/agent-paths.js", () => ({
  resolveKovaAgentDir: () => process.env.KOVA_AGENT_DIR ?? "/tmp/kova-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.KOVA_STATE_DIR ?? "/tmp/kova-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.KOVA_AGENT_DIR ?? "/tmp/kova-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under KOVA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("kova-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-oauth-sync-"));
    ...
    process.env.KOVA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-oauth-nosync-"));
    process.env.KOVA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside KOVA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-oauth-external-"));
    process.env.KOVA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* KOVA_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("kova-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("kova-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to KOVA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("kova-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
