...
beforeAll(() => {
  envSnapshot = captureEnv(["KOVA_PROFILE"]);
  process.env.KOVA_PROFILE = "isolated";
});
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/kova/pnpm-lock.yaml",
        markerPath: "/tmp/kova/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.KOVA_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.kova.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.kova.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.KOVA_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/kova-root.js", () => ({
  resolveKovaPackageRoot: vi.fn().mockResolvedValue("/tmp/kova"),
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("KOVA_GATEWAY_TOKEN", "abcd1234", async () => {
