...
const AGENT_CLI_SIGNALS = ["SIGINT", "SIGTERM"] as const;
type AgentCliSignal = (typeof AGENT_CLI_SIGNALS)[number];
...
function parseTimeoutSeconds(opts: { cfg: KovaConfig; timeout?: string }) {
  const raw =
...
function isGatewayAgentEmbeddedFallbackError(err: unknown): boolean {
  ...
  return err instanceof Error && err.message.includes("gateway not connected");
}
...
function readAcceptedRunContext(payload: unknown): {
  ...
  const record = payload as { runId?: unknown; sessionKey?: unknown; status?: unknown };
...
function createGatewayTimeoutFallbackSession(agentId?: string): {
  ...
  const sessionId = `${GATEWAY_TIMEOUT_FALLBACK_SESSION_PREFIX}${randomUUID()}`;
  return {
    sessionId,
...
export async function agentCliCommand(
  ...
  protectJsonStdout(opts);
  validateExplicitSessionKeyForDispatch(opts);
  const normalizedOpts = normalizeSessionKeyOptsForDispatch(opts);
  const gatewayDispatchOpts = normalizedOpts.runId
    ? normalizedOpts
    : { ...normalizedOpts, runId: randomIdempotencyKey() };
  ...
  try {
    ...
    try {
      ...
      if (isGatewayAgentTimeoutError(err)) {
        if (isControlCommandThatMustNotFallback(gatewayDispatchOpts)) {
          throw err;
        }
        const fallbackAgentId = await resolveAgentIdForGatewayTimeoutFallback(gatewayDispatchOpts);
