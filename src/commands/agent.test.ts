...
import { getCachedModelCatalog, loadModelCatalog } from "../agents/model-catalog.js";
...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".kova", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "kova-agent-", skipSessionCleanup: true });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<KovaConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<KovaConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  vi.mocked(getCachedModelCatalog).mockReturnValue(undefined);
  vi.mocked(loadModelCatalog).mockResolvedValue([]);
  vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(() => false);
  configIoMocks.readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as KovaConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand", () => {
  it("enables the Codex runtime plugin for one-shot OpenAI model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      expect(pluginRegistryMocks.ensurePluginRegistryLoaded).toHaveBeenCalledWith({
        ...
        workspaceDir: path.join(home, "kova"),
        onlyPluginIds: ["codex"],
      });
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly KOVA-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
  ...
  it("uses default fallback list for session model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(getCachedModelCatalog).mockReturnValueOnce([
