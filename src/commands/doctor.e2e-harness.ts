...
export const resolveKovaPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadKovaPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("getkova/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("getkova/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("getkova/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/kova-root.js", () => ({
  ...
  resolveKovaPackageRootSync: vi.fn(() => "/tmp/kova"),
}));
...
beforeEach(() => {
  ...
  resolveKovaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.KOVA_STATE_DIR;
  originalUpdateInProgress = process.env.KOVA_UPDATE_IN_PROGRESS;
  process.env.KOVA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-doctor-state-"));
