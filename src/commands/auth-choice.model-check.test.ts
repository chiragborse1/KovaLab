...
describe("warnIfModelConfigLooksOff", () => {
  ...
  it("keeps custom OpenAI-compatible provider auth separate from Codex OAuth profiles", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      'No auth configured for provider "openai". The agent may fail until credentials are added. Run `kova models auth login --provider openai`, `kova configure`, or set an API key env var.',
      "Model check",
    );
  });
