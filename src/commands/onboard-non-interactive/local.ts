...
export async function runNonInteractiveLocalSetup(params: {
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("kova gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("kova configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.neuralstudio.in/tools/web`,
    );
  }
}
