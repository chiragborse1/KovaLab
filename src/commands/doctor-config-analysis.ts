...
import { KovaSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: KovaConfig): {
  config: KovaConfig;
  removed: string[];
} {
  const parsed = KovaSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: KovaConfig): void {
