...
const TEMP_MEDIA_PREFIX = "kova-media-";
...
function createGroqAudioConfig(): KovaConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KovaConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KovaConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KovaConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredKovaTmpDir();
