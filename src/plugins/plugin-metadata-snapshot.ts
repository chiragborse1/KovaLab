...
const MEMO_RELEVANT_ENV_KEYS = [
  ...
  "KOVA_COMPATIBILITY_HOST_VERSION",
  "KOVA_CONFIG_PATH",
  "KOVA_DISABLE_BUNDLED_PLUGINS",
  "KOVA_DISABLE_BUNDLED_SOURCE_OVERLAYS",
  "KOVA_DISABLE_PERSISTED_PLUGIN_REGISTRY",
  "KOVA_HOME",
  "KOVA_NIX_MODE",
  "KOVA_STATE_DIR",
  "USERPROFILE",
...
function freezeOwnerMap(owners: Map<string, string[]>): ReadonlyMap<string, readonly string[]> {
  return new Map(
    [...owners.entries()].map(([ownedId, pluginIds]) => [
      ownedId,
      Object.freeze([...pluginIds].toSorted((left, right) => left.localeCompare(right))),
    ]),
  );
}
...
export function loadPluginMetadataSnapshot(
  ...
  if (pluginMetadataSnapshotMemo?.key === memoKey) {
    return pluginMetadataSnapshotMemo.snapshot;
  }

  const result = loadPluginMetadataSnapshotImpl(params);
  if (result.snapshot.index.plugins.length > 0 || result.registrySource !== "derived") {
