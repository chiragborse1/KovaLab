...
const fixtureRoot = makeTrackedTempDir("kova-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "kova.plugin.json");
...
async function loadPlugins(pluginPaths: string[], warnings?: string[]) {
  const { loadKovaPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("kova plugins list");
  });
});
