...
function makeTempDir() {
  return makeTrackedTempDir("kova-plugin-registry", tempDirs);
}
...
function createIndex(
  ...
  return {
    ...
    plugins: [
      {
        pluginId,
        manifestPath: path.join(pluginRoot, "kova.plugin.json"),
...
describe("plugin registry facade", () => {
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "kova.plugin.json"));
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "kova.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "kova.plugin.json"),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
    ...
    await writePersistedInstalledPluginIndex(
      createIndex("persisted", {
        plugins: [
          {
            ...createIndex("persisted").plugins[0],
            manifestPath: path.join(staleBundledRootDir, "kova.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ KOVA_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("falls back to the derived registry when persisted bundled records became external packages", async () => {
    ...
    fs.writeFileSync(
      path.join(staleBundledRootDir, "kova.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ KOVA_BUNDLED_PLUGINS_DIR: staleBundledRootDir }),
    });
