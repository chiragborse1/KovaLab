...
export type InstallKovaPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["getkova/plugin-sdk", "@getkova/plugin-sdk"] as const;
const pluginSdkNativeAliases = new Map<string, NativeAliasEntry[]>();
let installed = false;
let previousResolveFilename: ResolveFilename | undefined;

function resolveLoaderModulePath(options: InstallKovaPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "getkova" ||
          packageJson.name === "kova" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { kova?: unknown }).kova === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(options: InstallKovaPluginSdkNativeResolverOptions): string[] {
...
function listPluginSdkNativeAliases(
  options: InstallKovaPluginSdkNativeResolverOptions,
...
export function installKovaPluginSdkNativeResolver(
  options: InstallKovaPluginSdkNativeResolverOptions = {},
): string[] {
...
export function resetKovaPluginSdkNativeResolverForTest(): void {
