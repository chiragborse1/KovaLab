...
import { clearPluginDiscoveryCache, discoverKovaPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("kova-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    KOVA_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverKovaPlugins>[0],
) {
  return discoverKovaPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverKovaPlugins>[0]) {
  return discoverKovaPlugins(params);
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "kova.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverKovaPlugins>>,
...
describe("discoverKovaPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".kova", "extensions");
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@kovaai/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@kovaai/stray-workspace-plugin",
    });

    const result = discoverKovaPlugins({
      workspaceDir,
  ...
  it("ignores hidden directories under extension roots", () => {
    ...
    const globalExt = path.join(stateDir, "extensions");
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, ".kova-install-backups"),
      packageName: "@kovaai/backup-plugin",
      pluginId: "backup-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "visible-plugin"),
      packageName: "@kovaai/visible-plugin",
      pluginId: "visible-plugin",
    });

    const result = discoverKovaPlugins({ env: buildDiscoveryEnv(stateDir) });

    expectCandidatePresence(result, {
      present: ["visible-plugin"],
      absent: ["backup-plugin", ".kova-install-backups"],
    });
  });
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".kova", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    ...
    const { candidates, diagnostics } = discoverKovaPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
  ...
  it("ignores packaged bundled plugin paths in configured load paths", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "node_modules", "kova");
    ...
    const { candidates, diagnostics } = discoverKovaPlugins({
      ...
      env: {
        ...buildDiscoveryEnv(stateDir),
        KOVA_BUNDLED_PLUGINS_DIR: bundledRoot,
      },
    });
  ...
  it("discovers bind-mounted bundled source overlays before packaged dist bundles", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@kovaai/synology-chat",
  ...
  it("uses explicit runtime extension entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@kovaai/runtime-pack",
  ...
  it("rejects package runtimeExtensions that do not match extension entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@kovaai/runtime-mismatch-pack",
      extensions: ["./src/one.ts", "./src/two.ts"],
  ...
  it("infers built dist entries for installed TypeScript package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@kovaai/built-peer-pack",
      extensions: ["src/index.ts"],
      setupEntry: "src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
  ...
  it("preserves nested entry paths when inferring installed dist entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@kovaai/nested-pack",
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".kova", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));

    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@kovaai/workspace-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));

    const { candidates } = discoverKovaPlugins({
      workspaceDir,
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-kova");
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@opik/opik-kova",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-kova" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "kova"),
      packageName: "kova",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-kova"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".kova", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "kova");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@kovaai/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@kovaai/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@kovaai/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("kova");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@kovaai/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@kovaai/missing-entry-pack",
  ...
  it("reuses bundled and global discovery across workspace-specific cache misses", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@kovaai/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@kovaai/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".kova", "extensions", "workspace-a-plugin"),
      packageName: "@kovaai/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".kova", "extensions", "workspace-b-plugin"),
      packageName: "@kovaai/workspace-b-plugin",
