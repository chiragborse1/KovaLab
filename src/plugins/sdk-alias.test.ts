...
const fixtureRoot = makeTrackedTempDir("kova-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "kova.mjs"), "export {};\n", "utf-8");
  }
...
function createExtensionApiAliasFixture(params?: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "kova", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "kova.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".kova", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["getkova/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["getkova/plugin-sdk/channel-runtime"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/kova-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/kova-loader.js",
      argv1: (root: string) => path.join(root, "kova.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ KOVA_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ KOVA_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an Kova root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("aliases bundled plugin public surfaces for source and dist plugin loads", () => {
    const fixture = createPluginSdkAliasFixture({
      packageExports: {
        "./plugin-sdk/core": { default: "./dist/plugin-sdk/core.js" },
      },
    });
    const matrixRoot = path.join(fixture.root, "extensions", "matrix");
    const matrixDistRoot = path.join(fixture.root, "dist", "extensions", "matrix");
    mkdirSafeDir(matrixRoot);
    mkdirSafeDir(matrixDistRoot);
    fs.writeFileSync(
      path.join(matrixRoot, "package.json"),
      JSON.stringify({ name: "@kovaai/matrix", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(path.join(matrixRoot, "api.ts"), "export const sourceApi = true;\n", "utf-8");
    fs.writeFileSync(
      path.join(matrixRoot, "test-api.ts"),
      "export const sourceTestApi = true;\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(matrixDistRoot, "api.js"),
      "export const distApi = true;\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(matrixDistRoot, "test-api.js"),
      "export const distTestApi = true;\n",
      "utf-8",
    );
    const sourcePluginEntry = writePluginEntry(
      fixture.root,
    ...
    const sourceAliases = withEnv(
      { KOVA_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    const privateSourceAliases = withEnv(
      { KOVA_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(sourceAliases["@kovaai/matrix/api.js"] ?? "")).toBe(
      fs.realpathSync(path.join(matrixRoot, "api.ts")),
    );
    expect(sourceAliases["@kovaai/matrix/test-api.js"]).toBeUndefined();
    expect(fs.realpathSync(privateSourceAliases["@kovaai/matrix/test-api.js"] ?? "")).toBe(
      fs.realpathSync(path.join(matrixRoot, "test-api.ts")),
    );
    expect(fs.realpathSync(distAliases["@kovaai/matrix/api.js"] ?? "")).toBe(
      fs.realpathSync(path.join(matrixDistRoot, "api.js")),
    );
  });
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ KOVA_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["getkova/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["getkova/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["getkova/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running kova argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "kova.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the kova root
    // (e.g. single-binary distributions or custom process launchers).
    // Use kova.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real kova repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "kova.mjs")).href;
    ...
    // runner binary, inside the kova repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedKovaRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an Kova root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("scopes plugin loader Jiti config by plugin-sdk resolution", () => {
    ...
    const { auto, dist, distAgain } = withEnv({ NODE_ENV: undefined }, () => ({
      auto: resolvePluginLoaderJitiConfig({
        ...
        argv1: path.join(fixture.root, "kova.mjs"),
    ...
    expect(fs.realpathSync(auto.aliasMap["getkova/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["getkova/plugin-sdk"] ?? "")).toBe(
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "@getkova/plugin-sdk/infra-runtime";
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "getkova/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
        "@getkova/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "kova-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("prefers startup argv runtime candidates over cache ancestor fallbacks", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "kova-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "getkova");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "kova");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "kova-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["getkova/plugin-sdk"]).not.toBe(aliasB["getkova/plugin-sdk"]);
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ KOVA_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ KOVA_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["getkova/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["getkova/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["getkova/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["getkova/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("pre-normalizes and marks alias maps for Jiti", () => {
    ...
    const aliasMap = {
      "getkova/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "getkova/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@getkova/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
