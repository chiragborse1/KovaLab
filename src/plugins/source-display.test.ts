...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "kova", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".kova", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".kova", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "kova-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".kova", "extensions"),
      },
    });
  });
});
