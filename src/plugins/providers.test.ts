...
type LoadKovaPlugins = typeof import("./loader.js").loadKovaPlugins;
...
const loadKovaPluginsMock = vi.fn<LoadKovaPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/kova.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadKovaPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: KovaConfig = {
    plugins: {},
  };
  const autoEnabledConfig: KovaConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadKovaPlugins: (...args: Parameters<LoadKovaPlugins>) => loadKovaPluginsMock(...args),
  ...
  it("uses current metadata owner maps for model catalog aliases", () => {
    const plugins = [
      createManifestProviderPlugin({
        id: "openai",
        providerIds: ["openai"],
  ...
  beforeEach(() => {
    ...
    loadKovaPluginsMock.mockReturnValue(registry);
