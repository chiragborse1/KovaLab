...
const originalBundledDir = process.env.KOVA_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.KOVA_DISABLE_BUNDLED_PLUGINS;
...
function createKovaRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "kova" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@kovaai/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.KOVA_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.KOVA_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.KOVA_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.KOVA_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.KOVA_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.KOVA_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "kova-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "kova-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "kova-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    const repoRoot = createKovaRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createKovaRoot({
      prefix: "kova-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createKovaRoot({
      prefix: "kova-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
    ...
    process.env.KOVA_DISABLE_BUNDLED_PLUGINS = "1";
    delete process.env.KOVA_BUNDLED_PLUGINS_DIR;
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createKovaRoot({
          prefix: "kova-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createKovaRoot({
          prefix: "kova-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "kova.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createKovaRoot({
          prefix: "kova-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
