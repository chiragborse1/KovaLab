...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".kova", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "KOVA_HOME", "KOVA_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("kova-bundle-commands-home-");
      const workspaceDir = await createTempDir("kova-bundle-commands-workspace-");
