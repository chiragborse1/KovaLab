...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadKovaPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadKovaPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "kova-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.KOVA_BUNDLED_PLUGINS_DIR;
const prevPluginStageDir = process.env.KOVA_PLUGIN_STAGE_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "kova.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".kova", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ KOVA_STATE_DIR: stateDir, ...params.env }, () =>
    loadKovaPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.KOVA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevPluginStageDir === undefined) {
    delete process.env.KOVA_PLUGIN_STAGE_DIR;
  } else {
    process.env.KOVA_PLUGIN_STAGE_DIR = prevPluginStageDir;
  }
}
