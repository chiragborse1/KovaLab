...
const DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY = "group:plugins";
...
function denylistBlocksPlugin(params: {
  ...
  return (
    ...
    matchesAnyGlobPattern(DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY, params.denylist)
  );
}
...
function isOptionalToolAllowed(params: {
  ...
  return params.allowlist.has(DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY);
}
...
function isOptionalToolEntryPotentiallyAllowed(params: {
  ...
  const pluginKey = normalizeToolName(params.pluginId);
  if (
    params.allowlist.has(pluginKey) ||
    params.allowlist.has(DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY)
  ) {
    return true;
  }
...
function listManifestToolNamesForAllowlist(params: {
  ...
  if (params.allowlist.has("*") || params.allowlist.has(DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY)) {
...
function resolvePluginToolRuntimePluginIds(params: {
  ...
  if (normalizedPlugins.loadPaths.length > 0) {
    return undefined;
  }
  const snapshot =
    params.snapshot ??
...
function createCachedDescriptorPluginTool(params: {
  ...
  const pluginId = descriptor.owner.pluginId;
...
export function resolvePluginTools(params: {
  ...
  const cached = onlyPluginIds
    ? resolveCachedPluginTools({
        snapshot,
        config: context.config,
        availabilityConfig: params.context.runtimeConfig ?? context.config,
        env,
        allowlist,
        denylist,
        hasAuthForProvider: params.hasAuthForProvider ?? params.context.hasAuthForProvider,
        onlyPluginIds,
        existing,
        existingNormalized,
        ctx: params.context,
    ...
    : { tools: [], handledPluginIds: new Set<string>() };
  ...
  const loadOptions = buildPluginRuntimeLoadOptions(context, {
    activate: false,
    ...(runtimePluginIds === undefined ? {} : { onlyPluginIds: runtimePluginIds }),
    runtimeOptions,
  });
  ...
  if (!registry) {
    try {
      ...
      context.logger.error(
        `failed to cold-load plugin tool registry for plugin ids [${
          runtimePluginIds?.join(", ") ?? "*"
    ...
    if (!registry) {
      context.logger.warn(
        `plugin tool registry still unavailable after cold load for plugin ids [${
          runtimePluginIds?.join(", ") ?? "*"
        }]`,
      );
      return tools;
  ...
  const scopedPluginIds = runtimePluginIds === undefined ? undefined : new Set(runtimePluginIds);
  const registryToolPluginIds = new Set(registry.tools.map((entry) => entry.pluginId));
  const missingRegistryToolPluginIds =
    runtimePluginIds?.filter((pluginId) => !registryToolPluginIds.has(pluginId)) ?? [];
  ...
  for (const entry of registry.tools) {
    if (scopedPluginIds && !scopedPluginIds.has(entry.pluginId)) {
      continue;
    }
    ...
    const availabilityNames =
      declaredNames.length > 0
        ? declaredNames
        : ((entry as { declaredNames?: string[] }).declaredNames ?? []);
    ...
    for (const toolRaw of list) {
      ...
      const entryDeclaredNames = (entry as { declaredNames?: string[] }).declaredNames;
      const undeclared = entryDeclaredNames
        ? findUndeclaredPluginToolNames({
            declaredNames: entryDeclaredNames,
            toolNames: [tool.name],
          })
        : [];
