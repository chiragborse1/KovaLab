...
`kova acp` exposes an ACP agent over stdio and forwards prompts to a running
...
3. Point the IDE to run `kova acp` over stdio.
...
- ACP client spawns `kova acp` and speaks ACP messages over stdio.
...
`kova acp` resolves the Gateway URL and auth from CLI flags or config:
