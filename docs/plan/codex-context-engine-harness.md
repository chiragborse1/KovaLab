...
summary: "Specification for making the bundled Codex app-server harness honor Kova context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Kova context-engine
...
Kova-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Kova-controlled code. The external
boundary is the Codex app-server protocol itself: Kova can control what it
...
For Codex harness turns, Kova should preserve this lifecycle:

1. Read the mirrored Kova session transcript.
...
8. Mirror the Codex result back into the Kova transcript.
...
11. Preserve Codex native compaction signals and Kova compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Kova should
not try to mutate the app-server's internal history except through supported
protocol calls.

Kova's transcript mirror remains the source for Kova features:
...
The context-engine interface returns Kova `AgentMessage[]`, not a Codex
...
- Label it clearly as Kova assembled context.
...
```text
Kova assembled context for this turn:
...
inside Kova and preserves context-engine semantics.
...
1. Kova context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Kova compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Kova compaction should prefer the context engine's `compact()` result for
the Kova transcript mirror and plugin state.
...
the Kova session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Kova session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Kova compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Kova core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Kova/plugin state.
