...
| Messaging Gateway          | One background Gateway runs many platforms, sessions, cron, slash commands, and platform operations                                   | Gateway runs channels, routing, pairing, cron, local chat/Gateway clients compatibility, and multi-agent routing across a wide channel set  | Strong but fragmented                       | Add one channel capability matrix with text, media, files, reactions, threads, typing, streaming, voice, and operational status        |
...
| API server                 | OpenAI-compatible server and API stream surfaces                                                                                      | OpenAI-compatible `/v1/*`, `/tools/invoke`, Gateway HTTP/WS, plus removed browser surfaces                                                  | Strong                                      | Keep disabled-by-default security posture clear and surface endpoint state in terminal operator commands                               |
...
| UI coherence               | TUI plus docs present one integrated path                                                                                             | TUI, docs, CLI settings/status/logs, mobile/app surfaces, plus removed browser surfaces                                                     | Main gap                                    | Make CLI/TUI the complete path before adding any new browser panels                                                                    |
...
Kova's vision becomes legible again. Treat the Gateway clients as legacy
