...
Use `Package Acceptance` when the question is "does this installable Kova
...
1. `resolve_package` checks out `workflow_ref`, resolves one package candidate,
   writes `.artifacts/docker-e2e-package/kova-current.tgz`, writes
...
2. `docker_acceptance` calls
   `kova-live-and-e2e-checks-reusable.yml` with `ref=workflow_ref` and
...
- `source=ref`: packs a trusted `package_ref` branch, tag, or full commit SHA.
  ...
  detached worktree, and packs it with `scripts/package-kova-for-docker.mjs`.
...
gh workflow run package-acceptance.yml \
  ...
  -f workflow_ref=main \
  -f source=url \
  -f package_url=https://example.com/kova-current.tgz \
...
`e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `Kova Release Checks` also
...
| `blacksmith-6vcpu-macos-latest`  | `macos-node` on `chiragborse1/KovaLab`; forks fall back to `macos-latest`                                                                                                                                                                                                                                                                                                                                                                                               |
| `blacksmith-12vcpu-macos-latest` | `macos-swift` on `chiragborse1/KovaLab`; forks fall back to `macos-latest`                                                                                                                                                                                                                                                                                                                                                                                              |
