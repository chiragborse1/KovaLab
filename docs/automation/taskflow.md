...
read_when:
  ...
  - You encounter Task Flow or kova tasks flow in release notes or docs
...
- Browser availability and profile choice, for example `kova` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
...
For reusable team or community workflows, package the CLI, `.lobster` files, and any setup notes as a skill or plugin and distribute it as an npm package, local bundle, or archive. Keep workflow-specific guardrails in that package unless the plugin API is missing a needed generic capability.
...
`kova tasks flow cancel` sets a sticky cancel intent on the flow. Active tasks within the flow are cancelled, and no new steps are started. The cancel intent persists across restarts, so a cancelled flow stays cancelled even if the gateway restarts before all child tasks have terminated.
...
| Command                       | Description                                   |
| ----------------------------- | --------------------------------------------- |
| `kova tasks flow list`        | Shows tracked flows with status and sync mode |
...
- [CLI: tasks](/cli/tasks) — CLI command reference for `kova tasks flow`
