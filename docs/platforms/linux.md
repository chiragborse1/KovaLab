...
2. `npm i -g getkova@latest`
3. `kova onboard --install-daemon`
...
service for shared or always-on servers. `kova gateway install` and
`kova onboard --install-daemon` already render the current canonical unit
...
sessions and channel connections. Kova therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Kova starts the child through a short
