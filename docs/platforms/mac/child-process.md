...
service via the external `kova` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.kova.gateway`
  (or `ai.kova.<profile>` when using `--profile`/`KOVA_PROFILE`; legacy `com.kova.*` is supported).
