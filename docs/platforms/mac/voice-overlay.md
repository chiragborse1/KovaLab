...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `ai.kova`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.kova" AND category CONTAINS "voicewake"' --level info --style compact
  ```
