...
`kova node install ...` and `kova node start` (or run
`kova node run ...` in the foreground), then target that browser-capable
node.

## Prereqs on the remote host

1. Install Node + pnpm and build/install the Kova CLI (`pnpm install && pnpm build && pnpm link --global`).
2. Ensure `kova` is on PATH for non-interactive shells (symlink into `/usr/local/bin` or `/opt/homebrew/bin` if needed).
...
- Run `kova channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
...
Pick sounds per notification from scripts with `kova` and `node.invoke`, e.g.:

```bash
kova nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
