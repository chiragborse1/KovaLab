...
<AccordionGroup>
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Kova hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Kova config agents from `agents_list`.
  </Accordion>
</AccordionGroup>
...
- **Run timeout:** if `sessions_spawn.runTimeoutSeconds` is omitted, Kova uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
<Steps>
  ...
  <Step title="Bind">
    Kova creates or binds a thread to that session target in the active channel.
  </Step>
...
target agent first. After that, Kova applies the sub-agent restriction
layer.
...
Kova does not treat `endedAt` absence as permanent proof that a
...
- Sub-agent context only injects `AGENTS.md` + `TOOLS.md` (no `SOUL.md`, `IDENTITY.md`, `USER.md`, `PULSE.md`, legacy `HEARTBEAT.md`, or `BOOTSTRAP.md`).
