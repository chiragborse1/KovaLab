...
If no `provider` is set, Kova checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Kova resolves only the
...
- or omit that field and let Kova auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `kova onboard` or
`kova configure --section web`, Kova can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `kova doctor --fix`.
When you choose Grok during `kova onboard` or `kova configure --section web`,
...
<Tabs>
  <Tab title="Config file">
    Run `kova configure --section web` or set the key directly:
