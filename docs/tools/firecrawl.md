...
- Choosing Firecrawl in onboarding or `kova configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `kova doctor --fix`.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Kova
