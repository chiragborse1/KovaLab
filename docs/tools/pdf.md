...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  Kova can actually authenticate that provider for the agent.
...
- `file://` URL
- `http://` and `https://` URL
- Kova-managed inbound refs such as `media://inbound/<id>`
...
- Managed inbound refs and replayed paths under Kova's inbound media store are allowed with workspace-only file policy.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, Kova drops the rendered images and continues with the
  extracted text.
