...
read_when:
  ...
  - Enabling the plugin-tools or Kova-tools MCP bridge
...
normalized by Kova before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Kova/acpx cannot force a model selection.
...
```bash
kova plugins install acpx
kova config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
kova plugins install ./path/to/local/acpx-plugin
```
...
- `command` accepts an absolute path, relative path (resolved from the Kova workspace), or command name.
...
When you install Kova globally with `npm install -g kova`, the acpx
...
normally and reports the missing dependency through `kova acp doctor`.
...
```bash
kova config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```
...
### Kova tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Kova tools through
...
```bash
kova config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
```bash
kova config set plugins.entries.acpx.config.probeAgent claude
```
...
```bash
kova config set plugins.entries.acpx.config.permissionMode approve-all
kova config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Kova defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
