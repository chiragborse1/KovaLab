---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, Kova ACP, OpenCode) through the ACP backend"
read_when:
...
let Kova run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, Kova ACP, OpenCode, Gemini CLI, and other
...
directly to existing Kova channel conversations, use
[`kova mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
by default with a plugin-local pinned `acpx` binary that Kova probes
and self-repairs on startup. Run `/acp doctor` for a readiness check.

Kova only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Kova owns routing,
...
Kova plugin tools and built-in Kova tools are **not** exposed to
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Kova session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from Kova's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Kova dynamic tools still execute through Kova, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, Kova injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through Kova approvals. Codex `Stop` hooks are relayed to
    Kova `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Kova picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    Kova config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an Kova agent
    is configured with `runtime.type="acp"`, Kova uses
...
plugin is enabled. Use **sub-agents** when you want Kova-native
delegated runs.
...
1. Kova ACP session control plane.
...
spawned ACP session — no child thread, same chat surface. Kova keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; Kova returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnAcpSessions` is only required when Kova needs to create a child thread for `--thread auto|here` — not for `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, Kova inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations — `/acp ...` commands are handled by Kova even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.
  </Accordion>
  <Accordion title="Thread-bound sessions">
    ...
    adapter does not support thread bindings, Kova returns a clear
...
- For cross-agent ACP spawns without an explicit `cwd`, Kova inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, Kova uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Kova may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize Kova Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise Kova/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What Kova sends to the harness:
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Kova converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Kova-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, Kova uses an agent-to-agent (A2A) follow-up path
    ...
    Kova skips the A2A follow-up only when the requester is the
...
- Kova's sandbox policy does **not** wrap ACP harness execution.
- Kova still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Kova-native work.
  </Warning>
...
If no target resolves, Kova returns a clear error
...
aliases), the plugin-tools and Kova-tools MCP bridges, and ACP
