...
read_when:
  ...
  - Debugging why kova is interfering with your own Chrome
...
- The `kova` profile does **not** touch your personal browser profile.
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`kova`, `work`, `remote`, ...).
...
kova browser --browser-profile kova doctor --deep
kova browser --browser-profile kova status
...
If `kova browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `kova browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `kova browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `kova` vs `user`

- `kova`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `kova` browser.
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Kova pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Kova-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
...
- `POST /start?headless=true` and `kova browser start --headless` request a
  ...
  remote CDP profiles reject the override because Kova does not launch those
  browser processes.
...
- `color` (top-level and per-profile) tints the browser surface so you can see which profile is active.
- Default profile is `kova` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
Per-profile `executablePath` only affects local managed profiles that Kova
...
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Kova-managed browser profile.
- `headless` only affects local managed profiles that Kova launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `kova browser stop` stops the browser process that
  Kova launched
- attach-only and remote CDP profiles: `kova browser stop` closes the active
  ...
  though no browser process was launched by Kova
...
When Browserless is self-hosted in Docker and Kova runs on the host, treat
...
Kova process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Kova WebSocket base, such
...
an address Kova cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Kova treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Kova.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Kova accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** — `http://host[:port]` or `https://host[:port]`.
  Kova calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** — `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Kova connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** — `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Kova tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Kova
  ...
  accepts it, Kova falls back to that root as well. This lets a bare `ws://`
...
- If browser control is enabled and no shared-secret auth is configured, Kova
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Kova does **not** auto-generate that token when `gateway.auth.mode` is
...
- The `kova` profile is auto-created if missing.
...
```bash
kova browser --browser-profile user start
kova browser --browser-profile user status
kova browser --browser-profile user tabs
kova browser --browser-profile user snapshot --format ai
```
...
- `kova doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `kova` profile because it can
  act inside your signed-in browser session.
- Kova does not launch the browser for this driver; it only attaches.
...
When `cdpUrl` is set on an existing-session profile, Kova skips
...
Compared to the managed `kova` profile, existing-session drivers are more constrained:
...
control API** plus a matching `kova browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Kova cannot confirm that the browser control plane is healthy.
...
- For the local loopback `kova` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Kova's own local control plane.
