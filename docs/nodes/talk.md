...
- macOS local MLX playback uses the bundled `kova-mlx-tts` helper when present, or an executable on `PATH`. Set `KOVA_MLX_TTS_BIN` to point at a custom helper binary during development.
