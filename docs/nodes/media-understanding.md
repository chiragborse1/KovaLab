...
Vendor-specific media behavior is registered by vendor plugins, while Kova core owns the shared `tools.media` config, fallback order, and reply-pipeline integration.
...
If `tools.media.<capability>.enabled` is **not** set to `false` and you haven't configured models, Kova auto-detects in this order and **stops at the first working option**:
...
<Steps>
  ...
  <Step title="Provider auth">
    ...
    - Ollama image understanding is available when selected explicitly, for example through `agents.defaults.imageModel` or `kova infer image describe --model ollama/<vision-model>`.
...
When provider-based **audio** and **video** media understanding is enabled, Kova honors standard outbound proxy environment variables for provider HTTP calls:
...
If no proxy env vars are set, media understanding uses direct egress. If the proxy value is malformed, Kova logs a warning and falls back to direct fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared lists, Kova can infer defaults:
...
<AccordionGroup>
  <Accordion title="File-attachment extraction behavior">
    ...
    - If a file has no extractable text, Kova injects `[No extractable text]`.
