...
- CLI `args` should use `{{MediaPath}}` for the local audio file path. Run `kova doctor --fix` to migrate deprecated `{input}` placeholders from older `audio.transcription.command` configs.
