...
```bash
kova nodes describe --node <idOrNameOrIp>
kova nodes canvas snapshot --node <idOrNameOrIp>
kova logs --follow
```
...
kova nodes status
kova approvals get --node <idOrNameOrIp>
kova approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`kova approvals get --node ...`), not in the gateway pairing record.
