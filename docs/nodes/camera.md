...
```bash
kova nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
kova nodes camera snap --node <id> --facing front
kova nodes camera clip --node <id> --duration 3000
kova nodes camera clip --node <id> --no-audio
```
...
Use the main `kova` CLI to invoke camera commands on the macOS node.

Examples:

```bash
kova nodes camera list --node <id>            # list camera ids
kova nodes camera snap --node <id>            # prints MEDIA:<path>
kova nodes camera snap --node <id> --max-width 1280
kova nodes camera snap --node <id> --delay-ms 2000
kova nodes camera snap --node <id> --device-id <id>
kova nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
kova nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
kova nodes camera clip --node <id> --device-id <id>
kova nodes camera clip --node <id> --no-audio
```

Notes:

- `kova nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
kova nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
