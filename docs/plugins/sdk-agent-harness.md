...
An **agent harness** is the low level executor for one prepared Kova agent
...
runtime and the normal Kova provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Kova
  session transcript
...
Before a harness is selected, Kova has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Kova transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Kova-owned
...
Kova chooses a harness after provider/model resolution:
...
provider/model. Once a plugin harness has claimed a run, Kova does not
...
Kova. The harness then claims that provider in `supports(...)`.
...
- app-server request: Kova sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs continue to use the
normal Kova provider path unless you force the Codex harness with
...
Kova requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Kova only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Kova to the newer tested stable line.
...
The bundled `codex` harness is the native Codex mode for embedded Kova
...
compaction, and app-server execution. Kova still owns the chat channel,
...
harnesses can claim a provider/model pair. If none match, Kova falls back
to PI.
...
want Kova to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
Keep that binding explicitly associated with the Kova session, and keep
mirroring user-visible assistant/tool output into the Kova transcript.

The Kova transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Kova can
clear it when the owning Kova session is reset.

## Tool and media results

Core constructs the Kova tool list and passes it into the prepared attempt.
