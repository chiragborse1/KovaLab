...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    kova plugins install ./my-bundle

    # Archive
    kova plugins install ./my-bundle.tgz
...
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Kova config. `kova mcp set` and `kova doctor --fix` normalize the common alias.
...
- Kova loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `kova plugins inspect <id>`
...
- Bundled plugin runtime dependencies ship inside the Kova package under
  `dist/*`. Kova does **not** run `npm install` at startup for bundled
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `kova plugins inspect <id>`. If a capability is listed but marked as
