...
Kova manages sessions end-to-end across these areas:
...
- Clients should query the Gateway for session lists and token counts.
...
```bash
kova sessions cleanup --dry-run
kova sessions cleanup --enforce
```
...
When Kova splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Kova
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Kova preserves that pending tool block and keeps the unsummarized tail
  intact.
...
Kova can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: Kova still runs normal semantic compaction,
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  Kova rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, Kova falls back to built-in LLM summarization automatically.
...
As of `2026.1.10`, Kova also suppresses **draft/typing streaming** when a
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `kova status`.
