...
- `kova gateway install` writes `NODE_EXTRA_CA_CERTS` into the systemd service environment
- the `kova` CLI entrypoint re-execs itself with `NODE_EXTRA_CA_CERTS` set before Node startup
