...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/chiragborse1/KovaLab-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   kova health --json
   kova health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    ...
    Showcase: [https://www.neuralstudio.in/showcase](https://www.neuralstudio.in/showcase)
...
<AccordionGroup>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Kova falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Kova intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    kova tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `KOVA_SKIP_CRON` is not set.
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.kova.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.kova/skills`) to allow Linux:
       ...
       metadata: { "kova": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="How do I use my existing signed-in Chrome with Kova?">
    ...
    kova browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Kova validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
...
<AccordionGroup>
  ...
  <Accordion title="Where does Kova store its data?">
    Everything lives under `$KOVA_STATE_DIR` (default: `~/.kova`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$KOVA_STATE_DIR/kova.json`                             | Main config (JSON5)                                                |
    | `$KOVA_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$KOVA_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$KOVA_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$KOVA_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$KOVA_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$KOVA_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$KOVA_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$KOVA_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.kova/agent/*` (migrated by `kova doctor`).
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.kova` (credentials, sessions, tokens, or encrypted secrets payloads).
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Kova reads an optional **JSON5** config from `$KOVA_CONFIG_PATH` (default: `~/.chiragborse1/KovaLab.json`):
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Kova enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `kova doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - If you want no banner at all, set env `KOVA_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `kova configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Kova auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Kova protects many accidental clobbers:

    - Kova-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Kova-owned writes are rejected and saved as `kova.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, the Gateway restores the last-known-good config and saves the rejected file as `kova.json.clobbered.*`.
    - The main agent receives a boot warning after recovery so it does not blindly write the bad config again.

    Recover:

    - Check `kova logs --follow` for `Config auto-restored from last-known-good`, `Config write rejected:`, or `config reload restored last-known-good config`.
    - Inspect the newest `kova.json.clobbered.*` or `kova.json.rejected.*` beside the active config.
    - Keep the active restored config if it works, then copy only the intended keys back with `kova config set` or `config.patch`.
    - Run `kova config validate` and `kova doctor`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `kova doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `kova config set` for small changes.
    - Use `kova configure` for interactive edits.
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `kova gateway status`
    - Gateway health: `kova status`
    - Channel health: `kova channels status`
  ...
  <Accordion title="Can two Kova instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `kova agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    kova agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Kova load environment variables?">
    ...
    - a global fallback `.env` from `~/.kova/.env` (aka `$KOVA_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    `KOVA_LOAD_SHELL_ENV=1`, `KOVA_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `kova models status` reports whether **shell env import** is enabled. "Shell env: off"
...
<AccordionGroup>
  ...
  <Accordion title="How do I completely reset Kova but keep it installed?">
    ...
    - If you used profiles (`--profile` / `KOVA_PROFILE`), reset each state dir (defaults are `~/.kova-<profile>`).
    - Dev reset: `kova gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    kova directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - Use `kova doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > KOVA_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does kova gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `kova gateway status` and trust these lines:
  ...
  <Accordion title='Why does kova gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `KOVA_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    ...
    Fix: stop the other instance, free the port, or run with `kova gateway --port <port>`.
  ...
  <Accordion title="How do I run Kova in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `kova gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `kova devices list`
      - `kova devices rotate --device <id> --role operator`
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `KOVA_CONFIG_PATH` (per-instance config)
    - `KOVA_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `kova --profile <name> ...` per instance (auto-creates `~/.kova-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `kova --profile <name> gateway install`.

    Profiles also suffix service names (`ai.kova.<profile>`; legacy `com.kova.*`, `kova-gateway-<profile>.service`, `Kova Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    kova tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$KOVA_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.kova/logs/...`; profiles use `~/.kova-<profile>/logs/...`)
    - Linux: `journalctl --user -u kova-gateway[-<profile>].service -n 200 --no-pager`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `kova gateway --force` can reclaim the port. See [Gateway](/gateway).
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    kova channels status
    kova channels logs --channel telegram
    ```
  ...
  <Accordion title="ELI5: kova gateway restart vs kova gateway">
    - `kova gateway restart`: restarts the **background service** (launchd/systemd).
    - `kova gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `kova gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Kova assistant setup](/start/kova) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    kova message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Kova to inbound DMs?">
    ...
    - Default behavior on DM-capable channels is **pairing**:
      ...
      - Approve with: `kova pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `kova pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `kova doctor` to surface risky DM policies.
...
Still stuck? Ask in [Discord](https://discord.com/invite/kova) or open a [GitHub discussion](https://github.com/chiragborse1/KovaLab/discussions).
