...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    - `kova status`: quick snapshot of gateway/agent health + basic config.
    - `kova models status`: checks provider auth + model availability.
    - `kova doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `kova status --all`, `kova logs --follow`,
    `kova gateway status`, `kova health --verbose`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    ...
    **Not on localhost:**
    ...
    - **Tailnet bind**: run `kova gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Kova now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Windows install says git not found or kova not recognized">
    ...
    **2) kova is not recognized after install**
  ...
  <Accordion title="Can I ask Kova to update itself?">
    ...
    kova update
    kova update status
    kova update --channel stable|beta|dev
    kova update --tag <dist-tag|version>
  ...
  <Accordion title="What does onboarding actually do?">
    `kova onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    For Anthropic in Kova, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Kova**: Anthropic staff
      told us this usage is allowed again, and Kova is treating `claude -p`
    ...
    Kova also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Kova-style Claude CLI usage is allowed again, so
    Kova treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Kova treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Kova token path, but Kova now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Kova, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Kova Claude-login path with Extra Usage enabled).
  ...
  <Accordion title="Why does Kova still mention openai-codex?">
    ...
    `kova models auth login --provider openai-codex` and use
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Kova can show the currently visible provider usage/quota windows in
    `kova models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    2. Enable the plugin: `kova plugins enable google`
    3. Login: `kova models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="If I buy a Mac mini to run Kova, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `kova nodes status` / `kova nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `kova doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `kova logs --follow` and read `from.id`.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Kova-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).
