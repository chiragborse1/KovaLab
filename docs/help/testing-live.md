...
pnpm kova infer tts convert --local --json \
  ...
  --output /tmp/kova-live-smoke.mp3
```
...
- Optional target overrides:
  - `KOVA_ANDROID_NODE_ID` or `KOVA_ANDROID_NODE_NAME`.
  - `KOVA_ANDROID_GATEWAY_URL` / `KOVA_ANDROID_GATEWAY_TOKEN` / `KOVA_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `KOVA_LIVE_TEST=1` if invoking Vitest directly)
- Set `KOVA_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `KOVA_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.2 + Codex, Gemini 3, DeepSeek V4, GLM 4.7, MiniMax M2.7, Grok 4)
  - `KOVA_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `KOVA_LIVE_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `KOVA_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `KOVA_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `KOVA_LIVE_MODEL_CONCURRENCY` to override.
- How to select providers:
  - `KOVA_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `KOVA_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@kovaai" actually does)
...
- How to select models:
  ...
  - `KOVA_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `KOVA_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `KOVA_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid “OpenRouter everything”):
  - `KOVA_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `KOVA_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.2"`
  - `KOVA_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `KOVA_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Docker recipes default this off unless explicitly requested.
  - `KOVA_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `KOVA_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `KOVA_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Docker recipes default this off for aggregate reliability.
  - `KOVA_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Docker recipes default this off unless explicitly requested.
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code`, `@openai/codex`, or `@google/gemini-cli`) into a cached writable prefix at `KOVA_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/kova/docker-cli-tools`).
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `KOVA_LIVE_ACP_BIND_AGENT=claude`
  - `KOVA_LIVE_ACP_BIND_AGENT=codex`
  - `KOVA_LIVE_ACP_BIND_AGENT=droid`
  - `KOVA_LIVE_ACP_BIND_AGENT=gemini`
  - `KOVA_LIVE_ACP_BIND_AGENT=opencode`
  - `KOVA_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `KOVA_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `KOVA_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.2`
  - `KOVA_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `KOVA_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1`
  - `KOVA_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `KOVA_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.2`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `KOVA_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `KOVA_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `KOVA_LIVE_ACP_BIND_AGENTS=claude`, `KOVA_LIVE_ACP_BIND_AGENTS=codex`, `KOVA_LIVE_ACP_BIND_AGENTS=droid`, `KOVA_LIVE_ACP_BIND_AGENTS=gemini`, or `KOVA_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- The OpenCode Docker variant is a strict single-agent regression lane. It writes a temporary `OPENCODE_CONFIG_CONTENT` default model from `KOVA_LIVE_ACP_BIND_OPENCODE_MODEL` (default `opencode/kimi-k2.6`) after sourcing `~/.profile`, and `pnpm test:docker:live-acp-bind:opencode` requires a bound assistant transcript instead of accepting the generic post-bind skip.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - select `KOVA_AGENT_RUNTIME=codex`
...
- Enable: `KOVA_LIVE_CODEX_HARNESS=1`
- Default model: `openai/gpt-5.2`
- Optional image probe: `KOVA_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `KOVA_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `KOVA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
- The smoke sets `KOVA_AGENT_HARNESS_FALLBACK=none` so a broken Codex
...
```bash
source ~/.profile
KOVA_LIVE_CODEX_HARNESS=1 \
  KOVA_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  KOVA_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  KOVA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
  KOVA_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.2 \
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `KOVA_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `KOVA_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `KOVA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
- Docker also exports `KOVA_AGENT_HARNESS_FALLBACK=none`, matching the live
...
- Single model, direct (no gateway):
  - `KOVA_LIVE_MODELS="openai/gpt-5.2" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `KOVA_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `KOVA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `KOVA_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `KOVA_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
`KOVA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `KOVA_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.
...
- Enable: `KOVA_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `KOVA_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `KOVA_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image-preview,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `KOVA_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `KOVA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

For the shipped CLI path, add an `infer` smoke after the provider/runtime live
test passes:

```bash
KOVA_LIVE_TEST=1 KOVA_LIVE_INFER_CLI_TEST=1 pnpm test:live -- test/image-generation.infer-cli.live.test.ts
...
- Enable: `KOVA_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `KOVA_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `KOVA_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `KOVA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `KOVA_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `KOVA_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `KOVA_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Loads provider env vars from your login shell (`~/.profile`) before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `KOVA_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `KOVA_LIVE_TEST=1 KOVA_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `KOVA_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `KOVA_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `KOVA_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `KOVA_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `KOVA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
