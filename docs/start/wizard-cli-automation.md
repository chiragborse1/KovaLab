...
Use `--non-interactive` to automate `kova onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Kova now prefers Claude CLI reuse when available.
