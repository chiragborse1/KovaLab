...
It binds the local Gateway, workspace, model, channel layer, web recall, skills,
and launch surface in one guided flow.

```bash
kova onboard
```

Onboarding opens with **Kova First-Light**, a launch sequence that shows the
phases before any config is written.

<Info>
Fastest first chat: open the Command Deck browser dashboard, no channel layer
needed. Run `kova dashboard` and chat in the browser. Docs:
...
<Tip>
CLI onboarding includes a web recall step where you can pick a provider
...
Onboarding starts by choosing a launch path:

<Tabs>
  <Tab title="Spark path">
    - Fast local launch
    - Local Gateway on loopback
    - Workspace default (or existing workspace)
    - Gateway port **18789**
    - Gateway token seal generated by default
    ...
    - Tailnet exposure **Off**
    - Telegram + WhatsApp DMs default to **allowlist** (you'll be prompted for your phone number)
  </Tab>
  <Tab title="Forge path">
    - Full control over mode, workspace, network posture, Gateway auth, channels, daemon, and skills.
  </Tab>
</Tabs>
...
2. **Workspace anchor** — Location for agent files (default `~/.kova/workspace`). Seeds bootstrap files.
3. **Gateway seal** — Port, bind address, auth mode, Tailnet exposure.
   In interactive token mode, choose default plaintext token storage or opt into SecretRef.
   Non-interactive token SecretRef path: `--gateway-token-ref-env <ENV_VAR>`.
4. **Channel layer** — built-in and bundled chat channels such as BlueBubbles, Discord, Feishu, Google Chat, Mattermost, Microsoft Teams, QQ Bot, Signal, Slack, Telegram, WhatsApp, and more.
5. **Background service** — Installs a LaunchAgent (macOS), systemd user unit (Linux/WSL2), or native Windows Scheduled Task with per-user Startup-folder fallback.
...
6. **Signal check** — Starts the Gateway and verifies it is reachable.
7. **Skill layer** — Installs recommended skills and optional dependencies.
8. **Launch surface** — Opens Terminal cockpit or Command Deck, or finishes without launching.
