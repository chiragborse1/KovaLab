...
<Note>
Requires Kova 2026.4.25 or above. Run `kova --version` to check. Upgrade with `kova update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  kova channels login --channel feishu
  ```
...
```bash
kova pairing list feishu
kova pairing approve feishu <CODE>
```
...
5. Ensure the gateway is running: `kova gateway status`
6. Check logs: `kova logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `kova gateway restart`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Kova
...
conversion fails, Kova falls back to a file attachment and logs the reason.
...
topic session key. Normal group replies that Kova turns into threads keep
