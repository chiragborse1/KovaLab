...
Tlon ships as a bundled plugin in current Kova releases, so normal packaged
...
```bash
kova plugins install ./path/to/local/tlon-plugin
```
...
Use these with `kova message send` or cron delivery:
