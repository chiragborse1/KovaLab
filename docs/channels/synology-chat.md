...
Synology Chat ships as a bundled plugin in current Kova releases, so normal
...
```bash
kova plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged Kova releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `kova onboard` now shows Synology Chat in the same channel setup list as `kova channels add`.
   - Non-interactive setup: `kova channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
4. Finish setup in Kova.
   - Guided: `kova onboard`
   - Direct: `kova channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Kova accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `kova pairing list synology-chat`
  - `kova pairing approve synology-chat <CODE>`
...
```bash
kova message send --channel synology-chat --target 123456 --text "Hello from Kova"
kova message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Kova forwards the URL to the NAS webhook.
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Kova
- `Rate limit exceeded`:
