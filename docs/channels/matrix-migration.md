...
When the gateway starts, and when you run [`kova doctor --fix`](/gateway/doctor), Kova tries to repair old Matrix state automatically.
Before any actionable Matrix migration step mutates on-disk state, Kova creates or reuses a focused recovery snapshot.

When you use `kova update`, the exact trigger depends on how Kova is installed:

- source installs run `kova doctor --fix` during the update flow, then restart the gateway by default
...
- if you use `kova update --no-restart`, startup-backed Matrix migration is deferred until you later run `kova doctor --fix` and restart the gateway
...
- custom Matrix plugin path installs are surfaced by both gateway startup and `kova doctor`
...
1. Update Kova and the Matrix plugin normally.
   Prefer plain `kova update` without `--no-restart` so startup can finish the Matrix migration immediately.
2. Run:
...
6. If Kova tells you a recovery key is needed, run the command for the matching account:

   ```bash
   printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify backup restore --recovery-key-stdin
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | kova matrix verify backup restore --recovery-key-stdin --account assistant
   ```
...
7. If this device is still unverified, run the command for the matching account:
   ...
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | kova matrix verify device --recovery-key-stdin --account assistant
   ```
...
1. Startup or `kova doctor --fix` creates or reuses the pre-migration snapshot if encrypted migration is actionable.
2. Startup or `kova doctor --fix` inspects the old Matrix crypto store through the active Matrix plugin install.
...
- What to do: configure `channels.matrix`, then rerun `kova doctor --fix` or restart the gateway.
...
- What to do: start the gateway once with a working Matrix login, or rerun `kova doctor --fix` after cached credentials exist.
...
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `kova doctor --fix` or restart the gateway.
...
- What to do: inspect filesystem permissions and disk state, then rerun `kova doctor --fix`.
...
- What to do: start the gateway once with a working Matrix login, or rerun `kova doctor --fix` after cached credentials are available.
...
`- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "kova doctor --fix".`

- Meaning: Kova refused to mutate Matrix state because it could not create the recovery snapshot first.
- What to do: resolve the backup error, then rerun `kova doctor --fix` or restart the gateway.
...
`Legacy Matrix encrypted state for account "..." has backed-up room keys, but no local backup decryption key was found. Ask the operator to run "kova matrix verify backup restore --recovery-key-stdin" after upgrade if they have the recovery key.`
...
- What to do: rerun `kova doctor --fix`. If it repeats, keep the old state directory intact and recover using another verified Matrix client plus `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify backup restore --recovery-key-stdin`.
...
- What to do: run `kova matrix verify backup status`, then retry with `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify backup restore --recovery-key-stdin` if needed.

### Manual recovery messages

`Backup key is not loaded on this device. Run 'kova matrix verify backup restore' to load it and restore old room keys.`

- Meaning: Kova knows you should have a backup key, but it is not active on this device.
- What to do: run `kova matrix verify backup restore`, or set `MATRIX_RECOVERY_KEY` and run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify backup restore --recovery-key-stdin` if needed.

`Store a recovery key with 'kova matrix verify device --recovery-key-stdin', then run 'kova matrix verify backup restore'.`

- Meaning: this device does not currently have the recovery key stored.
- What to do: set `MATRIX_RECOVERY_KEY`, run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify device --recovery-key-stdin`, then restore the backup.

`Backup key mismatch on this device. Re-run 'kova matrix verify device --recovery-key-stdin' with the matching recovery key.`
...
current backup baseline with `kova matrix verify backup reset --yes`. When the
...
`Backup trust chain is not verified on this device. Re-run 'kova matrix verify device --recovery-key-stdin'.`
...
- Meaning: Kova could apply the recovery key, but Matrix still has not
...
- What to do: run `kova matrix verify self`, accept the request in another
  ...
  `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing`
...
- What to do: verify the device first, then recheck with `kova matrix verify backup status`.

`Matrix crypto backend cannot load backup keys from secret storage. Verify this device with 'kova matrix verify device --recovery-key-stdin' first.`

- Meaning: this device cannot restore from secret storage until device verification is complete.
- What to do: run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify device --recovery-key-stdin` first.
...
```bash
kova matrix verify status --verbose
kova matrix verify backup status --verbose
printf '%s\n' "$MATRIX_RECOVERY_KEY" | kova matrix verify backup restore --recovery-key-stdin --verbose
```
