...
Current Kova releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`kova onboard`) and `kova channels add` still surface
...
```bash
kova plugins install --link <path-to-local-nostr-plugin>
```
...
kova channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
Profile data is published as a NIP-01 `kind:0` event. Manage it directly in config or through channel setup/configuration commands.
