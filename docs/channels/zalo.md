...
Zalo ships as a bundled plugin in current Kova releases, so normal packaged
...
- Approve via:
  - `kova pairing list zalo`
  - `kova pairing approve zalo <CODE>`
...
- Example: `kova message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `kova channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `kova logs --follow`
