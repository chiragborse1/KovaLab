...
5. Send a first DM and approve pairing (`kova pairing approve signal <CODE>`).
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `kova pairing approve signal <PAIRING_CODE>`.
...
- Approve via:
  ...
  - `kova pairing approve signal <CODE>`
...
- Config validation errors after edits: run `kova doctor --fix`.
