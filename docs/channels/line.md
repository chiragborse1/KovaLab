...
LINE ships as a bundled plugin in current Kova releases, so normal
...
```bash
kova plugins install ./path/to/local/line-plugin
```
...
```bash
kova pairing list line
kova pairing approve line <CODE>
```
...
Outbound media URLs must be public HTTPS URLs. Kova validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
