...
Current Kova releases bundle QQ Bot, so normal packaged builds do not need
a separate `kova plugins install` step.
...
```bash
kova channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `kova channels add --channel qqbot --token-file ...` provides the
...
```bash
kova channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
As an alternative to pasting `AppID:AppSecret` manually, the engine supports a QR-code onboarding flow for linking a QQ Bot to Kova:

1. Run the QQ Bot setup path (for example `kova channels add --channel qqbot`) and pick the QR-code flow when prompted.
2. Scan the generated QR code with the phone app tied to the target QQ Bot.
3. Approve the pairing on the phone. Kova persists the returned credentials into `credentials/` under the right account scope.

Approval prompts generated by the bot itself (for example, "allow this action?" flows exposed by the QQ Bot API) surface as native Kova prompts that you can accept with `/bot-approve` rather than replying through the raw QQ client.
...
- **Repeated self-replies:** Kova records QQ outbound ref indexes as
