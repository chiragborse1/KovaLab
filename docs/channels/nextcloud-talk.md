...
Nextcloud Talk ships as a bundled plugin in current Kova releases, so
...
```bash
kova plugins install ./path/to/local/nextcloud-talk-plugin
```
...
4. Configure Kova:
   ...
   ```bash
   kova channels add --channel nextcloud-talk \
...
- Approve via:
  - `kova pairing list nextcloud-talk`
  - `kova pairing approve nextcloud-talk <CODE>`
