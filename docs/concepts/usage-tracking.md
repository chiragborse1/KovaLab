...
- CLI: `kova status --usage` prints a full per-provider breakdown.
- CLI: `kova channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Kova treats
  ...
  quota, so Kova inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Kova prefers the
...
can supply plugin-specific usage auth logic; otherwise Kova falls back to
