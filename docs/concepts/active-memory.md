...
Paste this into `kova.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Kova can show:
...
`config.promptOverride` replaces the default Active Memory prompt. Kova
...
5. Verify memory search itself works with `kova memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Kova auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `kova memory status --deep` to inspect the memory-search backend
      and index health.
