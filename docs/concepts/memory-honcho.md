...
`IDENTITY.md`, `memory/`, `canvas/`), `kova honcho setup` detects and
...
```bash
kova honcho setup                        # Configure API key and migrate files
kova honcho status                       # Check connection status
kova honcho ask <question>               # Query Honcho about the user
kova honcho search <query> [-k N] [-d D] # Semantic search over memory
```
