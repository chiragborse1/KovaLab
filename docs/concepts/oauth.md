...
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai-codex:default` profile, but once Kova has a local OAuth profile,
...
All of the above also respect `$KOVA_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)
...
Claude subscription limits, and Anthropic staff told us Kova-style Claude
CLI usage is allowed again. Kova therefore treats Claude CLI reuse and
...
If you want other subscription-style options in Kova, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/glm).
</Warning>

Kova also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

Kova supports Anthropic Claude CLI reuse again. If you already have a local
...
1. start Anthropic setup-token or paste-token from Kova
2. Kova stores the resulting Anthropic credential in an auth profile
...
Wizard path is `kova onboard` → auth choice `openai-codex`.
...
- exception: some external CLI credentials stay externally managed; Kova
  ...
  `openai-codex:default` profile, then Kova-owned refreshes keep the local
  profile canonical.
...
- `kova channels list --json` (shows `auth[]`)
