...
Kova gives agents tools to work across sessions, inspect status, and
...
the caller's current session; visible client labels such as `kova-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Kova
...
the completion origin only identifies a channel Kova can still reuse the
