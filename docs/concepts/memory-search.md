...
runtime package next to Kova and use `provider: "local"`.
...
When embeddings are unavailable, Kova still uses lexical ranking over FTS results instead of falling back to raw exact-match ordering only. That degraded mode boosts chunks with stronger query-term coverage and relevant file paths, which keeps recall useful even without `sqlite-vec` or an embedding provider.
...
**No results?** Run `kova memory status` to check the index. If empty, run
