...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-kova

    # Create a persistent volume (1GB is usually enough)
    fly volumes create kova_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `KOVA_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `KOVA_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set KOVA_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require a valid gateway auth path. This Fly.io example uses `KOVA_GATEWAY_TOKEN`, but `gateway.auth.password` or a correctly configured non-loopback `trusted-proxy` deployment also satisfy the requirement.
    - Treat these tokens like passwords.
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `kova.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-kova.fly.dev/`

    Authenticate with the configured shared secret. This guide uses the gateway
    token from `KOVA_GATEWAY_TOKEN`; if you switched to password auth, use
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `KOVA_GATEWAY_PORT=3000`).
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/kova.json`, so make sure your real config exists and includes `gateway.mode="local"` when you want a normal local gateway start.

Verify the config exists:

```bash
fly ssh console --command "cat /data/kova.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/kova.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/kova.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/kova.json"
```
...
**Fix:** Ensure `KOVA_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-kova

# Release public IPs
fly ips release <public-ipv4> -a my-kova
fly ips release <public-ipv6> -a my-kova
...
fly ips allocate-v6 --private -a my-kova
```
