---
summary: "Deploy Kova on Render with Infrastructure-as-Code"
read_when:
  - Deploying Kova to Render
...
Deploy Kova on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/chiragborse1/KovaLab)
...
repository configures everything needed to run Kova:
...
services:
  - type: web
    ...
    envVars:
      - key: KOVA_GATEWAY_PORT
        value: "8080"
      - key: KOVA_STATE_DIR
        value: /data/.kova
      - key: KOVA_WORKSPACE_DIR
        value: /data/workspace
      - key: KOVA_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Kova state
...
The Gateway clients is available at `https://<your-service>.onrender.com/`.

Connect using the configured shared secret. This deploy template auto-generates
`KOVA_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Kova repository, Render will not auto-deploy your Kova. To update it, run a manual Blueprint sync from the dashboard.
...
For Kova, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Kova state plus any configured
...
- Missing `KOVA_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `KOVA_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `kova backup create` in the Render shell.
