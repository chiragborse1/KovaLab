...
- Quickstart reuse path: when `gateway.auth.token` is already a SecretRef, onboarding resolves it before probe/Gateway clients bootstrap (for `env`, `file`, and `exec` refs) using the same fail-fast gate.
