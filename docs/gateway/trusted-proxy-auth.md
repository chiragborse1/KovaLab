...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    Kova checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    Kova extracts the user identity from the configured header.
  </Step>
...
`kova security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    Kova rejected a loopback-source trusted-proxy request.
...
<Steps>
  ...
  <Step title="Update Kova config">
    Update Kova config with trusted-proxy auth.
  </Step>
