...
Kova stores session transcripts on disk under `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
...
- If you install or update plugins (`kova plugins install <package>`, `kova plugins update <id>`), treat it like running untrusted code:
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `kova skills install` remains the separate ClawHub skill download/install flow.
