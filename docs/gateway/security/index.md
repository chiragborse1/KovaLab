...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Kova is **not** a hostile multi-tenant security boundary for multiple
...
## Quick check: `kova security audit`
...
- If someone can modify Gateway host state/config (`~/.kova`, including `kova.json`), treat them as a trusted operator.
...
- Kova's product default for trusted single-operator setups is that host exec on `gateway`/`node` is allowed without approval prompts (`security="full"`, `ask="off"` unless you tighten it). That default is intentional UX, not a vulnerability by itself.
...
`kova security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`kova security audit` raises `config.insecure_or_dangerous_flags` when
...
gateway:
  ...
  auth:
    mode: password
    password: ${KOVA_GATEWAY_PASSWORD}
```
...
boundary and lock down permissions on `~/.kova` (see the audit section below). If you need
...
- If you install or update plugins (`kova plugins install <package>`, `kova plugins update <id>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Kova runs a built-in dangerous-code scan before install/update. `critical` findings block by default.
...
- “Paste the full contents of ~/.kova or your logs.”

## External content special-token sanitization

Kova strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
Kova strips common model-family special-token literals from wrapped
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `KOVA_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__kova__/canvas/` and `/__kova__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_kova-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `KOVA_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `kova doctor --generate-gateway-token`.
...
paths, set `KOVA_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
break-glass. This is intentionally process environment only, not an
`kova.json` config key.
...
- Kova also has a narrow backend/container-local self-connect path for
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `KOVA_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `KOVA_GATEWAY_PASSWORD`).
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) and owner semantics for agent turns; narrower `x-kova-scopes` values do not reduce that shared-secret path.
...
- In those identity-bearing modes, omitting `x-kova-scopes` falls back to the normal operator default scope set; send the header explicitly when you want a narrower scope set.
...
Assume anything under `~/.kova/` (or `$KOVA_STATE_DIR/`) may contain secrets or private data:

- `kova.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
Kova loads workspace-local `.env` files for agents and tools, but never lets those files silently override gateway runtime controls.

- Any key that starts with `KOVA_*` is blocked from untrusted workspace `.env` files.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking the whole `KOVA_*` prefix means adding a new `KOVA_*` flag later can never regress into silent inheritance from workspace state.
...
- When sharing diagnostics, prefer `kova status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@kovaai", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.kova`) to filesystem tools.
...
- Prefer a dedicated profile for the agent (the default `kova` profile).
...
Kova’s browser navigation policy is strict by default: private/internal destinations stay blocked unless you explicitly opt in.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `kova gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `KOVA_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/chiragborse1/KovaLab-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.kova/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `kova security audit --deep` and confirm critical findings are resolved.
...
1. Email: [security@kovaai.ai](mailto:security@kovaai.ai)
