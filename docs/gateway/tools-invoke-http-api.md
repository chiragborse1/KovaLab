...
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"` on a private ingress) honor `x-kova-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `x-kova-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-kova-account-id: <accountId>` (when multiple accounts exist)
