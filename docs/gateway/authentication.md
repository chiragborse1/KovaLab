...
setup, but Kova also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `kova gateway`).
...
Anthropic setup-token auth is still available in Kova as a supported token
path. Anthropic staff has since told us that Kova-style Claude CLI usage is
allowed again, so Kova treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Kova to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Kova auth profile.
...
```bash
kova models auth paste-token --provider openrouter
```
...
- If auth exists but Kova cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Kova integration path is allowed again.
- Kova therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
When you debug order issues, `kova models status --probe` shows omitted
...
Run `kova models status` to confirm which profile is expiring. If an
