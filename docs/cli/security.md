---
summary: "CLI reference for `kova security` (audit and fix common security footguns)"
read_when:
...
kova security audit --deep --token <token>
...
```bash
kova security audit --json | jq '.summary'
kova security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
kova security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
