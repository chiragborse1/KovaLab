---
summary: "CLI reference for `kova agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
kova agent --to +15555550123 --message "status update" --deliver
kova agent --agent ops --message "Summarize logs"
kova agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
kova agent --agent ops --message "Run locally" --local
```
...
- Each `kova agent` invocation is treated as a one-shot run. Bundled or user-configured MCP servers opened for that run are retired after the reply, even when the command uses the Gateway path, so stdio MCP child processes do not stay alive between scripted invocations.
