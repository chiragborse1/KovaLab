---
summary: "CLI reference for `kova health` (gateway health snapshot via RPC)"
read_when:
...
- Default `kova health` asks the running gateway for its health snapshot. When the
