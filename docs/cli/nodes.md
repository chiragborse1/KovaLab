---
summary: "CLI reference for `kova nodes` (status, pairing, invoke, camera/canvas/screen)"
read_when:
...
```bash
kova nodes list
kova nodes list --connected
kova nodes list --last-connected 24h
kova nodes pending
kova nodes approve <requestId>
kova nodes reject <requestId>
kova nodes remove --node <id|name|ip>
kova nodes rename --node <id|name|ip> --name <displayName>
kova nodes status
kova nodes status --connected
kova nodes status --last-connected 24h
```
...
- `kova nodes pending` only needs pairing scope.
...
- `kova nodes approve <requestId>` inherits extra scope requirements from the
  pending request:
...
```bash
kova nodes invoke --node <id|name|ip> --command <command> --params <json>
```
...
For shell execution on a node, use the `exec` tool with `host=node` instead of `kova nodes run`.
