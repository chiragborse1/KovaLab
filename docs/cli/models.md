---
summary: "CLI reference for `kova models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
kova models status
kova models list
kova models set <model-or-alias>
kova models scan
```

`kova models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Kova falls back to matching OAuth/API-key
...
the command uses `KOVA_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, Kova resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Kova
...
By default Kova tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `KOVA_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Kova could not resolve a probeable
...
```bash
kova models auth add
kova models auth login --provider <id>
kova models auth setup-token --provider <id>
kova models auth paste-token
```
...
`kova plugins list` to see which providers are installed.
Use `kova models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
kova models auth login --provider openai-codex --set-default
```
...
- Anthropic note: Anthropic staff told us Kova-style Claude CLI usage is allowed again, so Kova treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Kova token path, but Kova now prefers Claude CLI reuse and `claude -p` when available.
