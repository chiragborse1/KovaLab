---
summary: "CLI reference for `kova message` (send + channel actions)"
read_when:
...
- `kova message` resolves supported channel SecretRefs before running the selected action.
...
```
kova message send --channel discord \
...
```bash
kova message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
kova message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
kova message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
kova message poll --channel msteams \
...
```
kova message react --channel slack \
...
```
kova message react --channel signal \
...
```
kova message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
kova message send --channel telegram --target @mychat \
