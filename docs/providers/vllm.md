...
vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. Kova connects to vLLM using the `openai-completions` API.

Kova can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth) and you do not define an explicit `models.providers.vllm` entry.

Kova treats `vllm` as a local OpenAI-compatible provider that supports
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    kova models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, Kova queries:
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden Kova attribution headers | Not injected on custom base URLs |
  ...
  <Accordion title="Qwen thinking controls">
    ...
    server expects Qwen chat-template kwargs. Kova maps `/think off` to:
  ...
  <Accordion title="Nemotron 3 thinking controls">
    ...
    returned as hidden reasoning or visible answer text. When an Kova session
  ...
  <Accordion title="Qwen tool calls appear as text">
    ...
    - vLLM returns an empty `tool_calls` array when Kova sends
      `tool_choice: "auto"`
    ...
    ```bash
    kova config set agents.defaults.models '{"vllm/Qwen-Qwen2.5-Coder-32B-Instruct":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for Kova.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set **and** no explicit `models.providers.vllm` config entry. If you have defined the provider manually, Kova skips discovery and uses only your declared models.
  </Accordion>
  ...
  <Accordion title="Tools render as raw text">
    ...
    - confirm the exact model id with `kova models list --provider vllm`
