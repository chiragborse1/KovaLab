---
summary: "fal image and video generation setup in Kova"
title: "Fal"
read_when:
  - You want to use fal image generation in Kova
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

Kova ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    kova onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in Kova, so `background:
...
<Tip>
Use `kova models list --provider fal` to see the full list of available fal
