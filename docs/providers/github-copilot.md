---
summary: "Sign in to GitHub Copilot from Kova using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `kova models auth login-github-copilot` flow
  ...
  <Tab title="Built-in provider (github-copilot)">
    ...
    <Steps>
      <Step title="Run the login command">
        ```bash
        kova models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        kova models set github-copilot/claude-opus-4.7
        ```
...
```bash
# Skip confirmation
kova models auth login-github-copilot --yes

# Login and set the default model in one step
kova models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

If you already have a GitHub OAuth access token for Copilot, import it during
headless setup with `kova onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Kova
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Kova sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    Kova resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Kova uses the highest-priority one.
    The device-login flow (`kova models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Kova runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Kova can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Kova discovers available
...
1. Kova resolves your GitHub token (from env vars or auth profile).
...
available, Kova skips Copilot and tries the next provider.
