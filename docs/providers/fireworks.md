...
title: "Fireworks"
read_when:
  - You want to use Fireworks with Kova
  - You need the Fireworks API key env var or default model id
---

[Fireworks](https://fireworks.ai) exposes open-weight and routed models through an OpenAI-compatible API. Kova includes a bundled Fireworks provider plugin.
...
<Steps>
  <Step title="Set up Fireworks auth through onboarding">
    ```bash
    kova onboard --auth-choice fireworks-api-key
    ```

    This stores your Fireworks key in Kova config and sets the Fire Pass starter model as the default.

  </Step>
  <Step title="Verify the model is available">
    ```bash
    kova models list --provider fireworks
    ```
  </Step>
</Steps>
...
Kova accepts dynamic Fireworks model ids too. Use the exact model or router id shown by Fireworks and prefix it with `fireworks/`.
...
<AccordionGroup>
  <Accordion title="How model id prefixing works">
    Every Fireworks model ref in Kova starts with `fireworks/` followed by the exact id or router path from the Fireworks platform. For example:

    - Router model: `fireworks/accounts/fireworks/routers/kimi-k2p5-turbo`
    - Direct model: `fireworks/accounts/fireworks/models/<model-name>`

    Kova strips the `fireworks/` prefix when building the API request and sends the remaining path to the Fireworks endpoint.
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key sitting only in `~/.profile` will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.kova/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
