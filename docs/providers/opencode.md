    ...
    **Best for:** the curated OpenCode multi-model proxy (Claude, GPT, Gemini).
    ...
    <Steps>
      <Step title="Run onboarding">
        ...
        ```bash
        kova onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Zen model as the default">
        ```bash
        kova config set agents.defaults.model.primary "opencode/claude-opus-4-6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        kova models list --provider opencode
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Go catalog">
    **Best for:** the OpenCode-hosted Kimi, GLM, and MiniMax lineup.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        kova onboard --auth-choice opencode-go
        ```

        Or pass the key directly:

        ```bash
        kova onboard --opencode-go-api-key "$OPENCODE_API_KEY"
        ```
      </Step>
      <Step title="Set a Go model as the default">
        ```bash
        kova config set agents.defaults.model.primary "opencode-go/kimi-k2.6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        kova models list --provider opencode-go
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Kova keeps
