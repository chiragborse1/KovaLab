...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    kova onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    kova onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    kova models list --provider mistral
    ```
  </Step>
</Steps>
...
Kova currently ships this bundled Mistral catalog:
...
<Note>
Kova defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Kova maps the session **thinking** level to Mistral's API:

    | Kova thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
