...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Kova detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        kova onboard --auth-choice zai-api-key
        ```
      </Step>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        kova onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        kova onboard --auth-choice zai-coding-cn

        # General API
        kova onboard --auth-choice zai-global

        # General API CN (China region)
        kova onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Kova currently seeds the bundled `zai` provider with:
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows Kova's `/think` controls. With thinking off,
    Kova sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, Kova sends
