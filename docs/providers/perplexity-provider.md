...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    kova config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
...
<Tabs>
  <Tab title="Native Perplexity API (pplx-)">
    When your key starts with `pplx-`, Kova uses the native Perplexity Search
  ...
  <Tab title="OpenRouter / Sonar (sk-or-)">
    When your key starts with `sk-or-`, Kova routes through OpenRouter using
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Kova Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.kova/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    Kova will detect the prefix and switch to the Sonar transport
    automatically.
