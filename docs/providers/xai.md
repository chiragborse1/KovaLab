...
Kova includes these xAI model families out of the box:
...
The bundled plugin maps xAI's current public API surface onto Kova's shared
...
<Note>
Kova uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different Kova contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    kova config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    Kova can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    Kova asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. Kova forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    Kova uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the Kova speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through Kova's
    ...
    transcription request. Prompt hints are accepted by the shared Kova
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an Kova tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an Kova tool for
  ...
  <Accordion title="Known limits">
    ...
    - xAI Realtime voice is not registered as an Kova provider yet. It
  ...
  <Accordion title="Advanced notes">
    - Kova applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Kova
      tools. Kova enables the specific xAI built-in it needs inside each tool
...
shared image live file verifies the same xAI provider through Kova's
