...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    kova config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets Kova detect the matching Z.AI endpoint from the key and
...
Kova currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Kova inspects the key format
