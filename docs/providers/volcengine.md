...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    kova onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    kova models list --provider volcengine
    kova models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Kova asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `kova onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Kova falls back to the unfiltered catalog instead of showing an
...
<Warning>
When running Kova as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Kova setup.
  </Card>
</CardGroup>
