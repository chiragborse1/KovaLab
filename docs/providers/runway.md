---
summary: "Runway video generation setup in Kova"
title: "Runway"
read_when:
  - You want to use Runway video generation in Kova
  - You need the Runway API key/env setup
  - You want to make Runway the default video provider
---

Kova ships a bundled `runway` provider for hosted video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    kova onboard --auth-choice runway-api-key
    ```
  </Step>
  <Step title="Set Runway as the default video provider">
    ```bash
    kova config set agents.defaults.videoGenerationModel.primary "runway/gen4.5"
    ```
  </Step>
...
<AccordionGroup>
  <Accordion title="Environment variable aliases">
    Kova recognizes both `RUNWAYML_API_SECRET` (canonical) and `RUNWAY_API_KEY`.
    Either variable will authenticate the Runway provider.
  </Accordion>

  <Accordion title="Task polling">
    Runway uses a task-based API. After submitting a generation request, Kova
