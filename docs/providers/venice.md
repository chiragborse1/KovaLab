...
<Steps>
  ...
  <Step title="Configure Kova">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        kova onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    kova agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
kova models set venice/kimi-k2-5
kova models set venice/claude-opus-4-6
```

List all available models:

```bash
kova models list | grep venice
```

You can also run `kova configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Kova fills the required DeepSeek V4
...
so Kova keeps that provider-specific replay fix separate from the native
...
kova agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
kova agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
kova agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
kova agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
kova agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `kova models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
