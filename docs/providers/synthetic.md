...
Kova registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    kova onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Kova's Anthropic client appends `/v1` to the base URL automatically, so use
...
`kova models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Kova appends `/v1` automatically.
