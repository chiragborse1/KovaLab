...
@Suite struct WakeWordGateTests {
    ...
    @Test func matchHandlesMultiWordTriggers() {
        ...
        let config = WakeWordGateConfig(triggers: ["hey kova"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    @Test func matchPrefersMostSpecificTriggerWhenOverlapping() {
        ...
        let config = WakeWordGateConfig(triggers: ["kova", "hey kova"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
        #expect(match?.trigger == "hey kova")
    }
