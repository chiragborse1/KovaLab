...
coverage:
  ...
  secondary:
    ...
    - agents.create
objective: Verify Crestodian can bootstrap a fresh Kova config, set the default model, create an agent, configure Discord through a SecretRef, validate config, and leave an audit trail.
successCriteria:
...
steps:
  - name: bootstraps config through Crestodian CLI
    actions:
      ...
      - set: configPath
        value:
          expr: "path.join(stateDir, 'kova.json')"
      ...
      - set: crestodianEnv
        value:
          expr: "({ KOVA_STATE_DIR: stateDir, KOVA_CONFIG_PATH: configPath, KOVA_BUNDLED_PLUGINS_DIR: path.join(env.repoRoot, 'dist', 'extensions'), [setupSpec.discordEnv]: setupSpec.discordToken })"
      - call: fs.rm
        args:
          - ref: stateDir
          - recursive: true
