...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@kovaai/speech-core",
...
function asLegacyTtsConfig(value: unknown): KovaConfig {
  return value as KovaConfig;
}

function asLegacyKovaConfig(value: Record<string, unknown>): KovaConfig {
  return value as unknown as KovaConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): KovaConfig {
...
function createResolvedSummarizationConfig(cfg: KovaConfig): ResolvedTtsConfig {
  const rawConfig =
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: KovaConfig = asLegacyKovaConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.KOVA_TTS_PREFS;
      process.env.KOVA_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.KOVA_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: KovaConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
