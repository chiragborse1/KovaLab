...
async function createToolRichScenario(workspaceDir: string): Promise<PromptScenario> {
  const skillsPrompt = [
    ...
    "<skill><name>release</name><description>Release Kova safely.</description><location>/skills/release/SKILL.md</location></skill>",
    "</available_skills>",
  ].join("\n");
...
export async function createWorkspaceWithPromptCompositionFiles(): Promise<string> {
  const workspaceDir = await makeTempWorkspace("kova-prompt-cache-");
  await writeWorkspaceFile({
    dir: workspaceDir,
    name: "AGENTS.md",
    content: [
...
export async function createPromptCompositionScenarios(): Promise<{
  ...
  const warningWorkspaceDir = await makeTempWorkspace("kova-prompt-cache-warning-");
  const scenarios = [
