...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked KOVA_HOME.
  delete process.env.KOVA_HOME;
  process.env.KOVA_STATE_DIR = path.join(base, ".kova");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "kova-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".kova", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
