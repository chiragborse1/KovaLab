...
const piEmbeddedMocks = getSharedMocks("kova.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("kova.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("kova.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("kova.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    kovaHome: process.env.KOVA_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("KOVA_HOME", snapshot.kovaHome);
  restoreKey("KOVA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.KOVA_STATE_DIR = join(home, ".kova");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "kova-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".kova", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): KovaConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "kova"),
