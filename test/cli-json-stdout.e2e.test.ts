...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".kova");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "kova.json"), "{}", "utf8");
        ...
        delete env.KOVA_STATE_DIR;
        delete env.KOVA_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "kova.mjs");
