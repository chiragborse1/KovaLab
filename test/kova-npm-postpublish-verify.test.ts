...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/kova-prefix", "getkova@2026.4.10");
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "kova-postpublish-context-engine-"));
  }
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/kova-prefix", "darwin")).toBe(
      "/tmp/kova-prefix/bin/kova",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/kova-prefix", "win32")).toBe("C:/kova-prefix/kova.cmd");
  });
});

describe("collectInstalledMirroredRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "kova-postpublish-installed-"));
  }
  ...
  it("skips manifest-only sidecar directories without package.json", () => {
    ...
    try {
      ...
      writePackageFile(packageRoot, "dist/extensions/device-pair/kova.plugin.json", {
        id: "device-pair",
      });
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "kova-postpublish-root-deps-"));
  }
