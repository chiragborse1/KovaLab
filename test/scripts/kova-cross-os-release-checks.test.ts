...
describe("scripts/kova-cross-os-release-checks", () => {
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "kova-cross-os-agent-output-"));
    try {
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        KOVA_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        KOVA_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        KOVA_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("kova-2026.4.14.tgz")).toBe("application/octet-stream");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('overrideEnvVar: "KOVA_BROWSER_CONTROL_MODULE"');
    ...
    expect(readFileSync("scripts/kova-cross-os-release-checks.ts", "utf8")).toContain(
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "kova-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "kova-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects bundled runtime-deps staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "kova-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Kova-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Kova-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
