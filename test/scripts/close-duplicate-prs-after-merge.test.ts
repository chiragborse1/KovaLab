...
function pr(params: {
  ...
  return {
    ...
    url: `https://github.com/chiragborse1/KovaLab/pull/${params.number}`,
  };
}
...
@@ -286,8 +286,11 @@`,
  ...
  it("dry-runs through gh reads without mutating", () => {
    ...
    const responses = new Map<string, string>([
      [
        "pr view 70532 --repo chiragborse1/KovaLab --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
        JSON.stringify(
          pr({
            body: "Fixes #70491",
            mergeCommit: "6415e35",
      ...
      [
        "pr view 70592 --repo chiragborse1/KovaLab --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
    ...
    const args = parseArgs(["--landed-pr", "70532", "--duplicates", "70592"], {
      GITHUB_REPOSITORY: "chiragborse1/KovaLab",
    });
  ...
  it("applies labels, comment, and close commands for close actions", () => {
    ...
    expect(calls).toEqual([
      ...
      ["pr", "comment", "70592", "--repo", "chiragborse1/KovaLab", "--body", "closing"],
      ["pr", "close", "70592", "--repo", "chiragborse1/KovaLab"],
    ]);
  });
});
