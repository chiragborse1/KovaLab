...
const CHECK_SCRIPT = "scripts/check-kova-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "kova-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "kova", version }));
    writeFileSync(
    ...
    const tarball = join(root, "kova.tgz");
...
describe("check-kova-package-tarball", () => {
