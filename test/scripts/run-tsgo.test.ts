...
describe("run-tsgo sparse guard", () => {
  ...
  it("recognizes the check:changed sparse-skip env", () => {
    expect(shouldSkipSparseTsgoGuardError({ KOVA_TSGO_SPARSE_SKIP: "1" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ KOVA_TSGO_SPARSE_SKIP: "true" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ KOVA_TSGO_SPARSE_SKIP: "0" })).toBe(false);
