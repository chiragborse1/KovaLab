...
function createTempHome(): string {
  return makeTempDir(tempDirs, "kova-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-kova.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.kova/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.kova/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".kova", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".kova", "external-plugins", "gluekova", "kova.plugin.json"),
      '{"id":"gluekova"}\n',
    );
    writeFile(
      path.join(realHome, ".kova", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.KOVA_LIVE_TEST_QUIET = "1";
    process.env.KOVA_CONFIG_PATH = "~/custom-kova.json5";
    process.env.KOVA_TEST_HOME = priorIsolatedHome;
    process.env.KOVA_STATE_DIR = path.join(priorIsolatedHome, ".kova");
    ...
    expect(process.env.KOVA_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".kova", "kova.json");
    ...
    expect(fs.existsSync(path.join(testEnv.tempHome, ".kova", "credentials", "token.txt"))).toBe(
      true,
    );
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".kova", "external-plugins", "gluekova", "kova.plugin.json"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".kova", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.KOVA_LIVE_TEST = "1";
    process.env.KOVA_LIVE_USE_REAL_HOME = "1";
    process.env.KOVA_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.KOVA_LIVE_TEST;
    delete process.env.KOVA_LIVE_GATEWAY;
    delete process.env.KOVA_LIVE_USE_REAL_HOME;
    delete process.env.KOVA_LIVE_TEST_QUIET;
