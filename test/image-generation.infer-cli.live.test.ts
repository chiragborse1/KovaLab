...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.KOVA_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through kova infer", () => {
    const outputBase = path.join(os.tmpdir(), `kova-infer-image-${process.pid}.png`);
