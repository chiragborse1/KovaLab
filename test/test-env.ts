...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.KOVA_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "KOVA_TEST_FAST", value: process.env.KOVA_TEST_FAST },
    {
      key: "KOVA_STRICT_FAST_REPLY_CONFIG",
      value: process.env.KOVA_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "KOVA_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.KOVA_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "KOVA_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.KOVA_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "KOVA_STATE_DIR", value: process.env.KOVA_STATE_DIR },
    { key: "KOVA_CONFIG_PATH", value: process.env.KOVA_CONFIG_PATH },
    { key: "KOVA_GATEWAY_PORT", value: process.env.KOVA_GATEWAY_PORT },
    { key: "KOVA_BRIDGE_ENABLED", value: process.env.KOVA_BRIDGE_ENABLED },
    { key: "KOVA_BRIDGE_HOST", value: process.env.KOVA_BRIDGE_HOST },
    { key: "KOVA_BRIDGE_PORT", value: process.env.KOVA_BRIDGE_PORT },
    { key: "KOVA_CANVAS_HOST_PORT", value: process.env.KOVA_CANVAS_HOST_PORT },
    { key: "KOVA_TEST_HOME", value: process.env.KOVA_TEST_HOME },
    { key: "KOVA_AGENT_DIR", value: process.env.KOVA_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "kova-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.KOVA_TEST_HOME = tempHome;
  process.env.KOVA_TEST_FAST = "1";
  process.env.KOVA_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.KOVA_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.KOVA_BRIDGE_ENABLED;
  delete process.env.KOVA_BRIDGE_HOST;
  delete process.env.KOVA_BRIDGE_PORT;
  delete process.env.KOVA_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.KOVA_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.KOVA_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".kova");
  const priorIsolatedHome = params.env.KOVA_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".kova")
  ) {
    realStateDir = path.join(params.realHome, ".kova");
  }
  const tempStateDir = path.join(params.tempHome, ".kova");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.KOVA_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.KOVA_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "kova.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(path.join(tempStateDir, "kova.json"), sanitizeLiveConfig(rawConfig), "utf8");
  }
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.KOVA_LIVE_TEST === "1" ||
    process.env.KOVA_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.KOVA_LIVE_USE_REAL_HOME);
