---
name: kova-qa-testing
description: Run, watch, debug, extend, or explain Kova qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
KOVA_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm kova qa suite \
...
5. If the user wants to watch the live UI, find the current `kova-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm kova qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
KOVA_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `KOVA_QA_PROVIDER_MODE`
  - `KOVA_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Kova`
  - item: `KOVA_QA_CONVEX_SITE_URL`
  - item: `KOVA_QA_CONVEX_SECRET_MAINTAINER`
  - item: `KOVA_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `KOVA QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `KOVA_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `KOVA_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Kova/KOVA_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
KOVA_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
KOVA_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
KOVA_QA_PROVIDER_MODE="mock-openai" \
KOVA_NPM_TELEGRAM_PACKAGE_SPEC="kova@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `KOVA_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `KOVA_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `KOVA_QA_PROVIDER_MODE`, map it explicitly to `KOVA_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Kova identity combines with the character.
...
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `KOVA_HOME` sandboxed.
