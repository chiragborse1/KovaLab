...
Kova maintainers can opt into Testbox-first validation by setting
`KOVA_TESTBOX=1` in their environment or standing agent rules. This mode is
maintainers-only and requires Blacksmith access.

When `KOVA_TESTBOX=1` is set in Kova:
...
- Use local commands only when the task explicitly sets
  `KOVA_LOCAL_CHECK_MODE=throttled|full`, or when the user asks for local
  proof.
...
For Kova specifically, normal local iteration stays local unless maintainer
Testbox mode is enabled with `KOVA_TESTBOX=1`:
...
If `KOVA_TESTBOX=1` is enabled, run those same repo commands inside the
warm Testbox. If the user wants laptop-friendly local proof for one command, use
the explicit escape hatch `KOVA_LOCAL_CHECK_MODE=throttled`.
...
For Kova maintainer Testbox mode, pre-warm at the start of longer or wider
tasks:
...
In Kova, raw commit SHAs are not reliable dispatch refs for `warmup --ref`;
use a branch or tag. The build-artifact workflow resolves `kova@beta` and
`kova@latest` to SHA cache keys internally.
...
Kova maintainer exception: if `KOVA_TESTBOX=1` is set by the user or
agent environment, treat Testbox as the normal validation path for this repo.
Use `KOVA_LOCAL_CHECK_MODE=throttled|full` as the explicit local escape
hatch.
...
1. Decide whether the repo's local loop is the right default. For Kova,
   `KOVA_TESTBOX=1` makes Testbox the maintainer default.
...
## Kova full test suite

For Kova, use the repo package manager and the measured stable full-suite
...
shard to one worker to avoid worker OOMs on Testbox:
    ...
    blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 KOVA_TEST_PROJECTS_PARALLEL=6 KOVA_VITEST_MAX_WORKERS=1 pnpm test"
...
If you need a longer session, increase the timeout at warmup time. For Kova
