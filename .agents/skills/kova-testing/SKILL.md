---
name: kova-testing
description: Choose, run, rerun, or debug Kova tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
KOVA_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
KOVA_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `KOVA_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `KOVA_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Kova Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the private `kova/releases-private` evidence ledger.
Use the manual `Kova Release Evidence`
(`kova-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation chiragborse1/KovaLab <run-id> blocking
package-acceptance chiragborse1/KovaLab <run-id> blocking
release-checks chiragborse1/KovaLab <run-id> blocking
```
...
`KOVA_RELEASES_PRIVATE_DISPATCH_TOKEN` is configured in the public repo, it
requests the private `Kova Release Evidence From Full Validation` workflow.
...
`Kova Release Checks` (`kova-release-checks.yml`) is the release child
...
```bash
gh workflow run kova-release-checks.yml \
  --repo chiragborse1/KovaLab \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`pnpm kova qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `KOVA_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `Kova Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`Kova Live And E2E Checks (Reusable)`
(`kova-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run kova-live-and-e2e-checks-reusable.yml \
  --repo chiragborse1/KovaLab \
  --ref main \
  -f ref=<sha> \
...
`KOVA_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
KOVA_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
KOVA_DOCKER_ALL_DRY_RUN=1 KOVA_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
KOVA_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
KOVA_DOCKER_ALL_LANES=<lane> \
KOVA_DOCKER_ALL_BUILD=0 \
KOVA_DOCKER_ALL_PREFLIGHT=0 \
KOVA_SKIP_DOCKER_BUILD=1 \
KOVA_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view kova@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/kova-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=kova@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=kova@YYYY.M.D-beta.N` or
  `package_spec=kova@YYYY.M.D`.
- For stable package proof, use `package_spec=kova@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `kova@beta`,
  `kova@latest`, or exact Kova release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `kova update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `kova@beta`, `kova@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Kova branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-kova-for-docker.mjs` is the
