...
description: Search duplicate Kova PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
```bash
ghr pr view -R chiragborse1/KovaLab <number> --comments
ghr pr reviews -R chiragborse1/KovaLab <number>
ghr pr comments -R chiragborse1/KovaLab <number>
```

For an issue:

```bash
ghr issue view -R chiragborse1/KovaLab <number> --comments
ghr issue comments -R chiragborse1/KovaLab <number>
```
...
```bash
ghr search related-prs -R chiragborse1/KovaLab <pr-number> --mode path_overlap --state all
ghr search related-prs -R chiragborse1/KovaLab <pr-number> --mode range_overlap --state all
ghr search mentions -R chiragborse1/KovaLab --query "<key phrase from title or body>" --mode fts --scope pull_requests --state all
ghr search mentions -R chiragborse1/KovaLab --query "<subsystem or error phrase>" --mode fts --scope issues --state all
```

Use `prs-by-paths` or `prs-by-ranges` when the likely duplicate surface is already known:

```bash
ghr search prs-by-paths -R chiragborse1/KovaLab --path src/example.ts --state all
ghr search prs-by-ranges -R chiragborse1/KovaLab --path src/example.ts --start 20 --end 80 --state all
```
...
```bash
ghr search mentions -R chiragborse1/KovaLab --query "<issue title phrase>" --mode fts --scope issues --state all
ghr search mentions -R chiragborse1/KovaLab --query "<error message or symptom>" --mode fts --scope issues --state all
ghr search mentions -R chiragborse1/KovaLab --query "<subsystem phrase>" --mode fts --scope pull_requests --state all
```
...
```bash
uvx --from pr-search-cli pr-search -R chiragborse1/KovaLab code similar <pr-number>
uvx --from pr-search-cli pr-search -R chiragborse1/KovaLab code clusters for-pr <pr-number>
uvx --from pr-search-cli pr-search -R chiragborse1/KovaLab issues for-pr <pr-number>
uvx --from pr-search-cli pr-search -R chiragborse1/KovaLab issues duplicate-prs
```
...
```bash
prtags search text -R chiragborse1/KovaLab "<problem phrase>" --types group --limit 10
prtags search similar -R chiragborse1/KovaLab "<problem summary>" --types group --limit 10
prtags group list -R chiragborse1/KovaLab
```
...
```bash
prtags group create -R chiragborse1/KovaLab \
  --kind mixed \
...
```bash
prtags field ensure -R chiragborse1/KovaLab --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R chiragborse1/KovaLab --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R chiragborse1/KovaLab --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R chiragborse1/KovaLab --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R chiragborse1/KovaLab <pr-number> \
...
```bash
prtags annotation issue set -R chiragborse1/KovaLab <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R chiragborse1/KovaLab
```
