---
name: kova-test-heap-leaks
description: Investigate Kova pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && KOVA_TEST_MEMORY_TRACE=1 KOVA_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 KOVA_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap KOVA_TEST_WORKERS=2 KOVA_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `KOVA_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/kova-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/kova-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/kova-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
