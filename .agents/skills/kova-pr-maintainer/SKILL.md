---
name: kova-pr-maintainer
description: Review, triage, close, label, comment on, or land Kova PRs/issues with maintainer evidence checks.
---
...
- Anytime you inspect Kova issues or PRs, check local `gitcrawl` data first for related threads, duplicate attempts, and already-landed fixes.
...
```bash
gitcrawl threads chiragborse1/KovaLab --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors chiragborse1/KovaLab --number <issue-or-pr-number> --limit 12 --json
gitcrawl search chiragborse1/KovaLab --query "<scope or title keywords>" --mode hybrid --json
gitcrawl cluster-detail chiragborse1/KovaLab --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete Kova problem/fix
  - random docs-only churn such as root README translations, generic wording tweaks, or community-plugin discoverability docs that should go through KovaHub
  ...
  - third-party channel/provider/tool/skill/plugin work that belongs on KovaHub instead of core
...
- For third-party capabilities, prefer the `r: third-party-extension` auto-response label when it applies; it points contributors to publish on KovaHub.
...
- Use `--repo chiragborse1/KovaLab` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo chiragborse1/KovaLab --match title,body --limit 50 -- "auto-update"
gh search issues --repo chiragborse1/KovaLab --match title,body --limit 50 -- "auto-update"
gh search issues --repo chiragborse1/KovaLab --match title,body --limit 50 \
