...
```bash
export KOVA_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.chiragborse1/KovaLab.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env KOVA_PARALLELS_DISCORD_TOKEN \
...
- Use the `kova` wrapper for guest `message send/read`; `node kova.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/kova-parallels-smoke.*`
...
- host inbound nonce appears in `kova message read` output
