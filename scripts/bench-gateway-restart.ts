...
function printUsage(): void {
  console.log(`Kova Gateway restart benchmark
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "kova-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "kova-bench",
    npm_config_update_notifier: "false",
    KOVA_CONFIG: configPath,
    KOVA_CONFIG_PATH: configPath,
    KOVA_GATEWAY_RESTART_TRACE: "1",
    KOVA_GATEWAY_STARTUP_TRACE: "1",
    KOVA_HOME: root,
    KOVA_NO_RESPAWN: "1",
    KOVA_STATE_DIR: path.join(root, "state"),
    KOVA_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}

function writeRestartIntent(env: NodeJS.ProcessEnv, targetPid: number, reason: string): boolean {
  const stateDir = env.KOVA_STATE_DIR;
  if (!stateDir) {
    return false;
  }
  try {
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "kova-gateway-restart-bench-"));
