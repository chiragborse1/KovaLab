...
function isSystemThrottleDisabled(env) {
  const normalized = env.KOVA_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.KOVA_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.KOVA_VITEST_MAX_WORKERS ?? env.KOVA_TEST_WORKERS);
  if (override !== null) {
