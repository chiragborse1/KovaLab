...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = pluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier:
      entrypoint === "index" ? "getkova/plugin-sdk" : `getkova/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "Kova plugin-sdk public surface", entrypoints);
}
