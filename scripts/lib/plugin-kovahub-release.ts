...
const KOVAHUB_DEFAULT_REGISTRY = "https://kovahub.ai";
const SAFE_EXTENSION_ID_RE = /^[a-z0-9][a-z0-9._-]*$/;
const KOVAHUB_SHARED_RELEASE_INPUT_PATHS = [
  ".github/workflows/plugin-kovahub-release.yml",
  ...
  "scripts/lib/plugin-kovahub-release.ts",
  "scripts/kova-npm-release-check.ts",
  "scripts/plugin-kovahub-publish.sh",
  "scripts/plugin-kovahub-release-check.ts",
  "scripts/plugin-kovahub-release-plan.ts",
] as const;

function getRegistryBaseUrl(explicit?: string) {
  return (
    explicit?.trim() ||
    process.env.KOVAHUB_REGISTRY?.trim() ||
    process.env.KOVAHUB_SITE?.trim() ||
    KOVAHUB_DEFAULT_REGISTRY
  );
}

export function collectKovaHubPublishablePluginPackages(
  rootDir = resolve("."),
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.kova?.release?.publishToKovaHub !== true) {
      continue;
    }
    if (!SAFE_EXTENSION_ID_RE.test(extensionId)) {
      validationErrors.push(
        `${extensionId}: extension directory name must match ^[a-z0-9][a-z0-9._-]*$ for KovaHub publish.`,
      );
      continue;
    }
  ...
  if (validationErrors.length > 0) {
    throw new Error(
      `Publishable KovaHub plugin metadata validation failed:\n${validationErrors.map((error) => `- ${error}`).join("\n")}`,
    );
  }
...
export function collectPluginKovaHubReleasePathsFromGitRange(params: {
  rootDir?: string;
  gitRange: GitRangeSelection;
}): string[] {
  return collectPluginKovaHubReleasePathsFromGitRangeForPathspecs(params, ["extensions"]);
}

function collectPluginKovaHubRelevantPathsFromGitRange(params: {
  rootDir?: string;
  gitRange: GitRangeSelection;
}): string[] {
  return collectPluginKovaHubReleasePathsFromGitRangeForPathspecs(params, [
    "extensions",
    ...KOVAHUB_SHARED_RELEASE_INPUT_PATHS,
  ]);
}

function collectPluginKovaHubReleasePathsFromGitRangeForPathspecs(
  params: {
    rootDir?: string;
    gitRange: GitRangeSelection;
  },
...
function hasSharedKovaHubReleaseInputChanges(changedPaths: readonly string[]) {
  return changedPaths.some((path) =>
    KOVAHUB_SHARED_RELEASE_INPUT_PATHS.some(
      (sharedPath) => path === sharedPath || path.startsWith(`${sharedPath}/`),
    ),
  );
}

export function resolveChangedKovaHubPublishablePluginPackages(params: {
...
export function resolveSelectedKovaHubPublishablePluginPackages(params: {
  plugins: PublishablePluginPackage[];
  selection?: string[];
  ...
  if (params.gitRange) {
    const changedPaths = collectPluginKovaHubRelevantPathsFromGitRange({
      rootDir: params.rootDir,
      gitRange: params.gitRange,
    });
    if (hasSharedKovaHubReleaseInputChanges(changedPaths)) {
      return params.plugins;
    }
    return resolveChangedKovaHubPublishablePluginPackages({
      plugins: params.plugins,
      changedPaths,
    });
  }
...
export function collectKovaHubVersionGateErrors(params: {
  plugins: PublishablePluginPackage[];
  gitRange: GitRangeSelection;
  rootDir?: string;
}): string[] {
  const changedPaths = collectPluginKovaHubReleasePathsFromGitRange({
    rootDir: params.rootDir,
    gitRange: params.gitRange,
  });
  const changedPlugins = resolveChangedKovaHubPublishablePluginPackages({
    plugins: params.plugins,
    changedPaths,
  });
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.kova?.release?.publishToKovaHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function isPluginVersionPublishedOnKovaHub(
  packageName: string,
  version: string,
  options: {
  ...
  throw new Error(
    `Failed to query KovaHub for ${packageName}@${version}: ${response.status} ${response.statusText}`,
  );
}

export async function collectPluginKovaHubReleasePlan(params?: {
  rootDir?: string;
  selection?: string[];
  selectionMode?: PluginReleaseSelectionMode;
  gitRange?: GitRangeSelection;
  ...
  const allPublishable = collectKovaHubPublishablePluginPackages(params?.rootDir);
  const selectedPublishable = resolveSelectedKovaHubPublishablePluginPackages({
  ...
  const all = await Promise.all(
    selectedPublishable.map(async (plugin) =>
      Object.assign({}, plugin, {
        alreadyPublished: await isPluginVersionPublishedOnKovaHub(
          plugin.packageName,
          plugin.version,
