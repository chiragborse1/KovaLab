...
import { parseReleaseVersion } from "../kova-npm-release-check.ts";
...
export function resolvePublishablePluginVersion(params: {
  ...
  if (parsedVersion === null) {
    params.validationErrors.push(
      `${params.extensionId}: package.json version must match X.Y.Z, X.Y.Z-beta.N, YYYY.M.D, YYYY.M.D-N, or YYYY.M.D-beta.N; found "${version}".`,
    );
    return null;
  }
...
export function collectPublishablePluginPackageErrors(
  ...
  if (!packageVersion) {
    ...
    errors.push(
      `package.json version must match X.Y.Z, X.Y.Z-beta.N, YYYY.M.D, YYYY.M.D-N, or YYYY.M.D-beta.N; found "${packageVersion}".`,
    );
  }
