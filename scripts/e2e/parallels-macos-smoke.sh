...
MODEL_ID=""
MODEL_ID_EXPLICIT=0
INSTALL_URL="https://www.neuralstudio.in/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_KOVA_BIN="/opt/homebrew/bin/kova"
GUEST_KOVA_ENTRY="/opt/homebrew/lib/node_modules/chiragborse1/KovaLab.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/kova-parallels-smoke.XXXXXX)"
...
TIMEOUT_UPDATE_DEV_S="${KOVA_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S:-1200}"
TIMEOUT_VERIFY_S=60
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://www.neuralstudio.in/install.sh
...
resolve_guest_git_kova_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/chiragborse1/KovaLab.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__KOVA_READY__\\n'\r"
expect "__KOVA_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/kova-prl.sh <<'__KOVA_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__KOVA_SCRIPT__\r"
send -- "/bin/bash /tmp/kova-prl.sh; rc=\$?; rm -f /tmp/kova-prl.sh; printf '__KOVA_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
guest_current_user_sh() {
  ...
  script+="$1"
  if headless_guest_fallback; then
    script_path="/tmp/kova-prl-${BASHPID:-$$}-$RANDOM.sh"
    local guest_home
    guest_home="$(parallels_macos_resolve_desktop_home "$VM_NAME" "$GUEST_CURRENT_USER")"
...
markers = [
    ...
    if line.startswith("__KOVA_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__KOVA_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__KOVA_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__KOVA_RUNNER__"$'\n'
  ...
  guest_home="$(resolve_guest_current_user_home)"
  guest_log_state_path="$(mktemp "${TMPDIR:-/tmp}/kova-guest-log-state.XXXXXX")"
  latest_npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/kova-guest-npm-log-state.XXXXXX")"
  npm_state_path="$(mktemp "${TMPDIR:-/tmp}/kova-guest-npm-log-path.XXXXXX")"
...
matches = re.findall(r"^__KOVA_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
curl -fsSL $install_url_q -o /tmp/kova-install.sh
bash /tmp/kova-install.sh${version_arg_q}
$GUEST_KOVA_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/kova-smoke-pnpm-bootstrap"
...
repair_legacy_dev_source_checkout_if_needed() {
  ...
  bootstrap_bin="/tmp/kova-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/kova"
  update_entry="$update_root/kova.mjs"
  ...
  guest_current_user_exec /usr/bin/git clone --depth 1 --branch main \
    https://github.com/chiragborse1/KovaLab.git "$update_root"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/kova-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/kova"
  update_log="/tmp/kova-smoke-update-dev.log"
  update_done="/tmp/kova-smoke-update-dev.done"
  update_runner="/tmp/kova-smoke-update-dev.sh"
  ensure_guest_pnpm_for_dev_update
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  KOVA_DISABLE_BUNDLED_PLUGINS=1 \
  $GUEST_NODE_BIN $GUEST_KOVA_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_KOVA_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_KOVA_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_KOVA_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_KOVA_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/kova-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
check_path "\$root/kova"
check_path "\$root/kova/extensions"
if [ -d "\$root/kova/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "\$extension_dir"
  done < <(/usr/bin/find "\$root/kova/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi
EOF
)"
...
start_manual_gateway_if_needed() {
  ...
  guest_gateway_log="/tmp/kova-parallels-macos-gateway.log"
  runner_log="/tmp/kova-parallels-gateway-start.log"
  done_path="/tmp/kova-parallels-gateway-start.done"
  runner_path="/tmp/kova-parallels-gateway-start.sh"
  printf 'manual gateway launch transport=%s user=%s\n' "$GUEST_CURRENT_USER_TRANSPORT" "$GUEST_CURRENT_USER"
  launch_cmd="$(cat <<EOF
set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'kova.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'kova-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'kova.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env \\
  ...
  KOVA_HOME=$(shell_quote "$guest_home") \\
  KOVA_STATE_DIR=$(shell_quote "$guest_home/.kova") \\
  KOVA_CONFIG_PATH=$(shell_quote "$guest_home/.chiragborse1/KovaLab.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_KOVA_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4 5 6 7 8; do
    if guest_current_user_exec "$GUEST_KOVA_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return 0
    fi
    if (( attempt < 8 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_KOVA_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_KOVA_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_KOVA_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_KOVA_ENTRY" models set "$MODEL_ID"
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_KOVA_ENTRY" config set agents.defaults.skipBootstrap true --strict-json
  guest_current_user_sh "$(cat <<EOF
...
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_KOVA_ENTRY") agent \
  --agent main \
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/kova-discord-token <<'__KOVA_TOKEN__'
$DISCORD_TOKEN_VALUE
__KOVA_TOKEN__
cat >/tmp/kova-discord-guilds.json <<'__KOVA_GUILDS__'
$guilds_json
__KOVA_GUILDS__
token="\$(tr -d '\n' </tmp/kova-discord-token)"
guilds_json="\$(cat /tmp/kova-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_KOVA_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_KOVA_ENTRY channels status --probe --json
rm -f /tmp/kova-discord-token /tmp/kova-discord-guilds.json
EOF
)"
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "kova-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "kova-main-upgrade.tgz"
