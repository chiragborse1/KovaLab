#!/usr/bin/env bash
# Verifies `kova plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "kova-plugin-update-e2e" KOVA_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${KOVA_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${KOVA_CURRENT_PACKAGE_TGZ:-}")"
...
npm install -g --prefix /tmp/npm-prefix \"\$package_tgz\" --no-fund --no-audit >/tmp/kova-install.log 2>&1
...
mkdir -p \"\$HOME/.kova/plugins\"
cat > \"\$HOME/.kova/plugins/installs.json\" <<'JSON'
{
  \"version\": 1,
  \"warning\": \"DO NOT EDIT. This file is generated by Kova plugin registry commands.\",
...
cat > /tmp/kova-e2e-registry.mjs <<'NODE'
...
NODE
node /tmp/kova-e2e-registry.mjs >/tmp/kova-e2e-registry.log 2>&1 &
registry_pid=\$!
...
if [ \"\$registry_ready\" -ne 1 ]; then
  ...
  cat /tmp/kova-e2e-registry.log || true
  exit 1
fi

before_config_hash=\"\"
if [ \"\$KOVA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT\" != \"1\" ]; then
  before_config_hash=\$(sha256sum \"\$HOME/.chiragborse1/KovaLab.json\" | awk '{print \$1}')
fi
...
node --input-type=module > /tmp/plugin-update-before.json <<'NODE'
  ...
  const config = readJson(path.join(home, \".kova\", \"kova.json\"));
  const index = readJson(path.join(home, \".kova\", \"plugins\", \"installs.json\"));
...
if [ -n \"\$before_config_hash\" ]; then
  after_config_hash=\$(sha256sum \"\$HOME/.chiragborse1/KovaLab.json\" | awk '{print \$1}')
