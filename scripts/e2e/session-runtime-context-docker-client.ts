...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".kova", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "kova.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("KOVA_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "kova-session-runtime-context-"));
  process.env.HOME = root;
  process.env.KOVA_STATE_DIR = path.join(root, ".kova");
  process.env.KOVA_CONFIG_PATH = path.join(process.env.KOVA_STATE_DIR, "kova.json");
  try {
    ...
    if (process.env.KOVA_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
