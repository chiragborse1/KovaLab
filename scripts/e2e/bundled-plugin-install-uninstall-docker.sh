...
IMAGE_NAME="$(docker_e2e_resolve_image "kova-bundled-plugin-install-uninstall-e2e" KOVA_BUNDLED_PLUGIN_INSTALL_UNINSTALL_E2E_IMAGE)"

docker_e2e_build_or_reuse "$IMAGE_NAME" bundled-plugin-install-uninstall

DOCKER_ENV_ARGS=(-e COREPACK_ENABLE_DOWNLOAD_PROMPT=0)
for env_name in \
  KOVA_BUNDLED_PLUGIN_SWEEP_TOTAL \
  KOVA_BUNDLED_PLUGIN_SWEEP_INDEX \
  KOVA_BUNDLED_PLUGIN_SWEEP_IDS; do
  env_value="${!env_name:-}"
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/kova-bundled-plugin-install-uninstall.XXXXXX")"
...
home_dir=$(mktemp -d "/tmp/kova-bundled-plugin-sweep.XXXXXX")
...
const explicit = (process.env.KOVA_BUNDLED_PLUGIN_SWEEP_IDS || "")
  .split(/[,\s]+/u)
...
const manifestEntries = fs
  ...
  .map((entry) => {
    const manifestPath = path.join(extensionRoot, entry.name, "kova.plugin.json");
...
const total = Number.parseInt(process.env.KOVA_BUNDLED_PLUGIN_SWEEP_TOTAL || "1", 10);
const index = Number.parseInt(process.env.KOVA_BUNDLED_PLUGIN_SWEEP_INDEX || "0", 10);
if (!Number.isInteger(total) || total < 1) {
  throw new Error(`KOVA_BUNDLED_PLUGIN_SWEEP_TOTAL must be >= 1, got ${process.env.KOVA_BUNDLED_PLUGIN_SWEEP_TOTAL}`);
}
if (!Number.isInteger(index) || index < 0 || index >= total) {
  throw new Error(`KOVA_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.KOVA_BUNDLED_PLUGIN_SWEEP_INDEX}`);
}
...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${KOVA_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${KOVA_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"
...
const managedPath = path.join(process.env.HOME, ".kova", "extensions", pluginId);
...
for plugin_entry in "${plugin_entries[@]}"; do
  ...
  install_log="/tmp/kova-install-${plugin_index}.log"
  uninstall_log="/tmp/kova-uninstall-${plugin_index}.log"
  echo "Installing bundled plugin: $plugin_id ($plugin_dir)"
  node "$KOVA_ENTRY" plugins install "$plugin_id" >"$install_log" 2>&1 || {
    cat "$install_log"
    exit 1
  }
  ...
  node "$KOVA_ENTRY" plugins uninstall "$plugin_id" --force >"$uninstall_log" 2>&1 || {
    cat "$uninstall_log"
    exit 1
  }
