...
async function main() {
  ...
  const stateDir =
    process.env.KOVA_STATE_DIR ??
    (await fs.mkdtemp(path.join(os.tmpdir(), "kova-crestodian-first-run-")));
  ...
  assert(
    shouldStartCrestodianForBareRoot(["node", "kova"]),
    "bare kova invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "kova", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
