...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsRes?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "kova/default") ?? modelIds.find((id) => id === "kova");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing kova model: ${JSON.stringify(modelIds)}`;
  } else if (modelsRes) {
...
if (!targetModel) {
  throw new Error(
    `kova model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
