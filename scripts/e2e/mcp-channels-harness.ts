...
// against the Kova npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/kova-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      KOVA_STATE_DIR: "/tmp/kova-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[kova mcp] ${String(chunk)}`);
  });
