...
const port = Number(process.env.MOCK_PORT ?? process.env.KOVA_MOCK_OPENAI_PORT);
const successMarker = process.env.SUCCESS_MARKER ?? "KOVA_E2E_OK";
const requestLog = process.env.MOCK_REQUEST_LOG;

if (!Number.isInteger(port) || port <= 0) {
  throw new Error("missing valid MOCK_PORT or KOVA_MOCK_OPENAI_PORT");
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/v1/models") {
    writeJson(res, 200, {
      object: "list",
      data: [{ id: "gpt-5.5", object: "model", owned_by: "kova-e2e" }],
    });
    return;
  }
