#!/usr/bin/env bash
# Installs a prepared Kova npm tarball in Docker, runs non-interactive
...
IMAGE_NAME="$(docker_e2e_resolve_image "kova-npm-onboard-channel-agent-e2e" KOVA_NPM_ONBOARD_E2E_IMAGE)"
DOCKER_TARGET="${KOVA_NPM_ONBOARD_DOCKER_TARGET:-bare}"
HOST_BUILD="${KOVA_NPM_ONBOARD_HOST_BUILD:-1}"
PACKAGE_TGZ="${KOVA_CURRENT_PACKAGE_TGZ:-}"
CHANNEL="${KOVA_NPM_ONBOARD_CHANNEL:-telegram}"

case "$CHANNEL" in
  telegram | discord) ;;
  *)
    echo "KOVA_NPM_ONBOARD_CHANNEL must be telegram or discord, got: $CHANNEL" >&2
    exit 1
    ;;
esac
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${KOVA_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "KOVA_NPM_ONBOARD_HOST_BUILD=0 requires KOVA_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
if ! docker run --rm \
  ...
  -e KOVA_NPM_ONBOARD_CHANNEL="$CHANNEL" \
  "${DOCKER_E2E_PACKAGE_ARGS[@]}" \
  "${DOCKER_E2E_HARNESS_ARGS[@]}" \
  -i "$IMAGE_NAME" bash -s >"$run_log" 2>&1 <<'EOF'
set -euo pipefail

export HOME="$(mktemp -d "/tmp/kova-npm-onboard.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-kova-npm-onboard-e2e"
export KOVA_GATEWAY_TOKEN="npm-onboard-channel-agent-token"

CHANNEL="${KOVA_NPM_ONBOARD_CHANNEL:?missing KOVA_NPM_ONBOARD_CHANNEL}"
PORT="18789"
MOCK_PORT="44080"
SUCCESS_MARKER="KOVA_AGENT_E2E_OK_ASSISTANT"
MOCK_REQUEST_LOG="/tmp/kova-mock-openai-requests.jsonl"
mock_pid=""

case "$CHANNEL" in
  telegram)
    CHANNEL_TOKEN="123456:kova-npm-onboard-token"
    DEP_SENTINEL="grammy"
    ;;
  discord)
    CHANNEL_TOKEN="kova-npm-onboard-discord-token"
...
dump_debug_logs() {
  ...
  for file in \
    /tmp/kova-install.log \
    /tmp/kova-onboard.json \
    /tmp/kova-channel-add.log \
    /tmp/kova-doctor.log \
    /tmp/kova-agent.combined \
    /tmp/kova-agent.err \
    /tmp/kova-agent.json \
    /tmp/kova-mock-openai.log \
    "$MOCK_REQUEST_LOG"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
assert_dep_absent() {
  local sentinel="$1"
  if find "$package_root" "$HOME/.kova" -path "*/node_modules/$sentinel/package.json" -print -quit 2>/dev/null | grep -q .; then
    echo "$sentinel should not be installed before channel activation repair" >&2
    find "$package_root" "$HOME/.kova" -path "*/node_modules/$sentinel/package.json" -print 2>/dev/null >&2 || true
    exit 1
  fi
}
...
assert_dep_present() {
  ...
  if ! find "$package_root" "$HOME/.kova" -path "*/node_modules/$sentinel/package.json" -print -quit 2>/dev/null | grep -q .; then
    ...
    find "$package_root" "$HOME/.kova" -maxdepth 6 -type d -name node_modules -print 2>/dev/null >&2 || true
    exit 1
  fi
}

MOCK_PORT="$MOCK_PORT" SUCCESS_MARKER="$SUCCESS_MARKER" MOCK_REQUEST_LOG="$MOCK_REQUEST_LOG" node scripts/e2e/mock-openai-server.mjs >/tmp/kova-mock-openai.log 2>&1 &
mock_pid="$!"
...
kova onboard --non-interactive --accept-risk \
  --mode local \
...
if (!fs.existsSync(configPath)) {
  throw new Error("onboard did not write kova.json");
}
...
if (authRaw.includes("sk-kova-npm-onboard-e2e")) {
...
kova channels add --channel "$CHANNEL" --token "$CHANNEL_TOKEN" >/tmp/kova-channel-add.log 2>&1
node - "$CHANNEL" "$CHANNEL_TOKEN" <<'NODE'
const fs = require("node:fs");
const path = require("node:path");
const channel = process.argv[2];
const token = process.argv[3];
const cfg = JSON.parse(fs.readFileSync(path.join(process.env.HOME, ".kova", "kova.json"), "utf8"));
...
kova doctor --repair --non-interactive >/tmp/kova-doctor.log 2>&1
...
kova agent --local \
  ...
  --thinking off \
  --json >/tmp/kova-agent.combined 2>&1

node - "$SUCCESS_MARKER" "$MOCK_REQUEST_LOG" <<'NODE'
const fs = require("node:fs");
const marker = process.argv[2];
const logPath = process.argv[3];
const output = fs.readFileSync("/tmp/kova-agent.combined", "utf8");
