...
import type { KovaConfig } from "../../dist/config/types.kova.js";
...
function makeParams(commandBody: string, cfg: KovaConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: KovaConfig, isGroup = false): Promise<string> {
...
async function main() {
  const stateDir =
    process.env.KOVA_STATE_DIR ?? (await fs.mkdtemp(path.join(os.tmpdir(), "kova-crestodian-")));
  const configPath = process.env.KOVA_CONFIG_PATH ?? path.join(stateDir, "kova.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env KOVA_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/kova-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/kova-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as KovaConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "KOVA_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/kova-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/kova-work",
    ),
