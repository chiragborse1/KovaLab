...
for env_name in \
  KOVA_PLUGINS_E2E_KOVAHUB \
  KOVA_PLUGINS_E2E_KOVAHUB_SPEC \
  KOVA_PLUGINS_E2E_KOVAHUB_ID \
  KOVA_KOVAHUB_URL \
  KOVAHUB_URL \
  KOVA_KOVAHUB_TOKEN \
  KOVAHUB_TOKEN \
  KOVAHUB_AUTH_TOKEN; do
  env_value="${!env_name:-}"
...
if [ "${KOVA_PLUGINS_E2E_KOVAHUB:-1}" = "0" ]; then
  echo "Skipping KovaHub plugin install and uninstall (KOVA_PLUGINS_E2E_KOVAHUB=0)."
else
echo "Testing KovaHub plugin install and uninstall..."
KOVAHUB_PLUGIN_SPEC="${KOVA_PLUGINS_E2E_KOVAHUB_SPEC:-kovahub:kova-now4real}"
KOVAHUB_PLUGIN_ID="${KOVA_PLUGINS_E2E_KOVAHUB_ID:-now4real}"
export KOVAHUB_PLUGIN_SPEC KOVAHUB_PLUGIN_ID

start_kovahub_fixture_server() {
  local fixture_dir="$1"
  local server_log="$fixture_dir/kovahub-fixture.log"
  local server_port_file="$fixture_dir/kovahub-fixture-port"
  local server_pid_file="$fixture_dir/kovahub-fixture-pid"
...
const packageName = "kova-now4real";
...
NODE
  ...
  for _ in $(seq 1 100); do
    if [[ -s "$server_port_file" ]]; then
      export KOVA_KOVAHUB_URL="http://127.0.0.1:$(cat "$server_port_file")"
  ...
  cat "$server_log"
  echo "Timed out waiting for KovaHub fixture server." >&2
  return 1
}

if [[ -z "${KOVA_KOVAHUB_URL:-}" && -z "${KOVAHUB_URL:-}" ]]; then
  # Keep the release-path smoke hermetic; live KovaHub can rate-limit CI.
  kovahub_fixture_dir="$(mktemp -d "/tmp/kova-kovahub-fixture.XXXXXX")"
  start_kovahub_fixture_server "$kovahub_fixture_dir"
fi

node - <<'NODE'
const spec = process.env.KOVAHUB_PLUGIN_SPEC;
if (!spec?.startsWith("kovahub:")) {
  throw new Error(`expected kovahub: spec, got ${spec}`);
}

const parsePackageName = (rawSpec) => {
  const value = rawSpec.slice("kovahub:".length).trim();
...
const baseUrl = (process.env.KOVA_KOVAHUB_URL || process.env.KOVAHUB_URL || "https://kovahub.ai")
  .replace(/\/+$/, "");
const token =
  process.env.KOVA_KOVAHUB_TOKEN ||
  process.env.KOVAHUB_TOKEN ||
  process.env.KOVAHUB_AUTH_TOKEN ||
  "";
...
if (!response.ok) {
  ...
  throw new Error(`KovaHub package preflight failed for ${packageName}: ${response.status} ${body}`);
}
const detail = await response.json();
const family = detail.package?.family;
if (family !== "code-plugin" && family !== "bundle-plugin") {
  throw new Error(`KovaHub package ${packageName} is not installable as a plugin: ${family}`);
}
if (detail.package?.runtimeId && detail.package.runtimeId !== process.env.KOVAHUB_PLUGIN_ID) {
  throw new Error(
    `KovaHub package ${packageName} runtimeId ${detail.package.runtimeId} does not match expected ${process.env.KOVAHUB_PLUGIN_ID}`,
  );
}
console.log(`Using KovaHub package ${packageName} (${family}).`);
NODE

run_logged install-kovahub node "$KOVA_ENTRY" plugins install "$KOVAHUB_PLUGIN_SPEC"
node "$KOVA_ENTRY" plugins list --json > /tmp/plugins-kovahub-installed.json
node "$KOVA_ENTRY" plugins inspect "$KOVAHUB_PLUGIN_ID" --json > /tmp/plugins-kovahub-inspect.json

node - <<'NODE'
const fs = require("node:fs");
const path = require("node:path");

const pluginId = process.env.KOVAHUB_PLUGIN_ID;
const spec = process.env.KOVAHUB_PLUGIN_SPEC;
const parsePackageName = (rawSpec) => {
  const value = rawSpec.slice("kovahub:".length).trim();
  const slashIndex = value.lastIndexOf("/");
...
const packageName = parsePackageName(spec);
const list = JSON.parse(fs.readFileSync("/tmp/plugins-kovahub-installed.json", "utf8"));
const inspect = JSON.parse(fs.readFileSync("/tmp/plugins-kovahub-inspect.json", "utf8"));
const plugin = (list.plugins || []).find((entry) => entry.id === pluginId);
if (!plugin) throw new Error(`KovaHub plugin not found after install: ${pluginId}`);
if (plugin.status !== "loaded") {
  throw new Error(`unexpected KovaHub plugin status for ${pluginId}: ${plugin.status}`);
}
if (inspect.plugin?.id !== pluginId) {
  throw new Error(`unexpected KovaHub inspect plugin id: ${inspect.plugin?.id}`);
}
...
if (!record) throw new Error(`missing KovaHub install record for ${pluginId}`);
if (record.source !== "kovahub") {
  throw new Error(`unexpected KovaHub install source for ${pluginId}: ${record.source}`);
}
if (record.kovahubPackage !== packageName) {
  throw new Error(
    `unexpected KovaHub package for ${pluginId}: ${record.kovahubPackage}, expected ${packageName}`,
  );
}
if (record.kovahubFamily !== "code-plugin" && record.kovahubFamily !== "bundle-plugin") {
  throw new Error(`unexpected KovaHub family for ${pluginId}: ${record.kovahubFamily}`);
}
if (typeof record.installPath !== "string" || record.installPath.length === 0) {
  throw new Error(`missing KovaHub install path for ${pluginId}`);
}

const installPath = record.installPath.replace(/^~(?=$|\/)/, process.env.HOME);
const extensionsRoot = path.join(process.env.HOME, ".kova", "extensions");
if (!installPath.startsWith(`${extensionsRoot}${path.sep}`)) {
  throw new Error(`KovaHub install path is outside managed extensions root: ${installPath}`);
}
if (!fs.existsSync(installPath)) {
  throw new Error(`KovaHub install path missing on disk: ${installPath}`);
}
fs.writeFileSync("/tmp/plugins-kovahub-install-path.txt", installPath, "utf8");
console.log("ok");
NODE

run_logged uninstall-kovahub node "$KOVA_ENTRY" plugins uninstall "$KOVAHUB_PLUGIN_SPEC" --force
node "$KOVA_ENTRY" plugins list --json > /tmp/plugins-kovahub-uninstalled.json
...
const pluginId = process.env.KOVAHUB_PLUGIN_ID;
const installPath = fs.readFileSync("/tmp/plugins-kovahub-install-path.txt", "utf8").trim();
const list = JSON.parse(fs.readFileSync("/tmp/plugins-kovahub-uninstalled.json", "utf8"));
if ((list.plugins || []).some((entry) => entry.id === pluginId)) {
  throw new Error(`KovaHub plugin still listed after uninstall: ${pluginId}`);
}
...
if (installRecords[pluginId]) {
  throw new Error(`KovaHub install record still present after uninstall: ${pluginId}`);
}

const configAfterUninstallPath = path.join(process.env.HOME, ".kova", "kova.json");
...
if (configAfterUninstall.plugins?.entries?.[pluginId]) {
  throw new Error(`KovaHub config entry still present after uninstall: ${pluginId}`);
}
if ((configAfterUninstall.plugins?.allow || []).includes(pluginId)) {
  throw new Error(`KovaHub allowlist entry still present after uninstall: ${pluginId}`);
}
if ((configAfterUninstall.plugins?.deny || []).includes(pluginId)) {
  throw new Error(`KovaHub denylist entry still present after uninstall: ${pluginId}`);
}
if (fs.existsSync(installPath)) {
  throw new Error(`KovaHub managed install directory still exists after uninstall: ${installPath}`);
}
console.log("ok");
NODE
fi
