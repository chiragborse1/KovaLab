...
export HOME="$(mktemp -d "/tmp/kova-bundled-channel-setup-entry.XXXXXX")"
...
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/kova-setup-entry-install.log 2>&1
...
for channel in "${!SETUP_ENTRY_DEP_SENTINELS[@]}"; do
  ...
  if find "$KOVA_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$dep_sentinel/package.json" -type f | grep -q .; then
    ...
    find "$KOVA_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
    exit 1
  fi
done

echo "Running packaged guided WhatsApp setup; runtime deps should be staged before finalize..."
KOVA_PACKAGE_ROOT="$root" node --input-type=module - <<'NODE'
import path from "node:path";
import { readdir } from "node:fs/promises";
import { pathToFileURL } from "node:url";

const root = process.env.KOVA_PACKAGE_ROOT;
if (!root) {
  throw new Error("missing KOVA_PACKAGE_ROOT");
}
const distDir = path.join(root, "dist");
...
if ! find "$KOVA_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/@whiskeysockets/baileys/package.json" -type f | grep -q .; then
...
kova doctor --non-interactive >/tmp/kova-setup-entry-doctor.log 2>&1
...
for channel in "${!SETUP_ENTRY_DEP_SENTINELS[@]}"; do
  ...
  if ! find "$KOVA_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$dep_sentinel/package.json" -type f | grep -q .; then
    ...
    cat /tmp/kova-setup-entry-doctor.log >&2
    find "$KOVA_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
    exit 1
  fi
done
