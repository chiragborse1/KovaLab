...
if [[ "${mode}" != "--dry-run" && "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/plugin-kovahub-publish.sh [--dry-run|--publish] <package-dir>" >&2
  exit 2
fi
...
if ! command -v kovahub >/dev/null 2>&1; then
  echo "kovahub CLI is required on PATH" >&2
  exit 1
fi
...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-chiragborse1/KovaLab}}"
source_commit="${SOURCE_COMMIT:-$(git rev-parse HEAD)}"
source_ref="${SOURCE_REF:-$(git symbolic-ref -q HEAD || true)}"
kovahub_workdir="${KOVAHUB_WORKDIR:-${KOVAHUB_WORKDIR:-$(pwd)}}"
...
echo "Resolved KovaHub workdir: ${kovahub_workdir}"
echo "Publish auth: GitHub Actions OIDC via KovaHub short-lived token"
...
if [[ "${mode}" == "--dry-run" ]]; then
  KOVAHUB_WORKDIR="${kovahub_workdir}" "${publish_cmd[@]}" --dry-run
  exit 0
fi

KOVAHUB_WORKDIR="${kovahub_workdir}" "${publish_cmd[@]}"
