...
DEFAULT_PROVIDER="${KOVA_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${KOVA_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${KOVA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${KOVA_LIVE_CLI_BACKEND_AUTH:-auto}"
CLI_SETUP_TIMEOUT_SECONDS="${KOVA_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}"
TEMP_DIRS=()
...
CLI_USE_CI_SAFE_CODEX_CONFIG="${KOVA_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG:-}"
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: KOVA_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: KOVA_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi

if [[ "$CLI_AUTH_MODE" == "api-key" && "$CLI_PROVIDER" == "codex-cli" ]]; then
  if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "ERROR: KOVA_LIVE_CLI_BACKEND_AUTH=api-key for codex-cli requires OPENAI_API_KEY." >&2
    exit 1
  fi
fi
...
export KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE="${KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export KOVA_LIVE_CLI_BACKEND_MCP_PROBE="${KOVA_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export KOVA_LIVE_CLI_BACKEND_RESUME_PROBE="${KOVA_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE="${KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export KOVA_LIVE_CLI_BACKEND_MCP_PROBE="${KOVA_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ "$CLI_AUTH_MODE" == "api-key" && "$CLI_PROVIDER" == "codex-cli" ]]; then
...
elif [[ -n "${KOVA_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(kova_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(kova_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
...
run_setup_command() {
  timeout --foreground "${KOVA_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s" "$@"
}
...
provider="${KOVA_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${KOVA_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${KOVA_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${KOVA_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ "$provider" = "codex-cli" ] && [ "${KOVA_LIVE_CLI_BACKEND_AUTH:-auto}" != "api-key" ]; then
...
if [ -z "${KOVA_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export KOVA_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${KOVA_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${KOVA_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ "$provider" = "codex-cli" ] && [ "${KOVA_LIVE_CLI_BACKEND_AUTH:-auto}" = "api-key" ]; then
  codex_login_command="${KOVA_LIVE_CLI_BACKEND_COMMAND:-$NPM_CONFIG_PREFIX/bin/codex}"
...
if [ -n "${KOVA_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${KOVA_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${KOVA_LIVE_CLI_BACKEND_COMMAND}"
  "${KOVA_LIVE_CLI_BACKEND_COMMAND}" -V || "${KOVA_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${KOVA_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="KOVA-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
cd "$tmp_dir"
if [ "${KOVA_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG:-0}" = "1" ]; then
...
DOCKER_AUTH_ENV=(
  -e KOVA_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "codex-cli" && "$CLI_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/kova-cli-backend-env.XXXXXX")"
  ...
  DOCKER_AUTH_ENV+=(
    ...
    -e KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV="$KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e KOVA_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV="${KOVA_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e KOVA_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG="$CLI_USE_CI_SAFE_CODEX_CONFIG" \
  -e KOVA_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e KOVA_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e KOVA_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e KOVA_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e KOVA_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e KOVA_LIVE_TEST=1 \
  -e KOVA_LIVE_CLI_BACKEND=1 \
  -e KOVA_LIVE_CLI_BACKEND_DEBUG="${KOVA_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e KOVA_CLI_BACKEND_LOG_OUTPUT="${KOVA_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e KOVA_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e KOVA_LIVE_CLI_BACKEND_COMMAND="${KOVA_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e KOVA_LIVE_CLI_BACKEND_ARGS="${KOVA_LIVE_CLI_BACKEND_ARGS:-}" \
  -e KOVA_LIVE_CLI_BACKEND_RESUME_ARGS="${KOVA_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e KOVA_LIVE_CLI_BACKEND_CLEAR_ENV="${KOVA_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e KOVA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e KOVA_LIVE_CLI_BACKEND_RESUME_PROBE="${KOVA_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${KOVA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE="${KOVA_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e KOVA_LIVE_CLI_BACKEND_MCP_PROBE="${KOVA_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e KOVA_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${KOVA_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e KOVA_LIVE_CLI_BACKEND_IMAGE_ARG="${KOVA_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e KOVA_LIVE_CLI_BACKEND_IMAGE_MODE="${KOVA_LIVE_CLI_BACKEND_IMAGE_MODE:-}")
