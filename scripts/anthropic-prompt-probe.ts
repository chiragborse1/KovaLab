...
// Usage:
// KOVA_PROMPT_TRANSPORT=direct|gateway
// KOVA_PROMPT_MODE=extra|override
// KOVA_PROMPT_TEXT='...'
// KOVA_PROMPT_CAPTURE=1
...
import { resolveKovaAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.KOVA_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.KOVA_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.KOVA_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.KOVA_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.KOVA_USER_PROMPT?.trim() || "is kova here?";
const ENABLE_CAPTURE = process.env.KOVA_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.KOVA_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.KOVA_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.KOVA_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.KOVA_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.KOVA_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.KOVA_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.KOVA_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing KOVA_PROMPT_TEXT or KOVA_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set KOVA_LIVE_SETUP_TOKEN_VALUE or KOVA_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["kova.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        KOVA_CONFIG_PATH: params.configPath,
        KOVA_STATE_DIR: params.stateDir,
        KOVA_AGENT_DIR: params.agentDir,
        KOVA_GATEWAY_TOKEN: params.gatewayToken,
        ...
        KOVA_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "kova.json");
