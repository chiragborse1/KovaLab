...
BASE_IMAGE="${BASE_IMAGE:-kova-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-kova-sandbox-common:bookworm-slim}"
...
KOVA_DOCKER_BUILD_USE_BUILDX="${KOVA_DOCKER_BUILD_USE_BUILDX:-0}"
KOVA_DOCKER_BUILD_CACHE_FROM="${KOVA_DOCKER_BUILD_CACHE_FROM:-}"
KOVA_DOCKER_BUILD_CACHE_TO="${KOVA_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=kova.sandbox=1)
NOTE
