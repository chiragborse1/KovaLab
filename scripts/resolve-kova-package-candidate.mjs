...
export const KOVA_PACKAGE_SPEC_RE =
  /^getkova@(beta|latest|((0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(-beta\.[1-9][0-9]*)?|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-beta\.[1-9][0-9]*)?))$/u;

function usage() {
  return `Usage: node scripts/resolve-kova-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateKovaPackageSpec(spec) {
  if (!KOVA_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Kova branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `kova-package-source-${process.pid}`,
  );
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-kova-for-docker.mjs",
        "--source-dir",
      ...
      validateKovaPackageSpec(options.packageSpec);
  ...
  await run("node", ["scripts/check-kova-package-tarball.mjs", target]);
