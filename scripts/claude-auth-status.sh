...
KOVA_AUTH="$HOME/.kova/agents/main/agent/auth-profiles.json"
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    kova_status=$(check_kova_auth 2>/dev/null || true)

    claude_expires=0
    kova_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        kova_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        kova_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$KOVA_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            kova: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$kova_status" == EXPIRED* ]] || [[ "$kova_status" == MISSING* ]]; then
        echo "KOVA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$kova_status" == EXPIRING* ]]; then
        echo "KOVA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Kova Auth (~/.kova/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$KOVA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$KOVA_AUTH" 2>/dev/null || echo "0")
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active kova >/dev/null 2>&1; then
