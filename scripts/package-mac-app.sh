...
# Build and bundle Kova into a minimal .app we can open.
# Outputs to dist/Kova.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Kova.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Kova"
MLX_TTS_HELPER_PRODUCT="kova-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.kova.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/chiragborse1/KovaLab/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Kova/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :KovaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :KovaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Kova"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Kova"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Kova"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Kova" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Kova/Resources/Kova.icns" "$APP_ROOT/Contents/Resources/Kova.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Kova/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
KOVAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/KovaKit_KovaKit.bundle"
if [ -d "$KOVAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/KovaKit_KovaKit.bundle"
  cp -R "$KOVAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/KovaKit_KovaKit.bundle"
else
  echo "WARN: KovaKit resource bundle not found at $KOVAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Kova"
killall -q Kova 2>/dev/null || true
