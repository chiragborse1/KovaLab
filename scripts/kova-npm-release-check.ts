...
const skipPackValidationEnv = "KOVA_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.bin?.kova !== "kova.mjs") {
    errors.push(`package.json bin.kova must be "kova.mjs"; found "${pkg.bin?.kova ?? ""}".`);
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`kova-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  const dayDistance =
    parsedVersion?.scheme === "calver" && parsedVersion.date
      ...
      : "n/a";
  console.log(
    `kova-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
