...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[kova] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[kova] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[kova] This usually means a file in node_modules is empty or truncated.");
  console.error("[kova] Recommended recovery:");
  console.error("[kova]   rm -rf node_modules");
  console.error("[kova]   pnpm store prune");
  console.error("[kova]   pnpm install");
  console.error("");
  console.error("[kova] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.KOVA_WATCH_MODE = "1";
  childEnv.KOVA_WATCH_SESSION = watchSession;
  ...
  childEnv.KOVA_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.KOVA_WATCH_COMMAND = deps.args.join(" ");
  }
