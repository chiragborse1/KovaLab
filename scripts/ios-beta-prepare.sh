...
- regenerates apps/ios/Kova.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${KOVA_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid KOVA_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid KOVA_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid KOVA_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid KOVA_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing KOVA_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(KOVA_URL_SLASH)$(KOVA_URL_SLASH)#g'
)"
...
KOVA_CODE_SIGN_STYLE = Automatic
KOVA_DEVELOPMENT_TEAM = ${TEAM_ID}
KOVA_IOS_SELECTED_TEAM = ${TEAM_ID}
KOVA_APP_BUNDLE_ID = ai.kova.client
KOVA_SHARE_BUNDLE_ID = ai.kova.client.share
KOVA_ACTIVITY_WIDGET_BUNDLE_ID = ai.kova.client.activitywidget
KOVA_WATCH_APP_BUNDLE_ID = ai.kova.client.watchkitapp
KOVA_WATCH_EXTENSION_BUNDLE_ID = ai.kova.client.watchkitapp.extension
KOVA_APP_PROFILE =
KOVA_SHARE_PROFILE =
KOVA_PUSH_TRANSPORT = relay
KOVA_PUSH_DISTRIBUTION = official
KOVA_URL_SLASH = /
KOVA_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
KOVA_PUSH_APNS_ENVIRONMENT = production
EOF
