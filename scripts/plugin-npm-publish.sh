...
if [[ "${KOVA_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${KOVA_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
build_package_runtime() {
  if [[ "${KOVA_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${KOVA_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
if [[ "${KOVA_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
