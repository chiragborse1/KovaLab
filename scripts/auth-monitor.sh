...
#
# Suggested cron: */30 * * * * /home/admin/kova/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Kova notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., kova-alerts)
...
STATE_FILE="$HOME/.kova/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Kova if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use kova
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Kova to $NOTIFY_PHONE..."
            kova send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Kova is down. Run: ssh l36 '~/kova/scripts/mobile-reauth.sh'" "urgent"
    exit 1
