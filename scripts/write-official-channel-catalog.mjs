...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.kova) ? packageJson.kova : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.kova?.channel?.id) || left.name;
    const rightId = trimString(right.kova?.channel?.id) || right.name;
