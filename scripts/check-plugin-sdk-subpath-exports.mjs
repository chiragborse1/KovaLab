...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("getkova/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced getkova/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced getkova/plugin-sdk/<subpath> must exist in the public package exports.",
  );
