...
const DEFAULT_WORKING_PROGRESS_TIMELINE = [
  { atMs: 0, line: "🛠️ checking gateway state" },
  { atMs: 2_000, line: "🔎 inspecting channel preview settings" },
  { atMs: 5_000, line: "🛠️ validating draft cleanup" },
  { atMs: 8_000, line: "Preview should clear before the final answer is sent." },
  { atMs: 11_000, line: "🧪 confirming durable final delivery" },
] as const;
...
function usage(): string {
  return [
    ...
    "",
    "Flows:",
    "  thinking-final      Thinking preview, then a final answer",
    "  working-final       Tool-progress preview, then a final answer",
    "",
    "Options:",
...
function createTelegramFlowApi(params: { accountId?: string; cfg: KovaConfig }): Bot["api"] {
  return {
...
export async function runTelegramThinkingFinalFlow(
  ...
  return { finalMessageId: final.messageId, previewUpdates: thinkingUpdates.length };
}
...
export async function runTelegramWorkingFinalFlow(
  ...
  return { finalMessageId: final.messageId, previewUpdates };
}
