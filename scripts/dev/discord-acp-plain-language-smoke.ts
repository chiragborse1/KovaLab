...
type DriverMode = "token" | "webhook" | "kova";
...
function resolveStateDir(): string {
  const override = process.env.KOVA_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.KOVA_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver kova] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Kova spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|kova> Driver transport mode (default: token)\n" +
    ...
    "  --kova-bin <path>        Kova CLI binary for driver=kova (default: kova)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  KOVA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  KOVA_DISCORD_SMOKE_DRIVER\n" +
    "  KOVA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  KOVA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  KOVA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  KOVA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  KOVA_DISCORD_SMOKE_AGENT\n" +
    "  KOVA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  KOVA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  KOVA_DISCORD_SMOKE_POLL_MS\n" +
    "  KOVA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  KOVA_DISCORD_SMOKE_KOVA_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.KOVA_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw = resolveArg("--driver") || process.env.KOVA_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "kova"
        ? "kova"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.KOVA_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.KOVA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.KOVA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.KOVA_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.KOVA_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.KOVA_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.KOVA_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.KOVA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.KOVA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.KOVA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const kovaBin = resolveArg("--kova-bin") || process.env.KOVA_DISCORD_SMOKE_KOVA_BIN || "kova";
...
async function kovaCliJson<T>(params: { kovaBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.kovaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`kova ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithKova(params: {
  kovaBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await kovaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    kovaBin: params.kovaBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "kova") {
    return await readMessagesWithKova({
      kovaBin: params.args.kovaBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `kova-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("kova message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "kova"
            ? await readMessagesWithKova({
                kovaBin: args.kovaBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Kova.`,
        diagnostics: {
          bindingCandidates: [
            {
