...
const RELEASE_HEADING_PATTERN =
  /^##\s+([0-9]+\.[0-9]+\.[0-9]+(?:-(?:alpha|beta|rc)\.[0-9]+|-[0-9A-Za-z.-]+)?)(?:\s+.*)?$/u;
const RELEASE_VERSION_PATTERN =
  /^([0-9]+\.[0-9]+\.[0-9]+)(?:-(?:alpha|beta|rc)\.[0-9]+|-[0-9A-Za-z.-]+)?$/u;
const PRERELEASE_VERSION_PATTERN = /^[0-9]+\.[0-9]+\.[0-9]+-(?:alpha|beta|rc)\.[0-9]+$/u;

export function resolvePackageChangelogVersions(packageVersion) {
  const match = RELEASE_VERSION_PATTERN.exec(packageVersion);
  if (!match) {
    throw new Error(`Unsupported Kova package version for changelog packaging: ${packageVersion}`);
  }
