...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/kova/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Kova Re-Auth" -c "After getting key, run: ssh $SERVER '~/kova/scripts/mobile-reauth.sh'" --id kova-auth
        ;;
    *)
