...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Kova")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Kova")
...
DMG_TEMP="$(mktemp -d /tmp/kova-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Kova/Resources/Kova.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/kova-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/kova-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/kova-dmg-limits.txt
