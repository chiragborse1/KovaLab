...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs, packageJson) {
  ...
  return env.KOVA_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "kova.plugin.json");
    ...
    if (packageJson.kova && "extensions" in packageJson.kova) {
      packageJson.kova = {
        ...packageJson.kova,
        extensions: rewritePackageExtensions(packageJson.kova.extensions),
        ...(typeof packageJson.kova.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.kova.setupEntry) }
          : {}),
      };
    }
