#!/bin/bash
# Setup Kova Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'kova-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Kova message: Send warning via Kova itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/kova-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/kova-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/kova-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now kova-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/KovaAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/KovaAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Kova*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (KovaAuth) shows status and opens auth URL if needed."
echo "The full widget (KovaAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status kova-auth-monitor.timer"
echo ""
echo "Quick commands:"
