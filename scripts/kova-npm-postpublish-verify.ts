...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./kova-npm-release-check.ts";
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32" ? join(prefixDir, "kova.cmd") : join(prefixDir, "bin", "kova");
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed Kova binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`kova-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error("Usage: node --import tsx scripts/kova-npm-postpublish-verify.ts <version>");
  }
  ...
  console.log(`kova-npm-postpublish-verify: verified published npm install paths for ${version}.`);
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`kova-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
