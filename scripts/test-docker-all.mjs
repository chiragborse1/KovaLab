...
// Builds shared Docker images, prepares one Kova npm tarball, assigns lanes
...
const DEFAULT_GITHUB_WORKFLOW = "kova-live-and-e2e-checks-reusable.yml";
...
function resourceLimitEnvName(resource) {
  return `KOVA_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.KOVA_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "KOVA_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.KOVA_DOCKER_BUILD_EXTENSIONS ?? env.KOVA_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.KOVA_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRerunCommand(laneNames, ref) {
  const fields = [
    "gh workflow run",
    shellQuote(process.env.KOVA_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
    "-f",
    `ref=${shellQuote(ref)}`,
    "-f",
  ...
  if (process.env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        process.env.KOVA_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (process.env.KOVA_DOCKER_E2E_BARE_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_bare_image=${shellQuote(process.env.KOVA_DOCKER_E2E_BARE_IMAGE)}`,
    );
  }
  if (process.env.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(process.env.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane ? e2eImageForLane(poolLane, baseEnv) : baseEnv.KOVA_DOCKER_E2E_IMAGE;
  const env = [
    ["KOVA_DOCKER_ALL_LANES", name],
    ["KOVA_DOCKER_ALL_BUILD", build],
    ["KOVA_DOCKER_ALL_PREFLIGHT", "0"],
    ["KOVA_SKIP_DOCKER_BUILD", "1"],
    ["KOVA_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_IMAGE],
    ["KOVA_DOCKER_E2E_BARE_IMAGE", baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE],
    ["KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE", baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE],
    ["KOVA_CURRENT_PACKAGE_TGZ", baseEnv.KOVA_CURRENT_PACKAGE_TGZ],
  ];
  if (baseEnv.KOVA_DOCKER_ALL_PNPM_COMMAND) {
    env.push(["KOVA_DOCKER_ALL_PNPM_COMMAND", baseEnv.KOVA_DOCKER_ALL_PNPM_COMMAND]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.KOVA_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
async function writeRunSummary(logDir, summary) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: process.env.KOVA_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        process.env.GITHUB_SERVER_URL && process.env.GITHUB_REPOSITORY && process.env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: process.env.KOVA_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary) {
  const ref =
    ...
    process.env.KOVA_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: process.env.KOVA_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:kova-(?:gateway-e2e|openwebui|openwebui-gateway|config-reload-e2e)-)/.test(name),
    );
}
...
async function prepareKovaPackage(baseEnv, logDir) {
  const existing = baseEnv.KOVA_CURRENT_PACKAGE_TGZ;
  if (existing) {
    const packageTgz = path.resolve(existing);
    baseEnv.KOVA_CURRENT_PACKAGE_TGZ = packageTgz;
    baseEnv.KOVA_BUNDLED_CHANNEL_HOST_BUILD = "0";
    baseEnv.KOVA_NPM_ONBOARD_HOST_BUILD = "0";
    console.log(`==> Kova package: ${packageTgz}`);
    return;
  }

  const packDir = path.join(logDir, "kova-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "kova-current.tgz");
  await runForeground(
    "Prepare Kova package once",
    `node scripts/package-kova-for-docker.mjs --output-dir ${shellQuote(packDir)} --output-name kova-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.KOVA_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.KOVA_NPM_ONBOARD_HOST_BUILD = "0";
  console.log(`==> Kova package: ${baseEnv.KOVA_CURRENT_PACKAGE_TGZ}`);
}

function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  if (image) {
    env.KOVA_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.KOVA_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.KOVA_DOCKER_CLI_TOOLS_DIR) {
    env.KOVA_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.KOVA_DOCKER_CACHE_HOME_DIR) {
    env.KOVA_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.KOVA_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.KOVA_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.KOVA_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.KOVA_DOCKER_CACHE_HOME_DIR}`,
      `==> [${name}] timeout: ${timeoutMs}ms`,
      `==> [${name}] retries: ${lane.retries ?? 0}`,
      `==> [${name}] e2e image kind: ${lane.e2eImageKind ?? "none"}`,
      `==> [${name}] e2e image: ${env.KOVA_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.KOVA_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune KOVA_DOCKER_ALL_PARALLELISM, KOVA_DOCKER_ALL_WEIGHT_LIMIT, or KOVA_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.KOVA_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "KOVA_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.KOVA_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "KOVA_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.KOVA_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "KOVA_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.KOVA_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "KOVA_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.KOVA_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "KOVA_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.KOVA_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "KOVA_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.KOVA_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "KOVA_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.KOVA_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.KOVA_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.KOVA_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.KOVA_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.KOVA_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.KOVA_DOCKER_ALL_BUILD, true);
  const planJson =
    cliArgs.has("--plan-json") || parseBool(process.env.KOVA_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.KOVA_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.KOVA_DOCKER_ALL_PROFILE);
  const releaseChunk = process.env.KOVA_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.KOVA_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.KOVA_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("KOVA_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.KOVA_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.KOVA_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "KOVA_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.KOVA_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.KOVA_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.KOVA_DOCKER_ALL_LOG_DIR || path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    KOVA_DOCKER_E2E_BARE_IMAGE:
      process.env.KOVA_DOCKER_E2E_BARE_IMAGE ||
      process.env.KOVA_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.KOVA_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.KOVA_DOCKER_E2E_IMAGE =
    process.env.KOVA_DOCKER_E2E_IMAGE || baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE}`);
  console.log(`==> Docker E2E functional image: ${baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE}`);
  ...
  console.log(`==> Live-test bundled plugin deps: ${baseEnv.KOVA_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (lanesNeedKovaPackage(scheduledLanes)) {
    await runPhase(phases, "prepare-kova-package", {}, async () => {
      await prepareKovaPackage(baseEnv, logDir);
    });
  } else {
    console.log("==> Kova package: not needed for selected lanes");
  }
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          KOVA_DOCKER_E2E_IMAGE: baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE,
          KOVA_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          KOVA_DOCKER_E2E_IMAGE: baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE,
          KOVA_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeRunSummary(logDir, {
      ...
      failures,
      image: baseEnv.KOVA_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.KOVA_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.KOVA_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      phases,
      profile,
