...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "kova-startup-memory-"));
...
const MAX_RSS_MARKER = "__KOVA_MAX_RSS_KB__=";
...
const cases = [
  {
    ...
    label: "--help",
    args: ["kova.mjs", "--help"],
    limitMb: Number(process.env.KOVA_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["kova.mjs", "status", "--json"],
    limitMb: Number(process.env.KOVA_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["kova.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.KOVA_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Kova startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
