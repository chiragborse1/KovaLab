...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.KOVA_VITEST_MAX_WORKERS || env.KOVA_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      KOVA_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.KOVA_TEST_PROJECTS_PARALLEL &&
        !baseEnv.KOVA_VITEST_MAX_WORKERS &&
        !baseEnv.KOVA_TEST_WORKERS &&
