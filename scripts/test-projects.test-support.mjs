...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-kova-runtime)?\.ts$/u,
...
const BROAD_CHANGED_ENV_KEY = "KOVA_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "KOVA_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "KOVA_VITEST_NO_OUTPUT_RETRY";
...
function isSkippedImportGraphDirectory(name) {
  return (
    ...
    name === "node_modules" ||
    name === "vendor" ||
    name.startsWith(".kova-runtime-deps")
  );
}
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(process.env.KOVA_TEST_PROJECTS_PARALLEL ?? "", 10);
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.KOVA_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(env.KOVA_VITEST_MAX_WORKERS ?? env.KOVA_TEST_WORKERS);
...
export function resolveParallelFullSuiteConcurrency(specCount, env = process.env, hostInfo) {
  const override = parsePositiveInt(env.KOVA_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.KOVA_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.KOVA_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `kova-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.KOVA_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.KOVA_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
