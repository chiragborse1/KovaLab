 ...
 * `kova update`. Use it to verify the completion installation flow
...
\`kova update\`, \`kova doctor\`, and \`kova onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Kova Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`kova completion --install\` later to enable.`));
    return;
  }
