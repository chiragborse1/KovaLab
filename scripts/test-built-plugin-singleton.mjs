...
const { clearPluginCommands, getPluginCommandSpecs, loadKovaPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadKovaPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kova-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@kovaai/build-smoke-plugin",
      type: "module",
      kova: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "kova.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'getkova/plugin-sdk';",
...
const registry = loadKovaPlugins({
  ...
  env: {
    ...process.env,
    KOVA_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
