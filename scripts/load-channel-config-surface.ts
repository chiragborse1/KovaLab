...
function copyModuleImportGraphWithoutNodeModules(params: {
  ...
  const tempParent = path.join(params.repoRoot, ".kova-config-doc-cache");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.KOVA_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing KOVA_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        KOVA_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "getkova/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
