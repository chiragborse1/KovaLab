...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      KOVA_GATEWAY_TOKEN|KOVA_PODMAN_CONTAINER|KOVA_PODMAN_IMAGE|KOVA_IMAGE|KOVA_PODMAN_PULL|KOVA_PODMAN_GATEWAY_HOST_PORT|KOVA_GATEWAY_PORT|KOVA_PODMAN_BRIDGE_HOST_PORT|KOVA_BRIDGE_PORT|KOVA_GATEWAY_BIND|KOVA_PODMAN_USERNS|KOVA_BIND_MOUNT_OPTIONS|KOVA_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-kova-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${KOVA_CONFIG_DIR:-$EFFECTIVE_HOME/.kova}"
ENV_FILE="${KOVA_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${KOVA_CONFIG_DIR:-$EFFECTIVE_HOME/.kova}"
ENV_FILE="${KOVA_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${KOVA_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${KOVA_PODMAN_CONTAINER:-kova}"
KOVA_IMAGE="${KOVA_PODMAN_IMAGE:-${KOVA_IMAGE:-kova:local}}"
PODMAN_PULL="${KOVA_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${KOVA_PODMAN_GATEWAY_HOST_PORT:-${KOVA_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${KOVA_PODMAN_BRIDGE_HOST_PORT:-${KOVA_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${KOVA_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$KOVA_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  KOVA_CONTAINER="" KOVA_CONFIG_DIR="$config_dir" \
    kova config get gateway.bind 2>/dev/null || true
}
...
# KOVA_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${KOVA_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
if [[ -z "${KOVA_GATEWAY_TOKEN:-}" ]]; then
  export KOVA_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "KOVA_GATEWAY_TOKEN" "$KOVA_GATEWAY_TOKEN"
  echo "Generated KOVA_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/kova.json"
...
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported KOVA_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (KOVA_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${KOVA_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${KOVA_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$KOVA_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.kova/.npm \
    -e KOVA_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.kova:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.kova/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$KOVA_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME kova dashboard --no-open"
