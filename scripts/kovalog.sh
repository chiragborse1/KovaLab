...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "kovalog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
kovalog - Kova Logging Utility

USAGE:
    kovalog [OPTIONS]

DESCRIPTION:
    View Kova logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Kova logs flow through the macOS unified log (subsystem: ai.kova).
...
QUICK START:
    kovalog -n 100             Show last 100 lines from all components
    kovalog -f                 Follow logs in real-time
    kovalog -e                 Show only errors
    kovalog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    kovalog                   Show last 50 lines from past 5 minutes (default)
    kovalog -f                Stream logs continuously
    kovalog -n 100            Show last 100 lines
    kovalog -e                Show only recent errors
    kovalog -l 30m -n 200     Show last 200 lines from past 30 minutes
    kovalog -c ServerManager  Show recent ServerManager logs
    kovalog -s "fail"         Search for "fail" in recent logs
    kovalog --server -e       Show recent server errors
    kovalog -f -d             Stream debug logs continuously
