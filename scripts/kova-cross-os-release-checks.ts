...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.KOVA_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.KOVA_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(env.VAR_MACOS_RUNNER, env.KOVA_RELEASE_CHECKS_MACOS_RUNNER),
  };
}
...
async function main(argv) {
  ...
  const summary = {
    ...
    runnerOs: process.env.KOVA_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.KOVA_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        KOVA_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    KOVA_HOME: lane.homeDir,
    KOVA_STATE_DIR: lane.stateDir,
    KOVA_CONFIG_PATH: join(lane.stateDir, "kova.json"),
    KOVA_DISABLE_BONJOUR: "1",
    KOVA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.KOVA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}
...
Write-Output "__KOVA_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__KOVA_PATH__="),
    );
    if (!cliPath) {
  ...
  const cliPath = parseMarkerLine(result.stdout, "__KOVA_PATH__=");
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `kova update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.kova/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.KOVA_DISCORD_SMOKE_BOT_TOKEN?.trim() || process.env.DISCORD_BOT_TOKEN?.trim() || "";
  const guildId = process.env.KOVA_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.KOVA_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.KOVA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
const startedPath = process.env.KOVA_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.KOVA_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.KOVA_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing KOVA_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "KOVA_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.KOVA_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.KOVA_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...params.env,
      KOVA_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href,
      KOVA_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      KOVA_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Kova Control</title>") &&
          html.includes("<kova-app></kova-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Kova Control</title>")} app=${html.includes("<kova-app></kova-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
