...
import { withAgentModelAliases } from "getkova/plugin-sdk/provider-onboard";
import {
...
} from "getkova/plugin-sdk/provider-setup";
import { WizardCancelledError, type WizardPrompter } from "getkova/plugin-sdk/setup";
import {
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.KOVA_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: KovaConfig): KovaConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<KovaConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
