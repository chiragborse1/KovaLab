import { CUSTOM_LOCAL_AUTH_MARKER } from "getkova/plugin-sdk/provider-auth";
...
import { resolveAgentModelPrimaryValue } from "getkova/plugin-sdk/provider-onboard";
import {
...
vi.mock("getkova/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("getkova/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): KovaConfig {
  return {
    models: {
      providers: {
        lmstudio: {
