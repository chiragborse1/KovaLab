...
import { encodePngRgba, fillPixel } from "getkova/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "getkova/plugin-sdk/plugin-test-runtime";
import {
  expectKovaLiveTranscriptMarker,
  runRealtimeSttLiveTest,
} from "getkova/plugin-sdk/provider-test-contracts";
import { getRuntimeConfig } from "getkova/plugin-sdk/runtime-config-snapshot";
import { isBillingErrorMessage } from "getkova/plugin-sdk/test-env";
import { describe, expect, it } from "vitest";
import plugin from "./index.js";
import { XAI_DEFAULT_STT_MODEL } from "./stt.js";

const XAI_API_KEY = process.env.XAI_API_KEY ?? "";
const LIVE_IMAGE_MODEL = process.env.KOVA_LIVE_XAI_IMAGE_MODEL?.trim() || "grok-imagine-image";
const liveEnabled = XAI_API_KEY.trim().length > 0 && process.env.KOVA_LIVE_TEST === "1";
const describeLive = liveEnabled ? describe : describe.skip;
const EMPTY_AUTH_STORE = { version: 1, profiles: {} } as const;

function createLiveConfig(): KovaConfig {
...
describeLive("xai plugin live", () => {
  it("synthesizes TTS through the registered speech provider", async () => {
    await runXaiLiveCase("tts", async () => {
      ...
      const audioFile = await speechProvider.synthesize({
        text: "Kova xAI text to speech integration test OK.",
        cfg,
        providerConfig: {
          apiKey: XAI_API_KEY,
          baseUrl: "https://api.x.ai/v1",
      ...
      const telephony = await speechProvider.synthesizeTelephony?.({
        text: "Kova xAI telephony check OK.",
        cfg,
        providerConfig: {
          apiKey: XAI_API_KEY,
          baseUrl: "https://api.x.ai/v1",
  ...
  it("transcribes audio through the registered media provider", async () => {
    await runXaiLiveCase("stt", async () => {
      ...
      const speechProvider = requireRegisteredProvider(speechProviders, "xai");
      const cfg = createLiveConfig();
      const phrase = "Kova xAI speech to text integration test OK.";
      ...
      expectKovaLiveTranscriptMarker(normalized);
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    await runXaiLiveCase("realtime-stream", async () => {
      ...
      const speechProvider = requireRegisteredProvider(speechProviders, "xai");
      const cfg = createLiveConfig();
      const phrase = "Kova xAI realtime transcription integration test OK.";
