...
describe("xAI OAuth", () => {
  ...
  it("builds the xAI authorize URL for Kova", () => {
    const url = new URL(
      buildXaiOAuthAuthorizeUrl({
    ...
    expect(url.searchParams.get("referrer")).toBe("kova");
  ...
  it("validates discovered endpoints before using them", async () => {
    ...
    expect(discoveryHeaders.get("user-agent")).toBe("kova/2026.3.22");
    vi.unstubAllEnvs();
  ...
  it("refreshes with the cached token endpoint and preserves refresh fallback", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (_url, init) => {
      ...
      expect(headers.get("user-agent")).toBe("kova/2026.3.22");
      return jsonResponse({
