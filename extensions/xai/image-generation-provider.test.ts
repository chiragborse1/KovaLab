...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.KOVA_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `kova/${version}`);
      headers.set("originator", "kova");
...
vi.mock("getkova/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("getkova/plugin-sdk/provider-http", () => ({
...
vi.mock("getkova/plugin-sdk/string-coerce-runtime", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/kova-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/kova-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("kova/2026.3.22");
    expect(request.headers?.get("originator")).toBe("kova");
