...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "getkova/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "getkova/plugin-sdk/provider-stream-shared";
import { jsonResult } from "getkova/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          message:
            "code_execution needs xAI credentials. Run `kova onboard --auth-choice xai-oauth` to sign in with Grok, run `kova onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`.",
          docs: "https://docs.neuralstudio.in/tools/code-execution",
        });
      }
