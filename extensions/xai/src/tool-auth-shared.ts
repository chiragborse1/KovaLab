...
function readLegacyGrokFallbackAuth(cfg?: KovaConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: KovaConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiApiKey(cfg?: KovaConfig): string | undefined {
