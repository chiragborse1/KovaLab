// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatKovaUserAgent`.

import { KOVA_VERSION as PACKAGE_VERSION } from "getkova/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "kova";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.KOVA_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.KOVA_SERVICE_VERSION) ??
...
// an empty record so the kova identity is not forwarded to the proxy.
