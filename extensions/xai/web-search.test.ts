import { createTestWizardPrompter } from "getkova/plugin-sdk/plugin-test-runtime";
...
import { withEnv, withEnvAsync, withFetchPreconnect } from "getkova/plugin-sdk/test-env";
...
vi.mock("getkova/plugin-sdk/provider-web-search", async (importOriginal) => {
  const original = await importOriginal<typeof import("getkova/plugin-sdk/provider-web-search")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      const result = await maybeTool.execute({ query: "Kova" });
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool.execute({ query: "Kova Grok proxy test" });
  ...
  it("reports malformed xAI web search JSON as a provider error", async () => {
    ...
    await expect(tool.execute({ query: "Kova" })).rejects.toThrow(
