...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): KovaConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as KovaConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): KovaConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as KovaConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: KovaConfig, accountId = "media") {
    await handleTelegramAction(
