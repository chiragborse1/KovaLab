...
import * as runtimeEnvModule from "getkova/plugin-sdk/runtime-env";
import { withEnv } from "getkova/plugin-sdk/testing";
...
function resolveAccountWithEnv(env: Record<string, string>, cfg: KovaConfig, accountId?: string) {
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", KOVA_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): KovaConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): KovaConfig => ({
    channels: {
      telegram: {
