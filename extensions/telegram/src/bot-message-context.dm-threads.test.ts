...
const { recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => ({
  ...
  resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/kova-session-store.json"),
}));
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("getkova/plugin-sdk/config-runtime");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/kova-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-telegram-topic-name-"));
