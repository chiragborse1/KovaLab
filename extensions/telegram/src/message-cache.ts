...
function normalizeMessageNode(
  ...
  const threadId = normalizeTelegramCacheThreadId(params.threadId ?? resolveMessageThreadId(msg));
  return {
    sourceMessage: msg,
...
function resolveMessageThreadId(msg: Message): number | undefined {
  return normalizeTelegramCacheThreadId((msg as { message_thread_id?: unknown }).message_thread_id);
}
...
function parsePersistedNode(value: unknown): TelegramCachedMessageNode | null {
  ...
  const threadId = normalizeTelegramCacheThreadId(readOptionalString(value, "threadId"));
...
function replaceFileAtomicSync(params: { filePath: string; content: string; tempPrefix: string }) {
  fs.mkdirSync(path.dirname(params.filePath), { recursive: true });
  const tempPath = path.join(
    path.dirname(params.filePath),
    `${params.tempPrefix}.${process.pid}.${Date.now()}.tmp`,
  );
...
function appendPersistedMessage(params: {
  ...
  fs.appendFileSync(persistedPath, serializePersistedEntry(params.key, params.node));
  return 1;
}
