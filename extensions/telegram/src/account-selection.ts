...
function resolveDefaultAgentId(cfg: KovaConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: KovaConfig): string[] {
...
function listBoundAccountIds(cfg: KovaConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: KovaConfig, channelId: string): string | null {
...
export function resolveDefaultTelegramAccountSelection(cfg: KovaConfig): {
  accountId: string;
  accountIds: string[];
