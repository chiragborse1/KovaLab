...
import { resolveStorePath, type KovaConfig } from "getkova/plugin-sdk/config-runtime";
import { logVerbose } from "getkova/plugin-sdk/runtime-env";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("kova.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<KovaConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<KovaConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<KovaConfig, "session">): SentMessageStore {
