...
describe("telegram message cache", () => {
  it("hydrates reply chains from persisted cached messages", async () => {
    const storePath = `/tmp/kova-telegram-message-cache-${process.pid}-${Date.now()}.json`;
  ...
  it("shares one persisted bucket across live cache instances", async () => {
    const storePath = `/tmp/kova-telegram-message-cache-shared-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
      const firstCache = createTelegramMessageCache({ persistedPath });
  ...
  it("appends cached records between compactions and reloads the bounded cache window", async () => {
    const storePath = `/tmp/kova-telegram-message-cache-append-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
  ...
  it("keeps the persisted log bounded by compacting cached records", async () => {
    const storePath = `/tmp/kova-telegram-message-cache-compact-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
  ...
  it("loads mixed legacy array caches and rewrites them as line-delimited entries", async () => {
    const storePath = `/tmp/kova-telegram-message-cache-legacy-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
      const legacyEntries = [
  ...
  it("does not select messages before the latest session reset command", () => {
    ...
    const staleInstruction = "okay so we just flip in kova? if yes do it up";
    const record = (params: {
      id: number;
      text: string;
      timestampMs: number;
  ...
  it("does not select messages before session start timestamp", () => {
    const cache = createTelegramMessageCache();
    const sessionStartedAt = Date.parse("2026-05-10T17:30:43.980Z");
    const staleInstruction = "old instructions";
    for (const [id, text, timestampMs] of [
      [100, staleInstruction, sessionStartedAt - 1000],
      [101, "fresh context", sessionStartedAt],
      [102, "new question", sessionStartedAt + 1000],
    ] as const) {
      cache.record({
        accountId: "default",
        chatId: 7,
        msg: {
          chat: { id: 7, type: "group", title: "Ops" },
          message_id: id,
          date: Math.floor(timestampMs / 1000),
          text,
          from: { id, is_bot: false, first_name: `User ${id}` },
        } as Message,
      });
    ...
    expect(context.map((entry) => entry.node.messageId)).toEqual(["101"]);
    expect(context.map((entry) => entry.node.body)).not.toContain(staleInstruction);
  });
});
