...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<KovaConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<KovaConfig["channels"]>["telegram"]>>,
): KovaConfig {
  return {
    channels: {
      telegram: {
