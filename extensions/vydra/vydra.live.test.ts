...
const ENABLE_VYDRA_VIDEO_LIVE = process.env.KOVA_LIVE_VYDRA_VIDEO === "1";
const LIVE_IMAGE_MODEL = process.env.KOVA_LIVE_VYDRA_IMAGE_MODEL?.trim() || "grok-imagine";
const LIVE_VIDEO_MODEL = process.env.KOVA_LIVE_VYDRA_VIDEO_MODEL?.trim() || "veo3";
const DEFAULT_LIVE_KLING_IMAGE_URL =
  "https://raw.githubusercontent.com/chiragborse1/KovaLab/main/docs/assets/showcase/roof-camera-sky.jpg";
const LIVE_KLING_IMAGE_URL =
  process.env.KOVA_LIVE_VYDRA_KLING_IMAGE_URL?.trim() || DEFAULT_LIVE_KLING_IMAGE_URL;
...
describe.skipIf(!LIVE || !VYDRA_API_KEY)("vydra live", () => {
  it("generates an image through the registered provider", async () => {
    ...
    const result = await provider.generateImage({
      ...
      agentDir: "/tmp/kova-live-vydra-image",
    });
  ...
  it.skipIf(!ENABLE_VYDRA_VIDEO_LIVE)(
    ...
    async () => {
      ...
      const result = await provider.generateVideo({
        ...
        cfg: { plugins: { enabled: true } } as never,
        agentDir: "/tmp/kova-live-vydra-video",
      });
  ...
  it.skipIf(!ENABLE_VYDRA_VIDEO_LIVE)(
    ...
    async () => {
      ...
      const result = await provider.generateVideo({
        ...
        cfg: { plugins: { enabled: true } } as never,
        agentDir: "/tmp/kova-live-vydra-kling",
