...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.KOVA_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "kova",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/kova" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    kova: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchKovaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveKovaUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopKovaChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.KOVA_GATEWAY_PORT;
  process.env.KOVA_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.KOVA_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.KOVA_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.KOVA_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.KOVA_GATEWAY_PASSWORD;
  } else {
    process.env.KOVA_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
