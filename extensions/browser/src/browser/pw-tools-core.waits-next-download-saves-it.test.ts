...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredKovaTmpDir: vi.fn(() => "/tmp/kova"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-kova-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-kova-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredKovaTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredKovaTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredKovaTmpDir.mockReturnValue("/tmp/kova");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".kova-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredKovaTmpDir.mockReturnValue("/tmp/kova-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "kova-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "kova-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredKovaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "kova-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "kova-preferred", "downloads")}${path.sep}`),
    );
  });
