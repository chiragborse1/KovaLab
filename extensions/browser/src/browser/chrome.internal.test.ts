...
import {
  buildKovaChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchKovaChrome,
  resolveKovaUserDataDir,
  stopKovaChrome,
} from "./chrome.js";
 ...
 * not exercise: launchKovaChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopKovaChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildKovaChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveKovaUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveKovaUserDataDir();
      expect(dir.endsWith(path.join("kova", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveKovaUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildKovaChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildKovaChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchKovaChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchKovaChrome(resolved, profile);
  ...
  describe("launchKovaChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "kova-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchKovaChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchKovaChrome(makeResolved(), profile);
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      ...
      const userDataDir = resolveKovaUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchKovaChrome(resolved, profile)).rejects.toThrow(
    ...
    it("uses the configured local launch timeout while waiting for CDP discovery", async () => {
      ...
      try {
        ...
        const rejection = expect(launchKovaChrome(resolved, profile)).rejects.toThrow(
          /Failed to start Chrome CDP/,
        );
  ...
  describe("stopKovaChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopKovaChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopKovaChrome>[0],
        1,
      );
  ...
  describe("launchKovaChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "kova-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveKovaUserDataDir(profileName));
        ...
        const staged = resolveKovaUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_KOVA_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchKovaChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchKovaChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateKovaProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateKovaProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateKovaProfile }, "decorateKovaProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".kova-profile-decorated") || s.endsWith("Preferences")) {
