...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      defaultProfile: "kova",
      profiles: {
        kova: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.KOVA_GATEWAY_PORT;
    process.env.KOVA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.KOVA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.KOVA_GATEWAY_PASSWORD;
