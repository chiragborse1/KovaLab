...
export const KOVA_BROWSER_HEADLESS_ENV = "KOVA_BROWSER_HEADLESS";
const LEGACY_BROWSER_HEADLESS_ENV = "KOVA_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_KOVA_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_KOVA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_KOVA_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_KOVA_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_KOVA_BROWSER_PROFILE_NAME]
        ? DEFAULT_KOVA_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "kova";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(
    env[KOVA_BROWSER_HEADLESS_ENV] ?? env[LEGACY_BROWSER_HEADLESS_ENV],
  );
...
export function getManagedBrowserMissingDisplayError(
  ...
  return (
    ...
    `Set ${KOVA_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
