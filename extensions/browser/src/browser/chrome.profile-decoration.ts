...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".kova-profile-decorated");
}
 ...
 */
export function decorateKovaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_KOVA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_KOVA_BROWSER_COLOR).toUpperCase();
