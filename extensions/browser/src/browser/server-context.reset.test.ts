...
function localKovaProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "kova",
...
function createLocalKovaResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localKovaProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveKovaUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kova-reset-"));
    const profileDir = path.join(tempRoot, "kova");
    ...
    const ops = createLocalKovaResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveKovaUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kova-reset-no-own-"));
    const profileDir = path.join(tempRoot, "kova");
    fs.mkdirSync(profileDir, { recursive: true });
