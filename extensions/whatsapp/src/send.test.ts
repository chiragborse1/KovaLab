...
let resetLogger: typeof import("getkova/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("getkova/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: KovaConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("getkova/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `kova-outbound-${crypto.randomUUID()}.log`);
