...
const hoisted = vi.hoisted(() => ({
  listWhatsAppAccountIds: vi.fn((cfg: KovaConfig) => {
  ...
  resolveWhatsAppAccount: vi.fn(
    ({ cfg, accountId }: { cfg: KovaConfig; accountId?: string | null }) => ({
      enabled:
...
vi.mock("./channel-actions.runtime.js", async () => {
  return {
    ...
    resolveWhatsAppReactionLevel: ({ cfg, accountId }: { cfg: KovaConfig; accountId?: string }) => {
      const accountLevel =
        accountId == null
          ? undefined
...
describe("whatsapp channel action helpers", () => {
  ...
  it("returns null when WhatsApp is not configured", () => {
    expect(
      describeWhatsAppMessageActions({ cfg: {} as KovaConfig, accountId: "default" }),
    ).toBeNull();
  });
