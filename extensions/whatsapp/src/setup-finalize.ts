...
import { formatCliCommand, formatDocsLink } from "getkova/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<KovaConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: KovaConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: KovaConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: KovaConfig, accountId: string): string {
  if (
...
function setWhatsAppDmPolicy(cfg: KovaConfig, accountId: string, dmPolicy: DmPolicy): KovaConfig {
...
export async function detectWhatsAppLinked(cfg: KovaConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Kova" },
    ],
  });
