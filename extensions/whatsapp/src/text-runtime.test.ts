...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", async () => {
    await withTempDir("kova-state-", async (stateDir) => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("kova-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("kova-lid-a-", async (first) => {
      await withTempDir("kova-lid-b-", (second) => {
