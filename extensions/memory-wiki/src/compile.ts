...
const AGENT_DIGEST_PATH = ".kova-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".kova-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- kova:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- kova:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- kova:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- kova:wiki:${group.dir}:index:end -->`,
