...
description: Maintain the Kova memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `kova wiki ingest`, `kova wiki compile`, and `kova wiki lint` as the default maintenance loop.
- In `bridge` mode, run `kova wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `kova wiki unsafe-local import` only when the user explicitly opted into private local path access.
