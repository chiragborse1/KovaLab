...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Kova**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.kova-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.kova-wiki/cache/agent-digest.json` and `.kova-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
