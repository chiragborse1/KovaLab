    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kova memory status", "Show index and provider status."],
          [
            "kova memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["kova memory status --deep", "Probe embedding provider readiness."],
          ["kova memory index --force", "Force a full reindex."],
          ['kova memory search "meeting notes"', "Quick search using positional query."],
          [
          ...
          [
            "kova memory rem-backfill --path ./memory",
          ...
          ["kova memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.neuralstudio.in/cli/memory")}\n`,
    );
