...
import { resolveMemoryRemDreamingConfig } from "getkova/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: KovaConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredKovaTmpDir(), "kova-rem-harness-"),
  );
...
function resolveAgent(cfg: KovaConfig, agent?: string) {
...
function resolveAgentIds(cfg: KovaConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: kova memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredKovaTmpDir(), "kova-rem-backfill-"),
      );
      try {
