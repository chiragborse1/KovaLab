...
import { formatMemoryDreamingDay } from "getkova/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "getkova/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "kova-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*kova-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*kova-memory-promotion:([^\n]+?)\s*-->/gi);
