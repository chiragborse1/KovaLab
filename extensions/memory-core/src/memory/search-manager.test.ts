...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "getkova/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("getkova/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): KovaConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("kova.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kova-qmd-workspace-"));
  ...
  it("closes only the requested agent qmd manager on scoped teardown", async () => {
    ...
    const otherPrimary = createManagerMock({
      backend: "qmd",
      provider: "qmd",
      model: "qmd",
      requestedProvider: "qmd",
