...
import { normalizeOptionalLowercaseString, sleep } from "getkova/plugin-sdk/text-runtime";
import { loadWebMedia } from "getkova/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, KovaConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as KovaConfig,
      channel: "msteams",
    });
