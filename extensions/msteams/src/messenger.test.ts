...
import { SILENT_REPLY_TOKEN } from "getkova/plugin-sdk/reply-chunking";
import type { PluginRuntime } from "getkova/plugin-sdk/runtime-store";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolvePreferredKovaTmpDir } from "../../../src/infra/tmp-kova-dir.js";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredKovaTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredKovaTmpDir(), "msteams-retry-"));
