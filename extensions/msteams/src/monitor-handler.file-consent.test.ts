  ...
  return {
    ...
    state: {
      resolveStateDir: (env?: NodeJS.ProcessEnv) => {
        const override = env?.KOVA_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return stateDir ?? path.join(os.homedir(), ".kova");
      },
    },
...
describe("msteams file consent invoke FS fallback", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "kova-msteams-invoke-"));
