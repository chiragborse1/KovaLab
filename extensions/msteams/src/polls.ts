...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["kovaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["kova", "pollId"]) ??
    readNestedString(value, ["kova", "poll", "id"]) ??
    readNestedString(value, ["data", "kovaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "kova", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { kovaPollId: pollId, pollId },
        },
      },
    ],
  };
