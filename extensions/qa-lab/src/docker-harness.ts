...
const QA_LAB_UI_OVERLAY_DIR = "/opt/kova-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        KOVA_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      KOVA_CONFIG_PATH: /tmp/chiragborse1/KovaLab.json
      ...
      KOVA_PROFILE: ""
    volumes:
      - ./state:/opt/kova-scaffold:ro
      - ${repoMount}:/opt/kova-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/kova/workspace /tmp/kova/state && cp /opt/kova-scaffold/kova.json /tmp/chiragborse1/KovaLab.json && cp -R /opt/kova-scaffold/seed-workspace/. /tmp/kova/workspace/ && ln -snf /opt/kova-repo /tmp/kova/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t kova:qa-local-prebaked --build-arg KOVA_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "kova:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "kova.json"),
  ];
