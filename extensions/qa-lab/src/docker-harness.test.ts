...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "kova.json"),
    ...
    expect(compose).toContain("image: kova:qa-local-prebaked");
    ...
    expect(compose).toContain(":/opt/kova-repo:ro");
    expect(compose).toContain("./state:/opt/kova-scaffold:ro");
    expect(compose).toContain("cp -R /opt/kova-scaffold/seed-workspace/. /tmp/kova/workspace/");
    expect(compose).toContain("KOVA_CONFIG_PATH: /tmp/chiragborse1/KovaLab.json");
    expect(compose).toContain("KOVA_STATE_DIR: /tmp/kova/state");
    expect(compose).toContain('KOVA_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("KOVA_GATEWAY_TOKEN=qa-token");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("kova:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t kova:qa-local-prebaked --build-arg KOVA_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/kova",
      ),
    ]);
  });
});
