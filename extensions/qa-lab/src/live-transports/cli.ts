import { listQaRunnerCliContributions } from "getkova/plugin-sdk/qa-runner-runtime";
  ...
  return {
    ...
    register(qa) {
      qa.command(params.commandName)
        ...
        .action(() => {
          throw new Error(
            `QA runner "${params.commandName}" is installed but not active. Enable or allow plugin "${params.pluginId}" in your Kova config, then try again.`,
          );
        });
    },
  };
