...
function createHealthyDockerDeps(calls: string[]): QaDockerUpDeps {
  return {
    async runCommand(command, args, cwd) {
      ...
      if (args.join(" ").includes("ps --format json kova-qa-gateway")) {
...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/kova",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/kova`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/kova`,
        ),
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json kova-qa-gateway @/repo/kova`,
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/kova`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/kova`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json kova-qa-gateway @/repo/kova`,
      ]);
      const compose = await readFile(path.join(outputDir, "docker-compose.qa.yml"), "utf8");
      expect(compose).toContain(":/opt/kova-qa-lab-ui:ro");
  ...
  it("uses a repo-root-relative default output dir when none is provided", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${path.join(repoRoot, ".artifacts/qa-docker/docker-compose.qa.yml")} ps --format json kova-qa-gateway @${repoRoot}`,
      ]);
    } finally {
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json kova-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q kova-qa-gateway")) {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/kova`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json kova-qa-gateway @/repo/kova`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps -q kova-qa-gateway @/repo/kova`,
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @/repo/kova",
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
