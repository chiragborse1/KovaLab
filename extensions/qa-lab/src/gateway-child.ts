...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    KOVA_HOME: params.homeDir,
    KOVA_CONFIG_PATH: params.configPath,
    KOVA_STATE_DIR: params.stateDir,
    KOVA_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    KOVA_NO_RESPAWN: "1",
    KOVA_TEST_FAST: "1",
    KOVA_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    KOVA_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { KOVA_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { KOVA_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(path.join(resolvePreferredKovaTmpDir(), "kova-qa-suite-"));
  ...
  const keepTemp = process.env.KOVA_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
