...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/kova-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/kova";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/kova-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `kova-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "KOVA_CONFIG_PATH" &&
          key !== "KOVA_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `KOVA_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `KOVA_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm kova qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredKovaTmpDir(), `${plan.vmName}-qa-suite-`),
  );
