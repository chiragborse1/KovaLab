...
import { disposeRegisteredAgentHarnesses } from "getkova/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.KOVA_QA_SUITE_PROGRESS);
...
function resolveQaSuiteTransportReadyTimeoutMs(
  ...
  const raw = env.KOVA_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
    return 120_000;
  }
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.KOVA_QA_DEBUG === "1") {
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  try {
    ...
    const keepTemp = process.env.KOVA_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
