...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.KOVA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-nostr-"));
  process.env.KOVA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.KOVA_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".kova");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.KOVA_STATE_DIR = previous;
    }
