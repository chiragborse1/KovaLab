...
import { resolveAccountEntry } from "getkova/plugin-sdk/account-resolution";
import { tryReadSecretFileSync } from "getkova/plugin-sdk/core";
import type {
  LineAccountConfig,
  LineConfig,
  LineTokenSource,
...
export function listLineAccountIds(cfg: KovaConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: KovaConfig): string {
