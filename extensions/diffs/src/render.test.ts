...
describe("renderDiffDocument", () => {
  ...
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-kova-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/kova/plugins/diffs/view/id/token").pathname,
    ).toBe("/kova/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-kova-diff-payload>(.*?)<\/script>/g)].map((match) =>
