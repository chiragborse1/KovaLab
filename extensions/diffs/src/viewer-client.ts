...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-kova-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-kova-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.kovaDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.kovaDiffsError = "true";
