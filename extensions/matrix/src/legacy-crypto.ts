...
import { writeJsonFileAtomically as writeJsonFileAtomicallyImpl } from "getkova/plugin-sdk/json-store";
    ...
    if (!stat.isDirectory()) {
      return {
        ...
        warning:
          ...
          "Kova skipped automatic crypto migration for that path.",
      };
    }
  } catch (err) {
    return {
...
function resolveMatrixAccountIds(cfg: KovaConfig): string[] {
    ...
    if (!summary.decryptionKeyBase64 && (summary.roomKeyCounts?.backedUp ?? 0) > 0) {
      warnings.push(
        `Legacy Matrix encrypted state for account "${plan.accountId}" has backed-up room keys, but no local backup decryption key was found. ` +
          `Ask the operator to run "kova matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`,
      );
    }
