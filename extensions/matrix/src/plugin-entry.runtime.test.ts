...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __kovaMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["kova", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@kovaai", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__kovaMatrixWrapperJitiOptions = options;",
...
function writeKovaPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "kova.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeKovaAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedKovaBinFixture(
  fixtureRoot: string,
...
function writeCapturingSourceRuntimeWrapperFixture(fixtureRoot: string) {
  delete matrixWrapperGlobal.__kovaMatrixWrapperJitiOptions;
  writeKovaAliasFixture(fixtureRoot);
...
function expectSourcePluginSdkAliases(fixtureRoot: string) {
  expect(matrixWrapperGlobal.__kovaMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeKovaPackageFixture(fixtureRoot);
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__kovaMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "getkova/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__kovaMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted kova package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__kovaMatrixWrapperJitiOptions;
  writeTrustedKovaBinFixture(fixtureRoot, "Kova.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeSourceRuntimeWrapperFixture(fixtureRoot);
  await importFixtureModule(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js");
