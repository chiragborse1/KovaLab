...
describe("registerMatrixMonitorEvents verification routing", () => {
  ...
  it("adds self-device guidance when decrypt failures come from the same Matrix user", async () => {
    ...
    expect(logger.warn).toHaveBeenNthCalledWith(
      2,
      "matrix: failed to decrypt a message from this same Matrix user. This usually means another Matrix device did not share the room key, or another Kova runtime is using the same account. Check 'kova matrix verify status --verbose --account ops' and 'kova matrix devices list --account ops'.",
      {
        roomId: "!room:example.org",
        eventId: "$enc-self",
        sender: "@gumadeiras:matrix.example.org",
  ...
  it("classifies repeated fresh post-healthy-sync decrypt failures separately", async () => {
    ...
    try {
      ...
      expect(logger.warn).toHaveBeenNthCalledWith(
        4,
        "matrix: repeated fresh encrypted messages are still failing to decrypt after Matrix resumed healthy sync. This device may still be missing new room keys. Check 'kova matrix verify status --verbose --account ops' and 'kova matrix devices list --account ops'.",
        expect.objectContaining({
          failureCount: 3,
          roomCount: 3,
          senderCount: 3,
