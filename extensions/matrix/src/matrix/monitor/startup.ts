...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleKovaDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Kova devices detected for ${params.auth.userId}: ${deviceHealth.staleKovaDevices.map((device) => device.deviceId).join(", ")}. Run 'kova matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
