...
function redactOtelAttributes(attributes) {
	...
	for (const [key, value] of Object.entries(attributes)) {
		...
		redactedAttributes[key] = typeof value === "string" ? (0, api_exports.redactSensitiveText)(value) : value;
	}
	return redactedAttributes;
}
function lowCardinalityAttr(value, fallback = "unknown") {
	if (!value) return fallback;
	const redacted = (0, api_exports.redactSensitiveText)(value.trim());
...
function addUpstreamRequestIdSpanEvent(span, upstreamRequestIdHash) {
	...
	span.addEvent?.("kova.provider.request", { "kova.upstreamRequestIdHash": boundedHash });
}
function clampOtelLogText(value, maxChars) {
	return value.length > maxChars ? `${value.slice(0, maxChars)}...(truncated)` : value;
}
function normalizeOtelLogString(value, maxChars) {
	return clampOtelLogText((0, api_exports.redactSensitiveText)(value), maxChars);
}
...
function assignOtelModelContentAttributes(attributes, event, policy) {
	if (policy.inputMessages) assignOtelContentAttribute(attributes, "kova.content.input_messages", event.inputMessages);
	if (policy.outputMessages) assignOtelContentAttribute(attributes, "kova.content.output_messages", event.outputMessages);
	if (policy.systemPrompt) assignOtelContentAttribute(attributes, "kova.content.system_prompt", event.systemPrompt);
}
function assignOtelToolContentAttributes(attributes, event, policy) {
	if (policy.toolInputs) assignOtelContentAttribute(attributes, "kova.content.tool_input", event.toolInput);
	if (policy.toolOutputs) assignOtelContentAttribute(attributes, "kova.content.tool_output", event.toolOutput);
}
function assignOtelLogAttribute(attributes, key, value) {
	...
	if ((0, api_exports.redactSensitiveText)(key) !== key) return;
...
function normalizeTraceContext(value) {
	...
	if (!(0, api_exports.isValidDiagnosticTraceId)(candidate.traceId)) return;
	if (candidate.spanId !== void 0 && !(0, api_exports.isValidDiagnosticSpanId)(candidate.spanId)) return;
	if (candidate.parentSpanId !== void 0 && !(0, api_exports.isValidDiagnosticSpanId)(candidate.parentSpanId)) return;
	if (candidate.traceFlags !== void 0 && !(0, api_exports.isValidDiagnosticTraceFlags)(candidate.traceFlags)) return;
	return {
...
function assignOtelLogEventAttributes(attributes, eventAttributes) {
	...
	for (const rawKey in eventAttributes) {
		...
		if ((0, api_exports.redactSensitiveText)(key) !== key) continue;
...
function addTraceAttributes(attributes, traceContext) {
	...
	if (normalized.spanId) attributes["kova.spanId"] = normalized.spanId;
	if (normalized.parentSpanId) attributes["kova.parentSpanId"] = normalized.parentSpanId;
	if (normalized.traceFlags) attributes["kova.traceFlags"] = normalized.traceFlags;
}
function createDiagnosticsOtelService() {
	...
	return {
		...
		async start(ctx) {
			...
			const messageDeliveryDurationHistogram = meter.createHistogram("kova.message.delivery.duration_ms", {
				unit: "ms",
			...
			const modelCallRequestBytesHistogram = meter.createHistogram("kova.model_call.request_bytes", {
				unit: "By",
				description: "UTF-8 byte size of sanitized model request payloads"
			});
			const modelCallResponseBytesHistogram = meter.createHistogram("kova.model_call.response_bytes", {
				unit: "By",
				description: "UTF-8 byte size of streamed model response events"
			});
			const modelCallTimeToFirstByteHistogram = meter.createHistogram("kova.model_call.time_to_first_byte_ms", {
				unit: "ms",
				description: "Elapsed time before the first streamed model response event"
			});
			const toolExecutionDurationHistogram = meter.createHistogram("kova.tool.execution.duration_ms", {
				unit: "ms",
			...
			if (logsEnabled) {
				...
				recordLogRecord = (evt, metadata) => {
					try {
						...
						if (evt.loggerName) assignOtelLogAttribute(attributes, "kova.logger", evt.loggerName);
						if (evt.loggerParents?.length) assignOtelLogAttribute(attributes, "kova.logger.parents", evt.loggerParents.join("."));
			...
			const addRunAttrs = (spanAttrs, evt) => {
				if (evt.provider) spanAttrs["kova.provider"] = evt.provider;
				if (evt.model) spanAttrs["kova.model"] = evt.model;
				if (evt.channel) spanAttrs["kova.channel"] = evt.channel;
				if (evt.trigger) spanAttrs["kova.trigger"] = evt.trigger;
			};
			...
			const recordModelUsage = (evt, metadata) => {
				...
				spanWithDuration("kova.model.usage", spanAttrs, evt.durationMs, {
			...
			const recordWebhookProcessed = (evt) => {
				...
				if (evt.chatId !== void 0) spanAttrs["kova.chatId"] = String(evt.chatId);
				spanWithDuration("kova.webhook.processed", spanAttrs, evt.durationMs).end();
			};
			const recordWebhookError = (evt) => {
				...
				if (!tracesEnabled) return;
				const redactedError = (0, api_exports.redactSensitiveText)(evt.error);
				const spanAttrs = {
					...attrs,
					"kova.error": redactedError
				};
				if (evt.chatId !== void 0) spanAttrs["kova.chatId"] = String(evt.chatId);
				const span = tracer.startSpan("kova.webhook.error", { attributes: spanAttrs });
				span.setStatus({
			...
			const recordMessageProcessed = (evt) => {
				...
				const spanAttrs = { ...attrs };
				if (evt.chatId !== void 0) spanAttrs["kova.chatId"] = String(evt.chatId);
				if (evt.messageId !== void 0) spanAttrs["kova.messageId"] = String(evt.messageId);
				if (evt.reason) spanAttrs["kova.reason"] = (0, api_exports.redactSensitiveText)(evt.reason);
				const span = spanWithDuration("kova.message.processed", spanAttrs, evt.durationMs);
				if (evt.outcome === "error" && evt.error) span.setStatus({
					code: SpanStatusCode.ERROR,
					message: (0, api_exports.redactSensitiveText)(evt.error)
				});
				span.end();
			};
			...
			const recordMessageDeliveryCompleted = (evt) => {
				...
				if (!tracesEnabled) return;
				spanWithDuration("kova.message.delivery", {
					...attrs,
			...
			const recordMessageDeliveryError = (evt) => {
				...
				messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
				if (!tracesEnabled) return;
				const span = spanWithDuration("kova.message.delivery", attrs, evt.durationMs, { endTimeMs: evt.ts });
				span.setStatus({
					code: SpanStatusCode.ERROR,
					message: (0, api_exports.redactSensitiveText)(evt.errorCategory)
				});
				span.end(evt.ts);
			};
			const recordRunStarted = (evt, metadata) => {
				...
				const span = trackTrustedSpan(evt, metadata, spanWithDuration("kova.run", spanAttrs, void 0, {
					parentContext: activeTrustedParentContext(evt, metadata),
					startTimeMs: evt.ts
				}));
				const parentSpanId = trustedTraceContext(evt, metadata)?.parentSpanId;
			...
			const recordSessionState = (evt) => {
				const attrs = { "kova.state": evt.state };
				if (evt.reason) attrs["kova.reason"] = (0, api_exports.redactSensitiveText)(evt.reason);
			...
			const toolLoopAttrs = (evt) => ({
				...
				...evt.pairedToolName ? { "kova.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") } : {}
			});
			...
			const recordMemoryPressure = (evt) => {
				...
				if (!tracesEnabled) return;
				const span = spanWithDuration("kova.memory.pressure", {
					...attrs,
					...
					...evt.thresholdBytes !== void 0 ? { "kova.memory.threshold_bytes": evt.thresholdBytes } : {},
					...evt.rssGrowthBytes !== void 0 ? { "kova.memory.rss_growth_bytes": evt.rssGrowthBytes } : {},
					...evt.windowMs !== void 0 ? { "kova.memory.window_ms": evt.windowMs } : {}
				}, 0, { endTimeMs: evt.ts });
			...
			const recordRunCompleted = (evt, metadata) => {
				...
				if (evt.channel) attrs["kova.channel"] = evt.channel;
				durationHistogram.record(evt.durationMs, attrs);
				if (!tracesEnabled) return;
				const spanAttrs = { "kova.outcome": evt.outcome };
				addRunAttrs(spanAttrs, evt);
				if (evt.errorCategory) spanAttrs["kova.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
				const trustedTrace = trustedTraceContext(evt, metadata);
				const trackedSpan = trustedTrace?.spanId ? activeTrustedSpans.get(trustedTrace.spanId) : void 0;
				const span = trackedSpan ?? spanWithDuration("kova.run", spanAttrs, evt.durationMs, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
				});
				...
				if (evt.outcome === "error") span.setStatus({
					code: SpanStatusCode.ERROR,
					...evt.errorCategory ? { message: (0, api_exports.redactSensitiveText)(evt.errorCategory) } : {}
				});
			...
			const harnessRunMetricAttrs = (evt) => ({
				...
				...evt.type === "harness.run.started" ? {} : { "kova.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome },
			...
			const recordHarnessRunStarted = (evt, metadata) => {
				if (!tracesEnabled || !metadata.trusted) return;
				trackTrustedSpan(evt, metadata, spanWithDuration("kova.harness.run", harnessRunMetricAttrs(evt), void 0, {
					parentContext: activeTrustedParentContext(evt, metadata),
					startTimeMs: evt.ts
				}));
			};
			const recordHarnessRunCompleted = (evt, metadata) => {
				...
				if (evt.resultClassification) spanAttrs["kova.harness.result_classification"] = lowCardinalityAttr(evt.resultClassification);
				if (typeof evt.yieldDetected === "boolean") spanAttrs["kova.harness.yield_detected"] = evt.yieldDetected;
				...
				const span = takeTrackedTrustedSpan(evt, metadata) ?? spanWithDuration("kova.harness.run", spanAttrs, evt.durationMs, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
				});
				setSpanAttrs(span, spanAttrs);
			...
			const recordContextAssembled = (evt, metadata) => {
				...
				addRunAttrs(spanAttrs, evt);
				if (evt.contextTokenBudget !== void 0) spanAttrs["kova.context.token_budget"] = evt.contextTokenBudget;
				if (evt.reserveTokens !== void 0) spanAttrs["kova.context.reserve_tokens"] = evt.reserveTokens;
				spanWithDuration("kova.context.assembled", spanAttrs, 0, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
				}).end(evt.ts);
			};
			...
			const recordModelCallStarted = (evt, metadata) => {
				...
				if (evt.api) spanAttrs["kova.api"] = evt.api;
				if (evt.transport) spanAttrs["kova.transport"] = evt.transport;
				trackTrustedSpan(evt, metadata, spanWithDuration("kova.model.call", spanAttrs, void 0, {
					parentContext: activeTrustedParentContext(evt, metadata),
					startTimeMs: evt.ts
				}));
			};
			const recordModelCallCompleted = (evt, metadata) => {
				...
				const span = takeTrackedTrustedSpan(evt, metadata) ?? spanWithDuration("kova.model.call", spanAttrs, evt.durationMs, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
				});
				setSpanAttrs(span, spanAttrs);
			...
			const recordModelCallError = (evt, metadata) => {
				...
				const metricAttrs = {
					...
					"kova.errorCategory": errorType,
					...evt.failureKind ? { "kova.failureKind": lowCardinalityAttr(evt.failureKind, "other") } : {}
				};
				modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
				recordModelCallSizeTimingMetrics(evt, metricAttrs);
				...
				if (evt.failureKind) spanAttrs["kova.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
				assignGenAiModelCallAttrs(spanAttrs, evt);
				if (evt.api) spanAttrs["kova.api"] = evt.api;
				if (evt.transport) spanAttrs["kova.transport"] = evt.transport;
				assignModelCallSizeTimingAttrs(spanAttrs, evt);
			...
			const recordToolExecutionStarted = (evt, metadata) => {
				if (!tracesEnabled || !metadata.trusted) return;
				trackTrustedSpan(evt, metadata, spanWithDuration("kova.tool.execution", toolExecutionBaseAttrs(evt), void 0, {
					parentContext: activeTrustedParentContext(evt, metadata),
					startTimeMs: evt.ts
				}));
			};
			const recordToolExecutionCompleted = (evt, metadata) => {
				...
				const span = takeTrackedTrustedSpan(evt, metadata) ?? spanWithDuration("kova.tool.execution", spanAttrs, evt.durationMs, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
				});
				setSpanAttrs(span, spanAttrs);
			...
			const recordToolExecutionError = (evt, metadata) => {
				...
				addRunAttrs(spanAttrs, evt);
				if (evt.errorCode) spanAttrs["kova.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
				assignOtelToolContentAttributes(spanAttrs, evt, contentCapturePolicy);
				const span = takeTrackedTrustedSpan(evt, metadata) ?? spanWithDuration("kova.tool.execution", spanAttrs, evt.durationMs, {
					parentContext: activeTrustedParentContext(evt, metadata),
					endTimeMs: evt.ts
			...
			const recordExecProcessCompleted = (evt) => {
				...
				if (evt.failureKind) attrs["kova.failureKind"] = evt.failureKind;
				...
				if (typeof evt.exitCode === "number") spanAttrs["kova.exec.exit_code"] = evt.exitCode;
				if (evt.exitSignal) spanAttrs["kova.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
				if (evt.timedOut !== void 0) spanAttrs["kova.exec.timed_out"] = evt.timedOut;
				const span = spanWithDuration("kova.exec", spanAttrs, evt.durationMs, { endTimeMs: evt.ts });
			...
			const recordTelemetryExporter = (evt, metadata) => {
				...
				telemetryExporterCounter.add(1, {
					...
					...evt.errorCategory ? { "kova.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") } : {}
				});
			};
