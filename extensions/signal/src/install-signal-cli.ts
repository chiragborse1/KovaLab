...
import { runPluginCommandWithTimeout } from "getkova/plugin-sdk/run-command";
import type { RuntimeEnv } from "getkova/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "getkova/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredKovaTmpDir(), "kova-signal-"));
