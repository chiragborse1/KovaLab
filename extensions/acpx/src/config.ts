...
import { formatPluginConfigIssue } from "getkova/plugin-sdk/extension-shared";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "kova-plugin-tools";
export const ACPX_KOVA_TOOLS_MCP_SERVER_NAME = "kova-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "kova.plugin.json")) &&
...
function resolveAcpxPluginRootFromKovaLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Kova layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromKovaLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveKovaRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const kovaRoot = resolveKovaRoot(pluginRoot);
  const distEntry = path.join(kovaRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(kovaRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
export function resolveKovaToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const kovaRoot = resolveKovaRoot(pluginRoot);
  const distEntry = path.join(kovaRoot, "dist", "mcp", "kova-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(kovaRoot, "src", "mcp", "kova-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.kovaToolsMcpBridge && resolved[ACPX_KOVA_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_KOVA_TOOLS_MCP_SERVER_NAME} is reserved when kovaToolsMcpBridge=true`,
    );
  }
  ...
  if (params.kovaToolsMcpBridge) {
    resolved[ACPX_KOVA_TOOLS_MCP_SERVER_NAME] = resolveKovaToolsMcpServerConfig(params.moduleUrl);
  }
  return resolved;
}
...
export function resolveAcpxPluginConfig(params: {
  ...
  const kovaToolsMcpBridge = normalized.kovaToolsMcpBridge === true;
