...
The bundled ACPX extension is a thin Kova wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `kova/acpx`, not in this extension.
...
Use this flow when Kova needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `kova/acpx` repo first.
2. In Kova, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Kova and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Kova ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Kova-specific glue, move it to `kova/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
