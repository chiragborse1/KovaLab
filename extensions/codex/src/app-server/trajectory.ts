...
import { resolveUserPath } from "getkova/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.KOVA_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.KOVA_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
