...
vi.mock("getkova/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("getkova/plugin-sdk/agent-harness-runtime")>()),
...
function buildCurrentCodexApprovalElicitation() {
  return {
    ...
    _meta: {
      ...
      tool_params_display: [
        { name: "repo", display_name: "Repository", value: "chiragborse1/KovaLab" },
      ],
    },
    requestedSchema: {
      type: "object",
...
describe("Codex app-server elicitation bridge", () => {
  ...
  it("accepts current Codex MCP approval elicitations with an empty form schema", async () => {
    ...
    expect(approvalRequest.description).toContain("Repository: chiragborse1/KovaLab");
  });
          ...
          tool_params_display: [
            {
              ...
              value: "\u001b]8;;https://evil.example\u001b\\chiragborse1/KovaLab\u001b]8;;\u001b\\",
            },
          ],
        },
        requestedSchema: {
  ...
  it("falls back to stable names when display labels sanitize to empty", async () => {
    ...
    expect(approvalRequest.description).toContain("- repo: chiragborse1/KovaLab");
