...
import {
  ...
  installKovaOwnedToolHooks,
  mediaToolResult,
  resetKovaOwnedToolHooks,
  textToolResult,
} from "../../../../test/helpers/agents/kova-owned-tool-runtime-contract.js";
...
describe("Kova-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetKovaOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installKovaOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installKovaOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installKovaOwnedToolHooks();
