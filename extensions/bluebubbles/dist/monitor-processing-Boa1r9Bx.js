...
import { i as normalizeSecretInputString } from "./secret-input-BpFibtWM.js";
import { t as getBlueBubblesRuntime } from "./runtime-DrM7c8VL.js";
import { i as resolveBlueBubblesInboundConversationId } from "./conversation-id-CMVZSNtz.js";
import { _ as fetchBlueBubblesMessageAttachments, a as resolveBlueBubblesMessageId, b as sendMessageBlueBubbles, g as downloadBlueBubblesAttachment, i as rememberBlueBubblesReplyCache, n as sendBlueBubblesReaction, o as resolveReplyContextFromCache, p as sendBlueBubblesTyping, r as getShortIdForUuid, t as normalizeBlueBubblesReactionInputStrict, u as markBlueBubblesChatRead, v as sendBlueBubblesAttachment, y as resolveChatGuidForTarget } from "./reactions-2cUbM9tT.js";
...
import { lowercasePreservingWhitespace, normalizeOptionalString, stripMarkdown as stripMarkdown$1 } from "openclaw/plugin-sdk/text-runtime";
...
import { resolveOutboundMediaUrls, resolveTextChunksWithFallback, sendMediaWithLeadingCaption } from "openclaw/plugin-sdk/reply-payload";
import { normalizeLowercaseStringOrEmpty as normalizeLowercaseStringOrEmpty$1, normalizeOptionalLowercaseString as normalizeOptionalLowercaseString$1, normalizeOptionalString as normalizeOptionalString$1 } from "openclaw/plugin-sdk/string-coerce-runtime";
...
import { logAckFailure, logTypingFailure, resolveAckReaction } from "openclaw/plugin-sdk/channel-feedback";
...
import { evictOldHistoryKeys, recordPendingHistoryEntryIfEnabled } from "openclaw/plugin-sdk/reply-history";
...
function resolveBlueBubblesConversationRoute(params) {
	...
	if (runtimeRoute.bindingRecord && !runtimeRoute.boundSessionKey) logVerbose(`bluebubbles: plugin-bound conversation ${conversationId}`);
	else if (runtimeRoute.boundSessionKey) logVerbose(`bluebubbles: routed via bound conversation ${conversationId} -> ${runtimeRoute.boundSessionKey}`);
	return route;
}
//#endregion
...
function normalizeSnippet(value) {
	return normalizeOptionalLowercaseString$1(stripMarkdown$1(value).replace(/\s+/g, " ")) ?? "";
}
...
function rememberPendingOutboundMessageId(entry) {
	...
	pendingOutboundMessageIds.push({
		...
		chatGuid: normalizeOptionalString$1(entry.chatGuid),
		chatIdentifier: normalizeOptionalString$1(entry.chatIdentifier),
		chatId: typeof entry.chatId === "number" ? entry.chatId : void 0,
		snippetRaw,
		snippetNorm,
		isMediaSnippet: normalizeLowercaseStringOrEmpty$1(snippetRaw).startsWith("<media:"),
		createdAt: Date.now()
	});
...
function chatsMatch(left, right) {
	const leftGuid = normalizeOptionalString$1(left.chatGuid);
	const rightGuid = normalizeOptionalString$1(right.chatGuid);
	if (leftGuid && rightGuid) return leftGuid === rightGuid;
	const leftIdentifier = normalizeOptionalString$1(left.chatIdentifier);
	const rightIdentifier = normalizeOptionalString$1(right.chatIdentifier);
...
function consumePendingOutboundMessageId(params) {
	...
	const isMediaBody = normalizeLowercaseStringOrEmpty$1(params.body).startsWith("<media:");
...
function logVerbose$1(core, runtime, message) {
...
function resolveBlueBubblesAckReaction(params) {
	...
	try {
		...
		const key = normalizeLowercaseStringOrEmpty$1(raw);
		if (!invalidAckReactions.has(key)) {
			invalidAckReactions.add(key);
			logVerbose$1(params.core, params.runtime, `ack reaction skipped (unsupported for BlueBubbles): ${raw}`);
		}
		return null;
	}
}
...
async function processMessage(message, target) {
	...
	const claim = await claimBlueBubblesInboundMessage({
		...
		onDiskError: (error) => logVerbose$1(core, runtime, `inbound-dedupe disk error: ${sanitizeForLog(error)}`)
	});
	if (claim.kind === "duplicate" || claim.kind === "inflight") {
		logVerbose$1(core, runtime, `drop: ${claim.kind} inbound key=${sanitizeForLog(dedupeKey ?? "")} sender=${sanitizeForLog(message.senderId)}`);
		return;
	}
	...
	if (claim.kind === "claimed") if (signal.deliveryFailed) {
		logVerbose$1(core, runtime, `inbound-dedupe: releasing claim for key=${sanitizeForLog(dedupeKey ?? "")} after reply delivery failure (will retry on replay)`);
		claim.release();
	} else {
		try {
			await claim.finalize();
		} catch (finalizeError) {
			logVerbose$1(core, runtime, `inbound-dedupe: finalize failed for key=${sanitizeForLog(dedupeKey ?? "")}: ${sanitizeForLog(finalizeError)}`);
		}
		...
		if (secondaryIds.length > 0) try {
			await commitBlueBubblesCoalescedMessageIds({
				...
				accountId: account.accountId,
				onDiskError: (error) => logVerbose$1(core, runtime, `inbound-dedupe: coalesced secondary commit disk error: ${sanitizeForLog(error)}`)
			});
		} catch (secondaryError) {
			logVerbose$1(core, runtime, `inbound-dedupe: coalesced secondary commit failed for primary=${sanitizeForLog(dedupeKey ?? "")}: ${sanitizeForLog(secondaryError)}`);
		}
	}
}
async function processMessageAfterDedupe(message, target, dedupeSignal) {
	...
	if (attachments.length === 0 && retryMessageId && baseUrl && password && (text.length === 0 || message.eventType === "updated-message")) try {
		...
		if (fetched.length > 0) {
			logVerbose$1(core, runtime, `attachment retry found ${fetched.length} attachment(s) for msgId=${message.messageId}`);
			attachments = fetched;
		}
	} catch (err) {
		logVerbose$1(core, runtime, `attachment retry failed for msgId=${message.messageId}: ${String(err)}`);
	}
	...
	if (isSelfChatMessage && hasBlueBubblesSelfChatCopy(selfChatLookup)) {
		logVerbose$1(core, runtime, `drop: reflected self-chat duplicate sender=${message.senderId}`);
		return;
	}
	if (!rawBody) {
		logVerbose$1(core, runtime, `drop: empty text sender=${message.senderId}`);
		return;
	}
	logVerbose$1(core, runtime, `msg sender=${message.senderId} group=${isGroup} textLen=${text.length} attachments=${attachments.length} chatGuid=${message.chatGuid ?? ""} chatId=${message.chatId ?? ""}`);
	...
	const groupName = normalizeOptionalString$1(message.chatName);
	if (accessDecision.decision !== "allow") {
		if (isGroup) {
			if (accessDecision.reasonCode === DM_GROUP_ACCESS_REASON.GROUP_POLICY_DISABLED) {
				logVerbose$1(core, runtime, "Blocked BlueBubbles group message (groupPolicy=disabled)");
			...
			if (accessDecision.reasonCode === DM_GROUP_ACCESS_REASON.GROUP_POLICY_EMPTY_ALLOWLIST) {
				logVerbose$1(core, runtime, "Blocked BlueBubbles group message (no allowlist)");
				logGroupAllowlistHint({
					runtime,
			...
			if (accessDecision.reasonCode === DM_GROUP_ACCESS_REASON.GROUP_POLICY_NOT_ALLOWLISTED) {
				logVerbose$1(core, runtime, `Blocked BlueBubbles sender ${message.senderId} (not in groupAllowFrom)`);
				logVerbose$1(core, runtime, `drop: group sender not allowed sender=${message.senderId} allowFrom=${effectiveGroupAllowFrom.join(",")}`);
				logGroupAllowlistHint({
					runtime,
		...
		if (accessDecision.reasonCode === DM_GROUP_ACCESS_REASON.DM_POLICY_DISABLED) {
			logVerbose$1(core, runtime, `Blocked BlueBubbles DM from ${message.senderId}`);
			logVerbose$1(core, runtime, `drop: dmPolicy disabled sender=${message.senderId}`);
			return;
		}
		if (accessDecision.decision === "pairing") {
			await pairing.issueChallenge({
				...
				onCreated: () => {
					...
					logVerbose$1(core, runtime, `bluebubbles pairing request sender=${message.senderId}`);
				},
				...
				onReplyError: (err) => {
					logVerbose$1(core, runtime, `bluebubbles pairing reply failed for ${message.senderId}: ${String(err)}`);
					runtime.error?.(`[bluebubbles] pairing reply failed sender=${message.senderId}: ${String(err)}`);
				}
			});
			return;
		}
		logVerbose$1(core, runtime, `Blocked unauthorized BlueBubbles sender ${message.senderId} (dmPolicy=${dmPolicy})`);
		logVerbose$1(core, runtime, `drop: dm sender not allowed sender=${message.senderId} allowFrom=${effectiveAllowFrom.join(",")}`);
		return;
	}
	...
	if (isGroup && commandGate.shouldBlock) {
		logInboundDrop({
			log: (msg) => logVerbose$1(core, runtime, msg),
			channel: "bluebubbles",
	...
	if (isGroup && requireMention && canDetectMention && !wasMentioned && !shouldBypassMention) {
		logVerbose$1(core, runtime, `bluebubbles: skipping group message (no mention)`);
		return;
	}
	if (isGroup && !message.participants?.length && baseUrl && password) try {
		...
		logVerbose$1(core, runtime, `bluebubbles: participant fallback lookup failed chat=${peerId}: ${String(err)}`);
	}
	...
	let mediaPaths = [];
	let mediaTypes = [];
	if (attachments.length > 0) if (!baseUrl || !password) logVerbose$1(core, runtime, "attachment download skipped (missing serverUrl/password)");
		...
		if (attachment.totalBytes && attachment.totalBytes > maxBytes) {
			logVerbose$1(core, runtime, `attachment too large guid=${attachment.guid} bytes=${attachment.totalBytes}`);
			continue;
		}
		try {
			...
			logVerbose$1(core, runtime, `attachment download failed guid=${attachment.guid} err=${String(err)}`);
		}
	}
	...
	if (replyToId) {
		...
		if (cached) {
			...
			if (core.logging.shouldLogVerbose()) {
				...
				logVerbose$1(core, runtime, `reply-context cache hit replyToId=${replyToId} sender=${replyToSender ?? ""} body="${preview}"`);
			}
		}
	}
	...
	if (hasReplyContext && !includeReplyContext && isGroup) logVerbose$1(core, runtime, `bluebubbles: drop reply context (mode=${contextVisibilityMode}, sender_allowed=${replySenderAllowed ? "yes" : "no"})`);
	...
	const fromLabel = isGroup ? `${normalizeOptionalString$1(message.chatName) || "Group"} id:${peerId}` : senderLabel !== message.senderId ? `${senderLabel} id:${message.senderId}` : senderLabel;
	const groupSubject = isGroup ? normalizeOptionalString$1(message.chatName) : void 0;
	...
	const ackReactionPromise = shouldAckReaction() && ackMessageId && chatGuidForActions && ackReactionValue ? sendBlueBubblesReaction({
		...
		logVerbose$1(core, runtime, `ack reaction failed chatGuid=${chatGuidForActions} msg=${ackMessageId}: ${String(err)}`);
		return false;
	}) : null;
	...
	if (chatGuidForActions && baseUrl && password && sendReadReceipts) try {
		...
		logVerbose$1(core, runtime, `marked read chatGuid=${chatGuidForActions}`);
	} catch (err) {
		runtime.error?.(`[bluebubbles] mark read failed: ${String(err)}`);
	}
	else if (!sendReadReceipts) logVerbose$1(core, runtime, "mark read skipped (sendReadReceipts=false)");
	else logVerbose$1(core, runtime, "mark read skipped (missing chatGuid or credentials)");
	...
	if (historyKey && historyLimit > 0) {
		...
		if (backfillAttempt) try {
			...
			if (backfillResult.entries.length > 0) {
				...
				logVerbose$1(core, runtime, `backfilled ${backfillResult.entries.length} history messages for ${isGroup ? "group" : "DM"}: ${historyIdentifier}`);
				...
				logVerbose$1(core, runtime, `history backfill unresolved for ${historyIdentifier}; retries left=${Math.max(remainingAttempts, 0)} next_in_ms=${nextBackoffMs}`);
			}
		} catch (err) {
			const remainingAttempts = HISTORY_BACKFILL_MAX_ATTEMPTS - backfillAttempt.attempts;
			const nextBackoffMs = Math.max(backfillAttempt.nextAttemptAt - nowMs, 0);
			logVerbose$1(core, runtime, `history backfill failed for ${historyIdentifier}: ${String(err)} (retries left=${Math.max(remainingAttempts, 0)} next_in_ms=${nextBackoffMs})`);
		}
	}
	let inboundHistory;
	if (historyKey && historyLimit > 0) {
	...
	const ctxPayload = core.channel.reply.finalizeInboundContext({
		...
		GroupSystemPrompt: isGroup ? normalizeOptionalString$1(account.config.groups?.[peerId]?.systemPrompt ?? account.config.groups?.["*"]?.systemPrompt) : void 0
	});
...
async function processReaction(reaction, target) {
	...
	if (reaction.isGroup && core.channel.groups.resolveRequireMention({
		...
		logVerbose$1(core, runtime, "bluebubbles: skipping group reaction (requireMention=true)");
		return;
	}
	const route = resolveBlueBubblesConversationRoute({
		cfg: config,
	...
	logVerbose$1(core, runtime, `reaction event enqueued: ${text}`);
}
//#endregion
export { warmupBlueBubblesInboundDedupe as a, sendBlueBubblesMedia as i, processMessage as n, processReaction as r, logVerbose$1 as t };
