...
import { type ClaimableDedupe, createClaimableDedupe } from "getkova/plugin-sdk/persistent-dedupe";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  if (env.VITEST || env.NODE_ENV === "test") {
    // Isolate tests from real ~/.kova state without sharing across tests.
    // Stable-per-pid so the scoped dedupe test can observe persistence.
    const name = "kova-vitest-" + process.pid;
    return path.join(resolvePreferredKovaTmpDir(), name);
  }
  // Canonical Kova state dir: honors KOVA_STATE_DIR (with `~` expansion
  // via resolveUserPath), plus legacy/new fallback. Using the shared helper
  // keeps this plugin's persistence aligned with the rest of Kova state.
