...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit KOVA_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.KOVA_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.kova.
  // Use resolvePreferredKovaTmpDir + string concat (mirrors
  ...
  // Canonical Kova state dir: honors `~` expansion + legacy/new
