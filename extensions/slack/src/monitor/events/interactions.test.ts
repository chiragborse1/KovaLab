...
vi.mock("getkova/plugin-sdk/approval-gateway-runtime", () => ({
  resolveApprovalOverGateway: (arg: unknown) => resolveApprovalOverGatewayMock(arg),
}));

vi.mock("getkova/plugin-sdk/security-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "kova:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the Kova prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("kova:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "kova:reply_button" }],
            },
          ],
        },
      },
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"kova:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "kova:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "kova:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "kova:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "kova:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:kova:deploy_form",
      callbackId: "kova:deploy_form",
      viewId: "V123",
      userId: "U777",
            ...
            values: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@kovaai.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.neuralstudio.in",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@kovaai.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.neuralstudio.in/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "kova:long_richtext",
