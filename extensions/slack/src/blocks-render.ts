...
import type { InteractiveReply, MessagePresentation } from "getkova/plugin-sdk/interactive-runtime";
...
export function buildSlackInteractiveBlocks(interactive?: InteractiveReply): SlackBlock[] {
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `kova_reply_buttons_${++state.buttonIndex}`,
        elements,
      });
      return state;
    }
    ...
    state.blocks.push({
      type: "actions",
      block_id: `kova_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
