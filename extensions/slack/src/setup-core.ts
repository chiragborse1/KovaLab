import { hasConfiguredSecretInput } from "getkova/plugin-sdk/secret-input";
import {
...
import { formatDocsLink } from "getkova/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: KovaConfig, accountId: string): KovaConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: KovaConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: KovaConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: KovaConfig; accountId: string }) =>
      enableSlackAccount(cfg, accountId),
    applySet: ({ cfg, accountId, value }: { cfg: KovaConfig; accountId: string; value: unknown }) =>
      patchChannelConfigForAccount({
        cfg,
        channel,
        accountId,
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: KovaConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: KovaConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: KovaConfig; accountId: string }) =>
    ...
    disable: (cfg: KovaConfig) => setSetupChannelEnabled(cfg, channel, false),
