...
} from "getkova/plugin-sdk/channel-pairing";
import type { KovaConfig } from "getkova/plugin-sdk/config-runtime";
import { buildPassiveProbedChannelStatusSummary } from "getkova/plugin-sdk/extension-shared";
import {
 ...
 * for Kova. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: KovaConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: KovaConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: KovaConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: KovaConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("getkova/plugin-sdk/runtime-env").RuntimeEnv;
