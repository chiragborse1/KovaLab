...
const KOVA_OPENSHELL_E2E = process.env.KOVA_E2E_OPENSHELL === "1";
const KOVA_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const KOVA_OPENSHELL_COMMAND = process.env.KOVA_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "kova-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && KOVA_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: KOVA_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(KOVA_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(KOVA_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-openshell-e2e-"));
      ...
      const gatewayName = `kova-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `kova-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/kova-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: KOVA_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("kova-openshell-e2e");
