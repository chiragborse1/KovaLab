...
const {
  ...
  formatMatrixQaCliCommand: (args: string[]) => `kova ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaKovaCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaKovaCli: vi.fn(),
  startMatrixQaKovaCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaKovaCli.mockReset();
    startMatrixQaKovaCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Kova Matrix QA run",
        scenarios: [
    ...
    try {
      ...
      await expect(
        runMatrixQaScenario(scenario!, {
          ...
          gatewayRuntimeEnv: {
            KOVA_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaKovaCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaKovaCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.KOVA_CONFIG_PATH) {
          const cliConfig = JSON.parse(await readFile(String(env.KOVA_CONFIG_PATH), "utf8")) as {
            channels?: {
              matrix?: {
      ...
      await expect(
        runMatrixQaScenario(scenario!, {
          ...
          driverPassword: "driver-password",
          gatewayRuntimeEnv: {
            KOVA_CONFIG_PATH: "/tmp/gateway-config.json",
            KOVA_STATE_DIR: "/tmp/gateway-state",
            PATH: process.env.PATH,
          },
          outputDir,
        }),
      ...
      expect(startMatrixQaKovaCli).toHaveBeenCalledTimes(1);
      expect(startMatrixQaKovaCli.mock.calls[0]?.[0].args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(startMatrixQaKovaCli.mock.calls[0]?.[0].timeoutMs).toBe(16_000);
      ...
      expect(runMatrixQaKovaCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaKovaCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(runMatrixQaKovaCli.mock.calls[0]?.[0].stdin).toBe("encoded-recovery-key\n");
      const cliEnv = startMatrixQaKovaCli.mock.calls[0]?.[0].env;
      expect(cliEnv?.KOVA_STATE_DIR).toContain("kova-matrix-cli-qa-");
      expect(cliEnv?.KOVA_CONFIG_PATH).toContain("kova-matrix-cli-qa-");
      const configPath = String(cliEnv?.KOVA_CONFIG_PATH);
      ...
      await expect(readdir(String(cliEnv?.KOVA_STATE_DIR))).rejects.toThrow();
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaKovaCli.mockImplementation(async ({ args, env }) => {
        if (env.KOVA_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.KOVA_CONFIG_PATH), "utf8"),
          ) as {
      ...
      expect(runMatrixQaKovaCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Kova Matrix QA CLI Account Add E2EE",
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Account Add Owner",
          registrationToken: "registration-token",
        }),
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaKovaCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.KOVA_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.KOVA_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Encryption Setup Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Encryption Idempotent Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi.fn().mockRejectedValue(new Error("kova matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaKovaCli.mockReturnValue({
      ...
      expect(startMatrixQaKovaCli.mock.calls[0]?.[0].env.KOVA_CONFIG_PATH).toContain(
        "kova-matrix-e2ee-setup-qa-",
      );
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Encryption Failure Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Invalid Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaKovaCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.KOVA_CONFIG_PATH);
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Multi Account Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix CLI setup then gateway encrypted reply through the QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Setup Gateway",
          registrationToken: "registration-token",
        }),
      );
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "Kova Matrix QA CLI Setup Driver",
          registrationToken: "registration-token",
        }),
      );
      expect(createPrivateRoom).toHaveBeenCalledWith({
