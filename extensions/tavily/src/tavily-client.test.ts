...
vi.mock("getkova/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: kova", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "kova" });
  });

  it("runTavilyExtract sends X-Client-Source: kova", async () => {
