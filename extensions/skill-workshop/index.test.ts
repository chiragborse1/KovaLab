...
import type { AnyAgentTool } from "getkova/plugin-sdk/agent-runtime";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kova-skill-workshop-test-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  it("uses the reviewer to propose existing skill repairs", async () => {
    ...
    const proposal = await reviewTranscriptForProposal({
      ...
      config: {
        ...
        maxSkillBytes: 40_000,
        curatorEnabled: true,
        curatorIntervalTurns: 50,
        curatorMinSkillAgeDays: 7,
        curatorStaleDays: 30,
        curatorArchiveDays: 90,
        curatorMaxActions: 20,
      },
