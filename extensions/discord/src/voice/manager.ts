...
import type { DiscordAccountConfig, TtsConfig } from "getkova/plugin-sdk/config-runtime";
...
import { parseTtsDirectives } from "getkova/plugin-sdk/speech";
...
function resolveVoiceTtsConfig(params: { cfg: KovaConfig; override?: TtsConfig }): {
  cfg: KovaConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredKovaTmpDir(), "discord-voice-"));
