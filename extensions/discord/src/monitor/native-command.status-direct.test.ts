...
vi.mock("getkova/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("getkova/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("getkova/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): KovaConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("getkova/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
