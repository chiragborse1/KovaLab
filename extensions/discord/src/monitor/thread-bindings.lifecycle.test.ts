...
import {
  ...
  type KovaConfig,
} from "getkova/plugin-sdk/runtime-config-snapshot";
...
const acpRuntime = await import("getkova/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "kova-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.KOVA_STATE_DIR = previousStateDir;
      }
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:kova-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
