...
let runtimeEnvModule: typeof import("getkova/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): KovaConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("getkova/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("getkova/plugin-sdk/plugin-runtime")>(
        "getkova/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("getkova/plugin-sdk/runtime-env");
  ...
  it("passes Kova EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
