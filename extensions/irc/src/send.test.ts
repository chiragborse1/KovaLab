...
vi.mock("getkova/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("getkova/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("getkova/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("getkova/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
