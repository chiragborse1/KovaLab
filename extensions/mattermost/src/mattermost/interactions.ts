...
import { safeEqualSecret } from "getkova/plugin-sdk/security-runtime";
import {
...
import { isTrustedProxyAddress, resolveClientIp, type KovaConfig } from "./runtime-api.js";

const INTERACTION_MAX_BODY_BYTES = 64 * 1024;
const INTERACTION_BODY_TIMEOUT_MS = 10_000;
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__kova_channel_id";
...
type InteractionCallbackConfig = Pick<KovaConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "kova-mattermost-interactions").update(botToken).digest("hex");
}
