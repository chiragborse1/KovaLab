...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: KovaConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  beforeEach(() => {
    ...
    delete process.env.KOVA_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("Kova");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.KOVA_BOT_NAME = "TestBot";
