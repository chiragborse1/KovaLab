...
function getChannelConfig(cfg: KovaConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: KovaConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: KovaConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: KovaConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Kova uses to send replies back to Chat.",
      ],
