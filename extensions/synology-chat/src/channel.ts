/**
 * Synology Chat Channel Plugin for Kova.
 *
...
import { createChatChannelPlugin, type ChannelPlugin } from "getkova/plugin-sdk/channel-core";
import { waitUntilAbort } from "getkova/plugin-sdk/channel-lifecycle";
import {
...
import { attachChannelToResult } from "getkova/plugin-sdk/channel-send-result";
import { createEmptyChannelDirectoryAdapter } from "getkova/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "kova pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: KovaConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: KovaConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Kova",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "Kova: your access has been approved.",
