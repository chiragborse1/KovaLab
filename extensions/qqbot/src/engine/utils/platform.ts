 ...
 * `getkova/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.kova/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".kova", "qqbot", ...subPaths);
}

/** Return a path under `~/.kova/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.kova/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Kova's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".kova", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.kova/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.kova/media`, Kova's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `kova/src/media/local-roots.ts`). Using it as a
 ...
 */
export function getKovaMediaDir(): string {
  return path.join(getHomeDir(), ".kova", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".kova", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Kova's shared `~/.kova/media`
  ...
  const allowedRoots = [getKovaMediaDir(), getQQBotMediaPath()];
