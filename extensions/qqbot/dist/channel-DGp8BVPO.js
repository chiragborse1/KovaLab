import { n as getBridgeLogger, t as ensurePlatformAdapter } from "./bootstrap-CVQb8lt9.js";
	...
	return createChannelApprovalCapability({
		...
		nativeRuntime: createLazyChannelApprovalNativeRuntimeAdapter({
			...
			load: async () => {
				...
				return (await import("./handler-runtime-DEI6gL5n.js")).qqbotApprovalNativeRuntime;
			}
		})
	});
}
...
function loadGatewayModule() {
	_gatewayModulePromise ??= import("./gateway-BgYqc-KV.js");
...
const qqbotPlugin = {
	...
	gateway: {
		startAccount: async (ctx) => {
			...
			if (!account.appId || !account.clientSecret) {
				...
				if (backup?.appId && backup?.clientSecret) try {
					...
					await getQQBotRuntime().config.replaceConfigFile({
						nextConfig: nextCfg,
		...
		logoutAccount: async ({ accountId, cfg }) => {
			...
			if (changed) await getQQBotRuntime().config.replaceConfigFile({
				nextConfig: nextCfg,
				afterWrite: { mode: "auto" }
			});
