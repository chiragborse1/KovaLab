...
- CLI: add `kova mcp status --probe` for explicit saved MCP command-presence and HTTP reachability checks without launching stdio MCP servers by default. Thanks @chiragborse1
...
- CLI/onboarding: show a full KOVA AGENT startup banner and keep the first terminal handoff quiet instead of sending an automatic starter message. Thanks @chiragborse1
...
- Installer/Windows: keep one-line installs readable in Windows PowerShell and start onboarding through the npm command shim instead of `Start-Process` on non-executable shims. Thanks @chiragborse1
