...
If still empty, check `/data/.kova/kova.json`:

```
cat /data/.kova/kova.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Kova dashboard or in the active Kova config path (`$KOVA_CONFIG_PATH`, default `~/.chiragborse1/KovaLab.json`) under `skills.entries.gh-issues`."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.kova/gh-issues-claims.json"
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.kova/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., kova-kova)
  ```
...
CONFIG_PATH="${KOVA_CONFIG_PATH:-${KOVA_STATE_DIR:-$HOME/.kova}/kova.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.kova/kova.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.kova/kova.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${KOVA_CONFIG_PATH:-${KOVA_STATE_DIR:-$HOME/.kova}/kova.json}"
