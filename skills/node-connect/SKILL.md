---
name: node-connect
description: Diagnose Kova Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---
...
- whether `kova devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `kova qr --json`. It uses the same setup-code payload Android scans.

```bash
kova config get gateway.mode
kova config get gateway.bind
kova config get gateway.tailscale.mode
kova config get gateway.remote.url
kova config get gateway.auth.mode
kova config get gateway.auth.allowTailscale
kova config get plugins.entries.device-pair.config.publicUrl
kova qr --json
kova devices list
kova nodes status
```
...
If `kova qr --json` says `Gateway is only bound to loopback`:
...
- `kova devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run kova qr again, rescan, then approve the pending device pairing.`
