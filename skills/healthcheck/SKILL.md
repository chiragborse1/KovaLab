---
name: healthcheck
description: Audit and harden hosts running Kova for SSH, firewall, updates, exposure, cron checks, and risk posture.
---
...
As part of the default read-only checks, run `kova security audit --deep`. Only offer alternatives if the user requests them:

1. `kova security audit` (faster, non-probing)
2. `kova security audit --json` (structured output)

Offer to apply Kova safe defaults (numbered):

1. `kova security audit --fix`
...
As part of the default read-only checks, run `kova update status`.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `kova cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `kova update status`
...
Before creating, `kova cron list` and match on exact `name`. If found, `kova cron edit <id> ...`.
If not found, `kova cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `kova update status` (preferred for source checkouts and channels)
2. `npm view kova version` (published npm version)

## Kova command accuracy

Use only supported commands and flags:

- `kova security audit [--deep] [--fix] [--json]`
- `kova status` / `kova status --deep`
- `kova health --json`
- `kova update status`
- `kova cron add|list|runs|run`
