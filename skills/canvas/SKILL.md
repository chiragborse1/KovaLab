...
```
http://<tailscale-hostname>:18793/__kova__/canvas/<file>.html
```
...
Place files in the canvas root directory (default `~/kova/canvas/`):

```bash
cat > ~/kova/canvas/my-game.html << 'HTML'
<!DOCTYPE html>
<html>
...
```bash
CONFIG_PATH="${KOVA_CONFIG_PATH:-${KOVA_STATE_DIR:-$HOME/.kova}/kova.json}"
cat "$CONFIG_PATH" | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:18793/__kova__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__kova__/canvas/<file>.html`
...
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__kova__/canvas/snake.html
```
...
1. Check server bind: `CONFIG_PATH="${KOVA_CONFIG_PATH:-${KOVA_STATE_DIR:-$HOME/.kova}/kova.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__kova__/canvas/<file>.html`
...
Node is offline. Use `kova nodes list` to find online nodes.
...
The canvas host serves from `/__kova__/canvas/` prefix:

```
http://<host>:18793/__kova__/canvas/index.html  → ~/kova/canvas/index.html
http://<host>:18793/__kova__/canvas/games/snake.html → ~/kova/canvas/games/snake.html
```

The `/__kova__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
